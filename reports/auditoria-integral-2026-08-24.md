...
|---|---|---|
| Los formularios `SurveyForm` y `SurveyFormWithToken` comparten estructura y lógica de respuestas. | Se extrajeron las tarjetas y el parseo seguro de opciones a `SurveyQuestionCards`; los contratos de envío aún difieren. | Completar un formulario base con estrategia de identidad (`session` o `anonymousToken`) después de unificar los contratos de guardado y envío. |
