    ...
    <div className="relative">
      <Label className="flex items-center gap-1.5 mb-1">
        ...
        <span className="text-xs text-muted-foreground font-normal">
          (opcional — prellenado automático)
        </span>
      </Label>
      <div className="relative">
        <Input
...
function DC3Form({
  ...
  catalogs:
    | {
        cnoAreas: { key: string; label: string }[];
        thematicAreas: { key: string; label: string }[];
      }
    | undefined;
  ...
  const createCompanyMutation = trpc.dc3ClientCompanies.create.useMutation({
    onSuccess: data => {
      toast({
        ...
        description: `${data.razonSocial} agregada al catálogo`,
      });
      setForm({
        ...form,
  ...
  const lookupCurpMutation = trpc.dc3.lookupCurp.useMutation({
    onSuccess: data => {
      if (!data.found) {
        toast({
          title: "CURP inválida",
          description: data.error ?? "Formato de CURP incorrecto",
          variant: "destructive",
        });
        return;
      }
  ...
  const validateRFCMutation = trpc.dc3.validateRFC.useMutation({
    onSuccess: data => {
      if (!data.valid) {
        toast({
          title: "RFC inválido",
          description: data.error ?? "El RFC no es válido",
          variant: "destructive",
        });
      } else {
        const tipo =
  ...
  const rfcValid =
    validateRFCMutation.isSuccess && validateRFCMutation.data?.valid;
  const rfcInvalid =
    validateRFCMutation.isSuccess && !validateRFCMutation.data?.valid;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[65vh] overflow-y-auto pr-2">
        ...
        <EmployeeTypeahead
          onSelect={emp => {
            setForm({
              ...form,
              workerName:
                `${emp.lastName?.toUpperCase() ?? ""} ${emp.firstName?.toUpperCase() ?? ""}`.trim(),
      ...
      <div>
        ...
        <div className="relative">
          <Input
            ...
            onChange={e =>
              setForm({ ...form, workerCurp: e.target.value.toUpperCase() })
            }
            onBlur={handleCurpBlur}
            className="uppercase"
          />
          {lookupCurpMutation.isSuccess &&
            lookupCurpMutation.data?.found &&
            lookupCurpMutation.data.localData && (
      ...
      <div>
        ...
        <Select
          value={form.workerOccupationCnoKey}
          onValueChange={v => {
            const area = catalogs?.cnoAreas.find(a => a.key === v);
            setForm({
              ...form,
              workerOccupationCnoKey: v,
              workerOccupationCnoDesc: area?.label ?? "",
            });
          }}
        >
          <SelectTrigger>
      ...
      <div>
        ...
        <Input
          value={form.workerOccupationCnoDesc}
          onChange={set("workerOccupationCnoDesc")}
        />
      </div>
      <div>
        <Label>Puesto</Label>
        ...
        <div className="mb-3">
          <div className="flex items-center justify-between mb-1">
            <Label className="text-xs text-muted-foreground">
              Seleccionar del catálogo de empresas cliente
            </Label>
            <Button
              type="button"
              variant="ghost"
          ...
          {clientCompanies && clientCompanies.length > 0 ? (
            ...
            <p className="text-xs text-muted-foreground italic">
              No hay empresas en el catálogo. Usa el botón "Nueva empresa" para
              agregar una.
            </p>
          )}
        </div>
        ...
        <Dialog open={showQuickCompany} onOpenChange={setShowQuickCompany}>
          <DialogContent className="max-w-md">
            ...
            <div className="space-y-3 py-2">
              <div>
                ...
                <Input
                  ...
                  value={quickCompany.razonSocial}
                  onChange={setQC("razonSocial")}
                />
              </div>
              <div>
                <Label>RFC</Label>
                <Input
                  placeholder="EEJ900101AAA"
                  value={quickCompany.rfc}
                  onChange={setQC("rfc")}
                  className="uppercase"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  value={quickCompany.representanteLegal}
                  onChange={setQC("representanteLegal")}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  value={quickCompany.domicilioFiscal}
                  onChange={setQC("domicilioFiscal")}
                />
              </div>
              <div>
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setShowQuickCompany(false)}
              >
                Cancelar
              </Button>
              <Button
                disabled={
                  !quickCompany.razonSocial.trim() ||
                ...
                onClick={() =>
                  createCompanyMutation.mutate({
                    ...
                    representanteLegal:
                      quickCompany.representanteLegal.trim() || undefined,
        ...
        <div>
          <Input
            ...
            onChange={e => {
              ...
              if (
                validateRFCMutation.isSuccess ||
                validateRFCMutation.isError
              ) {
          ...
          {rfcValid && validateRFCMutation.data?.rfcFormatted && (
            <p className="text-xs text-green-600 mt-1 flex items-center gap-1">
              ...
              {validateRFCMutation.data.rfcFormatted} ·{" "}
              {validateRFCMutation.data.type === "moral"
                ? "Persona Moral"
                : "Persona Física"}
            </p>
          )}
      ...
      <div>
        ...
        <Input
          ...
          min={1}
          placeholder="Ej. 16"
          value={form.courseDurationHours}
          onChange={set("courseDurationHours")}
        />
      </div>
      <div>
        <Label>Agente Capacitador / STPS</Label>
        <Input
          placeholder="Nombre del agente o empresa capacitadora"
          value={form.trainingAgentName}
          onChange={set("trainingAgentName")}
        />
      </div>
      <div>
      ...
      <div>
        ...
        <Input
          ...
          onChange={set("periodEndDate")}
        />
      </div>
      <div>
        <Label>Área Temática</Label>
        <Select
          value={form.thematicAreaKey}
          onValueChange={v => {
            const area = catalogs?.thematicAreas.find(a => a.key === v);
            setForm({
              ...form,
              thematicAreaKey: v,
              thematicAreaDesc: area?.label ?? "",
            });
          }}
        >
          <SelectTrigger>
      ...
      <div>
        ...
        <Input
          value={form.thematicAreaDesc}
          onChange={set("thematicAreaDesc")}
        />
      </div>
      ...
      <div>
        ...
        <Input
          placeholder="Nombre completo"
          value={form.employerRepName}
          onChange={set("employerRepName")}
        />
      </div>
      <div>
        <Label>Representante de los Trabajadores</Label>
        <Input
          placeholder="Solo para empresas &gt;50 trabajadores"
          value={form.workerRepName}
          onChange={set("workerRepName")}
        />
      </div>
      ...
      <div>
        ...
        <Select
          value={form.status}
          onValueChange={v =>
            setForm({ ...form, status: v as DC3FormData["status"] })
          }
        >
          <SelectTrigger>
            <SelectValue />
      ...
      <div>
        ...
        <Input
          placeholder="DC3-0001/2025"
          value={form.folioNumber}
          onChange={set("folioNumber")}
        />
      </div>
      <div className="md:col-span-2">
        <Label>Notas</Label>
...
export function DC3Manager() {
  ...
  const companyInfoQuery = trpc.systemSettings.getCompanyInfo.useQuery(
    undefined,
    { staleTime: 60_000 }
  );
  // P2: Catálogo de empresas cliente para selector multi-empresa
  const clientCompaniesQuery = trpc.dc3ClientCompanies.list.useQuery(
    undefined,
    { staleTime: 60_000 }
  );
  ...
  const listQuery = trpc.dc3.list.useQuery(
    {
      ...
      thematicAreaFilter:
        thematicAreaFilter !== "all" ? thematicAreaFilter : undefined,
    },
    { staleTime: 5000 }
  );
  ...
  const createMutation = trpc.dc3.create.useMutation({
    onSuccess: () => {
      toast({
        title: "Registro DC-3 creado",
        description: "La constancia fue guardada correctamente.",
      });
  ...
  const templateMutation = trpc.dc3.downloadTemplate.useMutation({
    onSuccess: data => {
      ...
      toast({
        ...
        description: "Abra el archivo en Excel para llenar los datos.",
      });
    },
    onError: e =>
  ...
  const exportMutation = trpc.dc3.exportToExcel.useMutation({
    onSuccess: data => {
      downloadBase64(data.data, data.filename, data.contentType);
      toast({
        title: `${data.count} registros exportados`,
  ...
  const exportSirceMutation = trpc.dc3.exportSirceXml.useMutation({
    onSuccess: data => {
      ...
      URL.revokeObjectURL(url);
      toast({
        title: "XML SIRCE generado",
        description: `${data.count} constancias exportadas al formato SIRCE-STPS.`,
      });
      setExportingSirce(false);
    },
    onError: e => {
      toast({
        title: "Error al exportar SIRCE",
        description: e.message,
        variant: "destructive",
      });
      setExportingSirce(false);
  ...
  const exportPdfMutation = trpc.dc3.exportToPdf.useMutation({
    onSuccess: data => {
      downloadBase64(
        data.pdfBase64,
        `DC3-${data.folioNumber}.pdf`,
        "application/pdf"
      );
      toast({
        title: "PDF generado",
        description: `Constancia ${data.folioNumber} descargada.`,
      });
  ...
  const handleEdit = (
    record: NonNullable<typeof listQuery.data>["records"][number]
  ) => {
    setEditId(record.id);
    setForm({
      ...
      courseDurationHours: record.courseDurationHours
        ? String(record.courseDurationHours)
        : "",
  ...
  const handleSave = () => {
    const payload = {
      ...form,
      courseDurationHours: form.courseDurationHours
        ? parseInt(form.courseDurationHours)
        : null,
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Constancia de Competencias o Habilidades Laborales — Formato oficial
            STPS
          </p>
        </div>
              ...
              <Badge className="ml-1.5 h-4 px-1 text-[10px] bg-primary text-primary-foreground">
                {
                  [
                    ...
                    dateFrom,
                    dateTo,
                    thematicAreaFilter !== "all" ? thematicAreaFilter : "",
      ...
      {selectedCount > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 bg-background border shadow-lg rounded-full px-5 py-2.5 text-sm">
          ...
          <span className="font-medium">
            {selectedCount} constancia{selectedCount !== 1 ? "s" : ""}{" "}
          ...
          <Button
            ...
            onClick={() => {
              ...
              exportSirceMutation.mutate({
                ids: selectableIds.filter(id => selectedIds.has(id)),
              });
            }}
          >
            {exportingSirce ? (
          ...
          {listQuery.isLoading ? (
            ...
            <div className="p-12 text-center">
              ...
              <p className="text-muted-foreground font-medium">
                No hay registros DC-3
              </p>
              <p className="text-sm text-muted-foreground mt-1">
                Descargue la plantilla Excel, llene los datos y use "Importar
                Excel", o agregue un registro manualmente.
              </p>
            </div>
          ) : (
                ...
                <TableBody>
                  {records.map(r => (
                    <TableRow
                      key={r.id}
                      className={
                        selectedIds.has(r.id) ? "bg-primary/5" : undefined
                      }
                    >
                      ...
                      <TableCell className="text-xs text-muted-foreground whitespace-nowrap">
                        {r.periodStartDate
                          ? String(r.periodStartDate).slice(0, 10)
                          : "—"}
                        {r.periodEndDate
                          ? ` → ${String(r.periodEndDate).slice(0, 10)}`
                          : ""}
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-1">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              if (confirm("¿Eliminar este registro DC-3?"))
          ...
          <Card>
            <CardContent className="pt-4 text-sm space-y-2">
              <p className="flex items-start gap-2">
                <span className="font-bold text-primary">1.</span> Descargue la{" "}
                <strong>Plantilla Excel</strong> — contiene 4 hojas: datos,
                catálogo CNO, áreas temáticas e instrucciones.
              </p>
              <p className="flex items-start gap-2">
                <span className="font-bold text-primary">2.</span> Llene los
                datos en la hoja <strong>"DC-3 Datos"</strong> usando los
                catálogos de las hojas de referencia.
              </p>
              <p className="flex items-start gap-2">
                <span className="font-bold text-primary">3.</span> Use{" "}
                <strong>"Importar Excel"</strong> para cargar el archivo. Los
                errores por fila se mostrarán en pantalla.
              </p>
              <p className="flex items-start gap-2">
                <span className="font-bold text-primary">4.</span> Cambie el
                estado a <strong>"Emitida"</strong> para generar el folio
                automático DC3-XXXX/AAAA.
              </p>
              <p className="flex items-start gap-2">
                <span className="font-bold text-primary">5.</span> Use el botón{" "}
                <strong>PDF</strong> (ícono azul) en cada fila para descargar la
                constancia individual en PDF.
              </p>
              <p className="flex items-start gap-2">
                <span className="font-bold text-primary">6.</span> Use{" "}
                <strong>"Exportar Excel"</strong> para descargar todos los
                registros y subirlos al sistema externo (SIRCE-STPS).
              </p>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardContent className="pt-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                {[
                  ...
                  [
                    "CURP",
                    "18 caracteres — Clave Única de Registro de Población",
                  ],
                  [
                    "Clave CNO",
                    "Catálogo Nacional de Ocupaciones STPS — ver catálogo",
                  ],
                  ...
                  [
                    "RFC",
                    "Con homoclave — Servicio de Administración Tributaria",
                  ],
                  [
                    "Nombre del Curso *",
                    "Nombre completo del programa de capacitación",
                  ],
                  ["Duración", "Total de horas del curso (número entero)"],
                  [
                    "Período",
                    "Fecha de inicio y término del curso (YYYY-MM-DD)",
                  ],
                  [
                    "Área Temática",
                    "Clave del catálogo STPS de áreas temáticas",
                  ],
                  [
                    "Agente Capacitador",
                    "Nombre del agente o empresa que impartió el curso",
                  ],
                  ["Instructor", "Nombre del instructor o tutor del curso"],
                  [
                    "Patrón / Rep. Legal",
                    "Nombre del patrón o representante legal de la empresa",
                  ],
                  [
                    "Rep. Trabajadores",
        ...
        <TabsContent value="legal">
          <Card>
            <CardContent className="pt-4 text-sm space-y-2 text-muted-foreground">
              <p>
                • La constancia debe entregarse al trabajador dentro de los{" "}
                <strong>20 días hábiles</strong> siguientes al término del
                curso.
              </p>
              <p>
                • Los datos se asientan bajo{" "}
                <strong>protesta de decir verdad</strong>.
              </p>
              <p>
                • Para empresas con más de 50 trabajadores, firma el
                representante de los trabajadores ante la{" "}
                <strong>Comisión Mixta de Capacitación</strong>.
              </p>
              <p>
                • El formato DC-3 es el documento oficial para acreditar la
                capacitación ante la <strong>STPS</strong> y el{" "}
                <strong>IMSS</strong>.
              </p>
              <p>
                • Los registros deben conservarse por un mínimo de{" "}
                <strong>2 años</strong> conforme a la NOM-035-STPS-2018.
              </p>
              <p className="text-xs">
                Fuente: Formato DC-3 oficial STPS —{" "}
                <a
                  href="https://www.stps.gob.mx"
                  target="_blank"
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>
              {editId ? "Editar Registro DC-3" : "Nuevo Registro DC-3"}
            </DialogTitle>
          </DialogHeader>
          <DC3Form
            form={form}
            ...
            companyFromConfig={
              companyInfoQuery.data
                ? {
                    name: companyInfoQuery.data.company_name ?? "",
                    rfc: companyInfoQuery.data.company_rfc ?? "",
                  }
                : undefined
            }
          />
          ...
          {!editId && (
            <p className="text-xs text-muted-foreground mt-2 flex items-center gap-1">
              ...
              Guarde el registro primero para poder capturar las firmas
              digitales.
            </p>
          )}
