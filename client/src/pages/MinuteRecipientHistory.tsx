...
export default function MinuteRecipientHistory() {
  ...
  const { data, isLoading, error } = trpc.minuteRecipients.getDispatches.useQuery(
  ...
  if (!recipientId || isNaN(recipientId)) {
    return (
      <div className="p-6">
        ...
        <Button variant="outline" onClick={() => setLocation("/committee/minute-recipients")} className="mt-4 gap-2">
  ...
  return (
    <div className="p-6 space-y-6">
      <Breadcrumb items={[
        ...
        { label: "Catálogo de Destinatarios", href: "/committee/minute-recipients" },
        { label: isLoading ? "Cargando..." : recipient?.name ?? "Historial" },
      ]} />

      <div className="flex items-center gap-3">
        <Button variant="outline" size="icon" className="h-9 w-9 shrink-0" onClick={() => setLocation("/committee/minute-recipients")}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-2xl font-bold text-foreground">Historial de Envíos</h1>
          <p className="text-sm text-muted-foreground">Trazabilidad documental de minutas enviadas a este destinatario</p>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="font-semibold text-lg leading-tight">{recipient.name}</p>
                  <Badge className={`text-xs ${recipient.isActive ? "bg-green-100 text-green-700 border-green-200" : "bg-muted text-muted-foreground opacity-60"}`}>
                    {recipient.isActive ? "Activo" : "Inactivo"}
                  </Badge>
                </div>
              </div>
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground pl-12">
                <span className="flex items-center gap-1.5"><Mail className="h-3.5 w-3.5" />{recipient.email}</span>
                <span className="flex items-center gap-1.5"><Briefcase className="h-3.5 w-3.5" />{recipient.position}</span>
                {recipient.department && <span className="flex items-center gap-1.5"><Building2 className="h-3.5 w-3.5" />{recipient.department}</span>}
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        {[
          { icon: <FileText className="h-5 w-5 text-primary" />, bg: "bg-primary/10", label: "Total enviadas", value: stats.total, color: "" },
          { icon: <CheckCircle2 className="h-5 w-5 text-green-600" />, bg: "bg-green-500/10", label: "Leídas", value: stats.read, color: "text-green-600" },
          { icon: <Clock className="h-5 w-5 text-blue-600" />, bg: "bg-blue-500/10", label: "Sin leer", value: stats.unread, color: "text-blue-600" },
          { icon: <BarChart3 className="h-5 w-5 text-amber-600" />, bg: "bg-amber-500/10", label: "Tasa de lectura", value: `${readRate}%`, color: "text-amber-600" },
        ].map((card) => (
          <div key={card.label} className="rounded-lg border bg-card p-4 flex items-center gap-3">
            <div className={`p-2 rounded-md ${card.bg} shrink-0`}>{card.icon}</div>
            <div>
      ...
      <div className="rounded-lg border bg-card overflow-hidden">
        <div className="px-4 py-3 border-b bg-muted/30 flex items-center gap-2">
          ...
          <span className="text-sm font-medium">Minutas enviadas a este destinatario</span>
        </div>
        <Table>
          <TableHeader>
            <TableRow className="bg-muted/50">
              <TableHead className="font-semibold">Folio</TableHead>
              <TableHead className="font-semibold">Título de la Minuta</TableHead>
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2 text-muted-foreground">
                    ...
                    <p className="font-medium">No se han enviado minutas a este destinatario aún.</p>
                    <p className="text-xs">Los envíos aparecerán aquí cuando se vinculen minutas a este destinatario.</p>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              dispatches.map((dispatch) => {
                ...
                return (
                  <TableRow key={dispatch.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="font-medium max-w-xs truncate" title={dispatch.minuteTitle ?? undefined}>
                      {dispatch.minuteTitle ?? <span className="italic text-muted-foreground">Sin título</span>}
                    </TableCell>
                    <TableCell className="text-sm text-muted-foreground">
                      {dispatch.minuteType ? (MEETING_TYPE_LABELS[dispatch.minuteType] ?? dispatch.minuteType) : "—"}
                    </TableCell>
                    <TableCell className="text-sm">{formatDate(dispatch.minuteDate)}</TableCell>
                    <TableCell className="text-sm">{formatDateTime(dispatch.sentAt)}</TableCell>
      ...
      {dispatches.length > 0 && (
        <p className="text-xs text-muted-foreground text-right">
          {stats.total} minuta{stats.total !== 1 ? "s" : ""} enviada{stats.total !== 1 ? "s" : ""} · {stats.read} leída{stats.read !== 1 ? "s" : ""} ({readRate}%)
        </p>
      )}
    </div>
  );
