...
export default function InterventionImpactDashboard() {
  ...
  const [shareReportType, setShareReportType] = useState<"pdf" | "excel">(
    "pdf"
  );
  ...
  const { data: dashboard, isLoading: dashboardLoading } =
    trpc.interventionImpact.getDashboard.useQuery();
  const {
    data: interventions,
    isLoading,
    refetch,
  } = trpc.interventionImpact.list.useQuery({
  ...
  const calculateMetricsMutation =
    trpc.interventionImpact.calculateMetrics.useMutation({
      onSuccess: data => {
        toast.success(
          `Métricas calculadas. Efectividad: ${data.metrics.effectivenessScore}/100`
        );
        refetch();
      },
      onError: error => {
  ...
  const generateInsightsMutation =
    trpc.interventionImpact.generateInsights.useMutation({
      onSuccess: () => {
  ...
  const shareReportMutation =
    trpc.interventionImpact.shareReportByEmail.useMutation({
      onSuccess: data => {
        toast.success(
          `Reporte enviado exitosamente a ${data.recipientCount} destinatario(s)`
        );
  ...
  const handleCreate = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    createMutation.mutate({
      ...
      targetDepartmentId: formData.get("targetDepartmentId")
        ? Number(formData.get("targetDepartmentId"))
        : undefined,
  ...
  const chartData = {
    labels:
      interventions
        ?.slice(0, 10)
        .map((i: any) => i.interventionName.substring(0, 20)) || [],
    datasets: [
      {
        ...
        data:
          interventions
            ?.slice(0, 10)
            .map((i: any) => Number(i.effectivenessScore) || 0) || [],
    ...
    <div className="container py-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Mide la efectividad de acciones correctivas y su correlación con la
            reducción de casos
          </p>
        </div>
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            onClick={handleExportAndSharePDF}
            disabled={exportPDFMutation.isPending}
          >
          ...
          <Button
            variant="outline"
            onClick={handleExportAndShareExcel}
            disabled={exportExcelMutation.isPending}
          >
            <Share2 className="mr-2 h-4 w-4" />
            Compartir Excel
            ...
            <DialogContent className="max-w-2xl">
              <form onSubmit={handleCreate}>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Registra una intervención para analizar su impacto en la
                    reducción de riesgos psicosociales
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    <Label htmlFor="interventionType">
                    ...
                    <Select name="interventionType" required>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="organizational_change">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="grid gap-2">
                      <Label htmlFor="measurementPeriodMonths">
                        Período de Medición (meses) *
                      </Label>
                      <Input
                  ...
                  <div className="grid gap-2">
                    <Label htmlFor="targetDepartmentId">
                      ID Departamento Objetivo (opcional)
                    </Label>
                    <Input
      ...
      <div className="grid gap-4 md:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {Number(dashboard?.avgEffectiveness || 0).toFixed(1)}/100
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Últimas 10 intervenciones registradas
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Comparación de métricas antes y después de cada intervención
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  >
                    ...
                    <td className="p-2">
                      <Badge variant="outline">
                        {intervention.interventionType === "training" &&
                          "Capacitación"}
                        {intervention.interventionType === "policy_change" &&
                          "Cambio de Política"}
                        ...
                        {intervention.interventionType ===
                          "awareness_campaign" && "Campaña"}
                        {intervention.interventionType === "other" && "Otro"}
                      </Badge>
                    </td>
                    <td className="p-2 text-center">
                      {intervention.casesBeforeCount}
                    </td>
                    <td className="p-2 text-center">
                      {intervention.casesAfterCount}
                    </td>
                    <td className="p-2 text-center">
                      {intervention.caseReductionPercentage ? (
                        <span
                          className={
                            Number(intervention.caseReductionPercentage) > 0
                              ? "text-green-600 font-semibold"
                              : "text-red-600"
                          }
                        >
                          {Number(intervention.caseReductionPercentage).toFixed(
                            1
                          )}
                          %
                        </span>
                    ...
                    <td className="p-2 text-center">
                      {intervention.effectivenessScore ? (
                        <Badge
                          variant={
                            Number(intervention.effectivenessScore) >= 70
                              ? "default"
                              : "secondary"
                          }
                        >
                          {Number(intervention.effectivenessScore).toFixed(1)}
                          /100
                        </Badge>
                      ) : (
                        "-"
      ...
      <Dialog open={detailsDialogOpen} onOpenChange={setDetailsDialogOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Análisis detallado de impacto y insights generados por IA
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {selectedIntervention.aiInsights.successFactors?.map(
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {selectedIntervention.aiInsights.challenges?.map(
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {selectedIntervention.aiInsights.recommendations?.map(
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {selectedIntervention.aiInsights.predictedImpact}
                  </p>
                </div>
              </>
            )}
            {!selectedIntervention?.aiInsights && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>
                  No se han generado insights con IA para esta intervención.
                </p>
                <Button
                  className="mt-4"
                  onClick={() => {
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Ranking de intervenciones con mayor impacto positivo
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
              ...
              .map((intervention, idx) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-bold text-green-600">
                      {Number(intervention.effectivenessScore || 0).toFixed(1)}
                      /100
                    </div>
                    <div className="text-sm text-muted-foreground">
                      {intervention.caseReductionPercentage
                        ? `${Number(intervention.caseReductionPercentage).toFixed(1)}% reducción`
                        : "Sin datos"}
                    </div>
                  </div>
                </div>
      ...
      <Dialog open={shareDialogOpen} onOpenChange={setShareDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Comparte el reporte {shareReportType === "pdf" ? "PDF" : "Excel"}{" "}
              ...
              <form onSubmit={handleShareByEmail} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Envía el reporte directamente a los destinatarios
                  </p>
                </div>
