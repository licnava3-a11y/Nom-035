...
export default function TokenManagement() {
  ...
  const { data: activeTokens, refetch: refetchTokens } =
    trpc.surveyTokensAdvanced.getActiveTokens.useQuery({
      periodId: filterPeriodId,
    });
  const { data: tokenStats } = trpc.surveyTokensAdvanced.getTokenStats.useQuery(
    { periodId: filterPeriodId || 0 }
  );
  ...
  const generateBulkTokens =
    trpc.surveyTokensAdvanced.generateBulkTokens.useMutation({
      onSuccess: data => {
  ...
  const handleGenerateBulkTokens = () => {
    if (selectedUserIds.length === 0 || !bulkPeriodId || !bulkSurveyId) {
      toast.error(
        "Por favor selecciona al menos un usuario y completa todos los campos"
      );
      return;
    }
        ...
        <TabsContent value="bulk">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-2">
                <Label>
                  Seleccionar Usuarios ({selectedUserIds.length} seleccionados)
                </Label>
              ...
              >
                ...
                {generateBulkTokens.isPending
                  ? "Generando..."
                  : `Generar ${selectedUserIds.length} Tokens`}
              </ProtectedButton>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <Select
                    value={filterPeriodId ? String(filterPeriodId) : "all"}
                    onValueChange={value =>
                      setFilterPeriodId(
