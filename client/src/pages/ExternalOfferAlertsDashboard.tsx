...
export default function ExternalOfferAlertsDashboard() {
  const { data: activeAlerts, isLoading, refetch } = trpc.externalOfferAlerts.getActiveAlerts.useQuery();
  const { data: stats } = trpc.externalOfferAlerts.getAlertStats.useQuery();
  const { data: byDepartment } = trpc.externalOfferAlerts.getAlertsByDepartment.useQuery();
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Empleados clave en riesgo de recibir ofertas externas</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{stats?.high_alerts || 0}</div>
            <p className="text-xs text-muted-foreground">Acción en 30-60 días</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats?.avg_risk_score ? parseFloat(stats.avg_risk_score as string).toFixed(1) : "0.0"}
            </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Empleados en riesgo de recibir ofertas externas</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
              ...
              <TableBody>
                {activeAlerts?.map((alert: any) => (
                  <TableRow key={alert.id}>
                    ...
                    <TableCell>
                      <span className="font-semibold">{parseFloat(alert.riskScore).toFixed(0)}</span>/100
                    </TableCell>
                    <TableCell>
                      <span className={parseFloat(alert.salaryGapPercentage) < -20 ? "text-red-600 font-semibold" : ""}>
                  ...
                  <TableRow key={index}>
                    ...
                    <TableCell>
                      {dept.critical_count > 0 && (
                        <Badge variant="destructive">{dept.critical_count}</Badge>
                      )}
                    </TableCell>
                    <TableCell>{parseFloat(dept.avg_risk_score).toFixed(1)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
