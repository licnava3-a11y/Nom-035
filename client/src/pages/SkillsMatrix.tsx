...
export default function SkillsMatrix() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.skillsMatrix.getMatrix.useQuery(filters);
  const { data: importHistory } = trpc.skillsMatrix.getImportHistory.useQuery();

  // Mutations
  const updateSkillMutation =
    trpc.skillsMatrix.setEmployeeSkillLevel.useMutation({
      onSuccess: () => {
        toast.success("Nivel actualizado", {
          description: "El nivel de habilidad se actualizó correctamente",
        });
        refetch();
        setEditingCell(null);
      },
  ...
  const generateTrainingProgramMutation =
    trpc.skillsMatrix.generateTrainingProgram.useMutation({
      onSuccess: result => {
  ...
  const saveSnapshotMutation =
    trpc.skillsMatrixSnapshots.saveSnapshot.useMutation({
      onSuccess: () => {
        toast.success("Snapshot guardado", {
          description: "El snapshot se guardó exitosamente",
        });
  ...
  const handleGenerateTrainingProgram = () => {
    if (
      window.confirm(
        "¿Deseas generar automáticamente el programa de capacitación basado en las brechas de habilidades identificadas?\n\nEsto agregará las competencias prioritarias al programa personal de cada empleado."
      )
    ) {
  ...
  const handleSaveSnapshot = () => {
    if (!snapshotName.trim()) {
      toast.error("Error", {
        description: "El nombre del snapshot es requerido",
      });
      return;
    }
  ...
  const handleExport = async () => {
    try {
      ...
      XLSX.utils.sheet_add_json(ws, data.data, {
        origin: -1,
        skipHeader: false,
      });
      ...
      if (data.developmentAnalysis && data.developmentAnalysis.length > 0) {
        const devMetadata = [
          ...
          [
            "Descripción:",
            "Brechas de habilidades y sugerencias de capacitación por empleado",
          ],
          [],
        ];
        const wsDevAnalysis = XLSX.utils.aoa_to_sheet(devMetadata);
        XLSX.utils.sheet_add_json(wsDevAnalysis, data.developmentAnalysis, {
          origin: -1,
          skipHeader: false,
        });
      ...
      if (data.successionAnalysis) {
        ...
        const succMetadata = [
          ...
          [
            "Descripción:",
            "Empleados con mayor potencial ordenados por nivel de competencia",
          ],
          [],
        ];
        const wsSuccession = XLSX.utils.aoa_to_sheet(succMetadata);
        XLSX.utils.sheet_add_json(wsSuccession, successionData, {
          origin: -1,
          skipHeader: false,
        });
      ...
      if (data.trainingRecommendations) {
        ...
        const trainMetadata = [
          ...
          [
            "Descripción:",
            "Top 5 competencias con mayor brecha por departamento",
          ],
          [],
        ];
        const wsTraining = XLSX.utils.aoa_to_sheet(trainMetadata);
        XLSX.utils.sheet_add_json(wsTraining, trainingData, {
          origin: -1,
          skipHeader: false,
        });
      ...
      if (data.developmentAnalysis && data.developmentAnalysis.length > 0) {
        ...
        const avgCompetencyLevel = (
          data.developmentAnalysis.reduce(
            (sum: number, emp: any) =>
              sum + parseFloat(emp.nivelPromedio || "0"),
            0
          ) / totalEmployees
        ).toFixed(2);
        const employeesWithGaps = data.developmentAnalysis.filter(
          (emp: any) => emp.brechasIdentificadas > 0
        ).length;
        const gapPercentage = (
          (employeesWithGaps / totalEmployees) *
          100
        ).toFixed(1);
        const totalDepartments = Object.keys(
          data.successionAnalysis || {}
        ).length;
        const highPotentialCount = data.developmentAnalysis.filter(
          (emp: any) => parseFloat(emp.nivelPromedio || "0") >= 3
        ).length;
        ...
        const executiveSummary = [
          ...
          [
            "Nivel Promedio de Competencia",
            avgCompetencyLevel,
            "Escala: 0 (Sin evaluar) a 4 (Experto)",
          ],
          [
            "Empleados con Brechas de Habilidades",
            `${employeesWithGaps} (${gapPercentage}%)`,
            "Requieren capacitación",
          ],
          [
            "Empleados de Alto Potencial",
            `${highPotentialCount} (${((highPotentialCount / totalEmployees) * 100).toFixed(1)}%)`,
            "Nivel ≥ 3.0 (Avanzado)",
          ],
          [
        ...
        executiveSummary.push(
          [
            "Sin evaluar / Básico (0-1)",
            levelCounts["0-1"].toString(),
            `${((levelCounts["0-1"] / totalEmployees) * 100).toFixed(1)}%`,
          ],
          [
            "Básico / Intermedio (1-2)",
            levelCounts["1-2"].toString(),
            `${((levelCounts["1-2"] / totalEmployees) * 100).toFixed(1)}%`,
          ],
          [
            "Intermedio / Avanzado (2-3)",
            levelCounts["2-3"].toString(),
            `${((levelCounts["2-3"] / totalEmployees) * 100).toFixed(1)}%`,
          ],
          [
            "Avanzado / Experto (3-4)",
            levelCounts["3-4"].toString(),
            `${((levelCounts["3-4"] / totalEmployees) * 100).toFixed(1)}%`,
          ],
          [""],
        ...
        // Auto-size columns
        wsExecutive["!cols"] = [{ wch: 40 }, { wch: 20 }, { wch: 50 }];
      }
      ...
      toast.success(
        `Exportación exitosa: ${data.data.length} empleados + análisis completo`
      );
    } catch (error: any) {
      toast.error("Error", {
        description: error.message || "No se pudo exportar la matriz",
      });
    }
  };
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={value =>
                  setFilters({
                    ...filters,
                    positionId: value === "all" ? undefined : parseInt(value),
                  })
                }
              >
                <SelectTrigger>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {
                competencyAverages.filter(
                  (c: { average: number }) => c.average < 2
                ).length
              }
            </div>
            <p className="text-xs text-muted-foreground">
                        ...
                        {competencies.map((comp: { id: number }) => {
                          const entry = matrix.find(
                            (m: { employeeId: number; competencyId: number }) =>
                              m.employeeId === emp.id &&
                              m.competencyId === comp.id
                          );
                          const level: SkillLevel =
                            (entry?.level as SkillLevel) || "Sin evaluar";
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <LoadingButton
              ...
              loading={saveSnapshotMutation.isPending}
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <p className="text-sm text-muted-foreground">
              El archivo debe tener las siguientes columnas: email, nombre,
              departamento, puesto, y una columna por cada competencia con los
                      ...
                      (imp: {
                        ...
                        <div key={imp.id} className="flex justify-between">
                          ...
                          <span className="text-muted-foreground">
                            {imp.recordsImported} importados,{" "}
                            {imp.recordsFailed} fallidos
                          </span>
                        </div>
                      )
                    )}
