...
export default function SurveysAdminPanel() {
  ...
  const { data: stats, isLoading: statsLoading } =
    trpc.surveysAdmin.getStats.useQuery({
      surveyType,
  ...
  const { data: responsesData, isLoading: responsesLoading } =
    trpc.surveysAdmin.getResponses.useQuery({
      surveyType,
      status,
  ...
  const filteredResponses =
    responsesData?.responses.filter(r => {
  ...
  const handleExport = async () => {
    try {
      ...
      const metadata = [
        ...
        [
          "  - Tipo de Encuesta:",
          surveyType === "all" ? "Todas" : getSurveyTypeLabel(surveyType),
        ],
        [
          "  - Estado:",
          status === "all"
      ...
      const excelData = data.map(r => ({
        ...
        "Fecha Inicio": r.startedAt
          ? new Date(r.startedAt).toLocaleString("es-MX")
          : "N/A",
        "Fecha Completado": r.completedAt
          ? new Date(r.completedAt).toLocaleString("es-MX")
          : "En progreso",
      ...
      const typeLabel =
        surveyType === "all" ? "todas" : surveyType.replace("guia_", "guia");
      const filename = `encuestas_nom035_${typeLabel}_${timestamp}.xlsx`;

      // Download
      XLSX.writeFile(wb, filename);
      toast.success(
        `Archivo Excel generado: ${data.length} registros exportados`
      );
    } catch (error) {
  ...
  const getSurveyTypeLabel = (type: string) => {
    switch (type) {
      case "guia_i":
        return "Guía I - ATS";
      case "guia_ii":
        return "Guía II - Identificación";
      case "guia_iii":
        return "Guía III - Evaluación";
      default:
        return type;
    }
  };
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filtra las respuestas por tipo de encuesta, estado, departamento y
            periodo
          </CardDescription>
        </CardHeader>
                ...
                <TableBody>
                  {filteredResponses.map((response: any) => (
                    <TableRow key={response.id}>
                      ...
                      <TableCell>
                        {getStatusBadge(response.completedAt)}
                      </TableCell>
