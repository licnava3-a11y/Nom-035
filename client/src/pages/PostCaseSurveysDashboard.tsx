...
export default function PostCaseSurveysDashboard() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<
    "pending" | "sent" | "completed" | "expired" | undefined
  >();
  ...
  const { data: stats, isLoading: statsLoading } =
    trpc.postCaseSurveys.getEffectivenessStats.useQuery();
  const {
    data: surveys,
    isLoading: surveysLoading,
    refetch: refetchSurveys,
  } = trpc.postCaseSurveys.getAllSurveys.useQuery({
  ...
  // Mutations para jobs
  const createPendingSurveysMutation =
    trpc.postCaseSurveys.createPendingSurveys.useMutation({
  ...
  const sendPendingSurveysMutation =
    trpc.postCaseSurveys.sendPendingSurveys.useMutation({
      onSuccess: (result: any) => {
          ...
          tooltip: {
            callbacks: {
              afterLabel: context => {
                ...
                const count =
                  stats.byPeriod[period as "30" | "60" | "90"].count;
    ...
    try {
      ...
      const surveyRows = surveys.map((item: any) => {
        ...
        return {
          ...
          "Fecha Creación": item.survey.createdAt
            ? new Date(item.survey.createdAt).toLocaleDateString("es-MX")
            : "",
          "Fecha Envío": item.survey.sentAt
            ? new Date(item.survey.sentAt).toLocaleDateString("es-MX")
            : "",
          "Fecha Completada": item.survey.completedAt
      ...
      if (stats) {
        const summaryRows = [
          ...
          { Métrica: "", Valor: "" },
          {
            Métrica: "Encuestas completadas 30 días",
            Valor: stats.byPeriod["30"].count,
          },
          {
            Métrica: "Score promedio 30 días",
            Valor: stats.byPeriod["30"].avgScore,
          },
          {
            Métrica: "Encuestas completadas 60 días",
            Valor: stats.byPeriod["60"].count,
          },
          {
            Métrica: "Score promedio 60 días",
            Valor: stats.byPeriod["60"].avgScore,
          },
          {
            Métrica: "Encuestas completadas 90 días",
            Valor: stats.byPeriod["90"].count,
          },
          {
            Métrica: "Score promedio 90 días",
            Valor: stats.byPeriod["90"].avgScore,
          },
        ];
    ...
    try {
      ...
      if (stats) {
        ...
        const summaryData = [
          [
            ...
            "Score General (1-5)",
            String(stats.overallScore),
          ],
          [
            "Promedio Mejora",
            String(stats.avgImprovement),
            "Promedio Satisfacción",
            String(stats.avgSatisfaction),
          ],
          [
            "Promedio Apoyo",
            String(stats.avgSupport),
            "Promedio Recomendación",
            String(stats.avgRecommendation),
          ],
        ];
        autoTable(doc, {
          startY: 35,
        ...
        const periodData = Object.entries(stats.byPeriod).map(
          ([days, data]: [string, any]) => [
            `${days} días`,
            String(data.count),
      ...
      const tableRows = surveys.map((item: any) => {
        ...
        return [
          item.caseNumber || "—",
          item.caseType || "—",
          item.survey.daysSinceClosure
            ? `${item.survey.daysSinceClosure}d`
            : "—",
          item.survey.status || "—",
          ...
          item.survey.comments
            ? item.survey.comments.substring(0, 60) +
              (item.survey.comments.length > 60 ? "..." : "")
            : "—",
        ];
      });
      autoTable(doc, {
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        doc.text(
          `Página ${i} de ${pageCount} — Plataforma NOM-035 STPS 2018 — Confidencial`,
          14,
          205
        );
      }
      ...
      <div className="container mx-auto py-8">
        <div className="flex items-center justify-center h-64">
          <p className="text-muted-foreground">
            Cargando dashboard de encuestas...
          </p>
        </div>
      </div>
    );
    ...
    <div className="container mx-auto py-8 space-y-6">
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-1 mt-1">
              {[...Array(5)].map((_, i) => (
                <Star
                  key={i}
                  className={`h-3 w-3 ${i < Math.round(stats.overallScore) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`}
                />
              ))}
            </div>
          </CardContent>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Score promedio según días transcurridos desde cierre
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={v =>
                  setSelectedPeriod(v === "all" ? undefined : (v as any))
                }
              >
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue placeholder="Todos" />
            ...
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={v =>
                  setSelectedDepartmentId(v === "all" ? undefined : parseInt(v))
                }
              >
                <SelectTrigger className="h-8 text-xs">
                  <SelectValue placeholder="Todos" />
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {surveysLoading
                  ? "Cargando..."
                  : `${surveys?.length || 0} encuestas encontradas`}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="text-center p-2">
                          <Badge variant="secondary" className="text-xs">
                            {item.survey.daysSinceClosure} días
                          </Badge>
                        </td>
                        <td className="text-center p-2">
                        ...
                        <td className="text-center p-2 text-xs text-muted-foreground">
                          {item.survey.sentAt
                            ? new Date(item.survey.sentAt).toLocaleDateString(
                                "es-MX"
                              )
                            : "-"}
                        </td>
                        ...
                        <td className={`text-center p-2 ${scoreColor}`}>
                          {avgScore}
                        </td>
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-muted-foreground">
          <p>
            • <strong>Creación automática:</strong> El job detecta casos
            cerrados que cumplan 30, 60 o 90 días y crea encuestas pendientes
            automáticamente.
          </p>
          <p>
            • <strong>Envío:</strong> El job "Enviar Pendientes" envía correos
            HTML a los reportantes con un enlace único para responder sin login.
            Recordatorio automático a los 3 días si no hay respuesta.
          </p>
          <p>
            • <strong>Expiración:</strong> Las encuestas no respondidas en 7
            días se marcan como expiradas automáticamente.
          </p>
          <p>
            • <strong>Ratings:</strong> Cada encuesta mide 4 aspectos en escala
            1-5: Mejora de la situación, Satisfacción con la resolución, Apoyo
            recibido y Recomendación del proceso.
          </p>
          <p>
            • <strong>Exportación:</strong> El botón "Exportar Excel" genera un
            archivo .xlsx con dos hojas: listado completo de encuestas y resumen
            de efectividad para reportes NOM-035 ante la STPS.
          </p>
        </CardContent>
      </Card>
    </div>
