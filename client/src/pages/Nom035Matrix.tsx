...
function ActionRow({
  ...
  const prioridadConf =
    PRIORIDAD_CONFIG[action.prioridad] || PRIORIDAD_CONFIG.media;
  const isVencida =
    action.plazo &&
    new Date(action.plazo) < new Date() &&
    action.estado !== "cumplida";

  return (
    <TableRow className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
      <TableCell className="font-mono text-xs font-bold text-gray-500 w-20">
        {action.accionId}
      </TableCell>
      ...
      <TableCell>
        <span
          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${estadoConf.color}`}
        >
          {estadoConf.icon}
          {estadoConf.label}
        </span>
      ...
      <TableCell>
        <span
          className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${prioridadConf.color}`}
        >
          {prioridadConf.label}
        </span>
      </TableCell>
      ...
      <TableCell>
        {action.plazo ? (
          <span
            className={`text-xs ${isVencida ? "text-red-600 font-semibold" : "text-gray-600 dark:text-gray-400"}`}
          >
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6">
      {[
        ...
        {
          label: "Vencidas",
          value: Number(stats.vencidas) || 0,
          color: "text-red-600",
        },
        {
          label: "Con evidencia",
          value: Number(stats.conEvidencia) || 0,
          color: "text-blue-600",
        },
        {
          label: "% Cumplimiento",
...
export function Nom035Matrix() {
  ...
  const [viewEvidencesAction, setViewEvidencesAction] = useState<any | null>(
    null
  );
  ...
  // Queries
  const plansQuery = trpc.nom035Matrix.listPlans.useQuery({
    status: "activo",
    page: 1,
    pageSize: 20,
  });
  ...
  const createEvidenceToken =
    trpc.nom035Matrix.createEvidenceToken.useMutation();
  ...
  const handleExportPdf = async () => {
    if (!selectedPlanId) {
      toast({
        ...
        description: "Debes seleccionar un plan para generar el PDF.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleEvidenceUploaded = () => {
    ...
    toast({
      ...
      description: "La evidencia se agregó correctamente a la acción.",
    });
  };
    ...
    <div className="p-6 max-w-full">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Gestión de planes de intervención, acciones y evidencias de
            cumplimiento
          </p>
        </div>
        <div className="flex gap-2">
          <Button
            variant="outline"
            size="sm"
            onClick={() => utils.nom035Matrix.listActions.invalidate()}
          >
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        ...
        <Select
          ...
          onValueChange={val =>
            setSelectedPlanId(val === "all" ? null : Number(val))
          }
        >
              ...
              <SelectItem key={p.id} value={p.id.toString()}>
                <span className="flex items-center gap-1">
                  ...
                  <span className="truncate max-w-40">
                    {p.identificadorNivel}
                  </span>
                  <Badge variant="outline" className="text-xs ml-1">
                    {TIPO_PLAN_LABELS[p.tipoPlan]}
                  </Badge>
                </span>
              </SelectItem>
            ))}
              ...
              <TableRow>
                <TableCell colSpan={8} className="text-center py-12">
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Crea un nuevo plan para generar acciones automáticamente
                  </p>
                  <Button
                    size="sm"
                    className="mt-3"
      ...
      {actionsQuery.data && actionsQuery.data.total > 25 && (
        <div className="flex items-center justify-between mt-4">
          <p className="text-sm text-gray-500">
            ...
            {Math.min(filters.page * 25, actionsQuery.data.total)} de{" "}
            {actionsQuery.data.total} acciones
          </p>
          <div className="flex gap-2">
            <Button
              variant="outline"
      ...
      <Dialog
        open={!!uploadAction}
        onOpenChange={open => !open && setUploadAction(null)}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Acción:{" "}
              <span className="font-semibold">{uploadAction?.accionId}</span> —{" "}
              {uploadAction?.objetivo?.substring(0, 80)}
            </DialogDescription>
          </DialogHeader>
          {uploadAction && (
            <EvidenceUploader
      ...
      <Dialog
        open={!!viewEvidencesAction}
        onOpenChange={open => !open && setViewEvidencesAction(null)}
      >
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              Evidencias — {viewEvidencesAction?.accionId}
            </DialogTitle>
            <DialogDescription>
              {viewEvidencesAction?.objetivo?.substring(0, 100)}
            </DialogDescription>
          </DialogHeader>
          {viewEvidencesAction && (
      ...
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              Genera un enlace temporal (72 horas, máx. 3 usos) para que un
              responsable externo suba evidencias sin necesitar cuenta.
            </DialogDescription>
          </DialogHeader>
          {shareAction && (
                ...
                <div className="space-y-3">
                  <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-lg p-3">
                    ...
                    <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-2">
                      ⏱ Válido hasta:{" "}
                      {shareResult.expiresAt.toLocaleString("es-MX", {
                        dateStyle: "long",
                        timeStyle: "short",
                      })}
                    </p>
                  ...
                  <p className="text-xs text-gray-500">
                    Comparte este enlace por correo o WhatsApp. El destinatario
                    podrá subir hasta 3 archivos sin necesitar cuenta en la
                    plataforma.
                  </p>
                </div>
              )}
      ...
      <Dialog open={showNewPlanDialog} onOpenChange={setShowNewPlanDialog}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              La IA generará un plan con acciones específicas basado en los
              datos de evaluación NOM-035.
            </DialogDescription>
          </DialogHeader>
          <NewPlanForm
            onSuccess={planId => {
              ...
              toast({
                title: "Plan generado",
                description:
                  "El plan y sus acciones fueron creados correctamente.",
              });
            }}
  ...
  if (!evidencesQuery.data?.length) {
    return (
      <div className="py-8 text-center text-gray-500">
        ...
        <p className="text-sm">
          No hay evidencias registradas para esta acción.
        </p>
      </div>
    );
  }
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <Badge variant="secondary" className="text-xs">
                {TIPO_LABELS[ev.tipoEvidencia] || ev.tipoEvidencia}
              </Badge>
              {ev.descripcion && (
...
function NewPlanForm({
  ...
  const [form, setForm] = useState({
    nivelAplicacion: "organizacional" as
      | "organizacional"
      | "grupal"
      | "individual",
    tipoPlan: "intervencion" as
      | "intervencion"
      | "violencia_laboral"
      | "no_discriminacion"
      | "consolidado",
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.identificadorNivel.trim()) {
      toast({
        title: "Falta el nombre del centro/área",
        variant: "destructive",
      });
      return;
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Select
            value={form.nivelAplicacion}
            onValueChange={val =>
              setForm(f => ({ ...f, nivelAplicacion: val as any }))
            }
          >
        ...
        <div>
          ...
          <Select
            value={form.tipoPlan}
            onValueChange={val =>
              setForm(f => ({ ...f, tipoPlan: val as any }))
            }
          >
            <SelectTrigger className="mt-1">
              <SelectValue />
      ...
      <div>
        <label className="text-xs font-medium text-gray-700 dark:text-gray-300">
          Nombre de la empresa / área / trabajador *
        </label>
        <Input
          value={form.identificadorNivel}
          onChange={e =>
            setForm(f => ({ ...f, identificadorNivel: e.target.value }))
          }
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            ...
            onChange={e =>
              setForm(f => ({ ...f, centroTrabajo: e.target.value }))
            }
        ...
        <div>
          ...
          <Input
            ...
            onChange={e =>
              setForm(f => ({ ...f, giroEmpresa: e.target.value }))
            }
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <Input
            ...
            onChange={e =>
              setForm(f => ({ ...f, totalTrabajadores: e.target.value }))
            }
            placeholder="Ej. 150"
            className="mt-1"
          />
        ...
        <div>
          ...
          <Input
            value={form.filtroAplicado}
            onChange={e =>
              setForm(f => ({ ...f, filtroAplicado: e.target.value }))
            }
