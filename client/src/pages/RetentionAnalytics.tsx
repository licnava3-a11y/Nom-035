...
import { AlertTriangle, TrendingDown, TrendingUp, Minus, Bell, RefreshCw } from "lucide-react";
...
export default function RetentionAnalytics() {
  ...
  // Queries
  const { data: stats, isLoading: statsLoading } = trpc.predictiveAnalytics.getRetentionStats.useQuery();
  const { data: atRiskData, isLoading: atRiskLoading, refetch } = trpc.predictiveAnalytics.identifyAtRiskEmployees.useQuery({
  ...
  const getTrendIcon = (trend: string) => {
    if (trend === "descending") return <TrendingDown className="h-4 w-4 text-red-500" />;
    if (trend === "ascending") return <TrendingUp className="h-4 w-4 text-green-500" />;
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Análisis de Retención de Talento</h1>
          <p className="text-muted-foreground">
            Identificación de empleados en riesgo de rotación basado en tendencias de competencias (Evaluación 360°)
          </p>
        </div>
        <LoadingButton
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{statsLoading ? "..." : stats?.totalActiveEmployees || 0}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{statsLoading ? "..." : `${stats?.retentionRate || 0}%`}</div>
            <p className="text-xs text-muted-foreground mt-1">
              {statsLoading ? "..." : `${stats?.totalAtRisk || 0} en riesgo (${stats?.atRiskPercentage || 0}%)`}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{statsLoading ? "..." : stats?.criticalRisk || 0}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-orange-600">{statsLoading ? "..." : stats?.highRisk || 0}</div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Personaliza la visualización de empleados en riesgo</CardDescription>
        </CardHeader>
        <CardContent className="flex gap-4 items-end">
          <div className="flex-1">
            <label className="text-sm font-medium mb-2 block">Departamento</label>
            <Select value={departmentFilter?.toString() || "all"} onValueChange={(v) => setDepartmentFilter(v === "all" ? undefined : Number(v))}>
              <SelectTrigger>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {atRiskLoading ? "Cargando..." : `${atRiskData?.totalAtRisk || 0} empleados identificados con score &lt; ${minScore}`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {atRiskLoading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">Score de Retención</TableHead>
                ...
                {atRiskData?.employees.map((employee: any) => (
                  <TableRow key={employee.employeeId}>
                    ...
                    <TableCell className="text-center">
                      <span className={getScoreColor(employee.retentionScore)}>{employee.retentionScore}</span>
                    </TableCell>
                    <TableCell className="text-center">
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">({employee.trendValue > 0 ? "+" : ""}{employee.trendValue})</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-center">{getRiskBadge(employee.riskLevel)}</TableCell>
                    <TableCell className="text-center">{employee.evaluationCount}</TableCell>
                    <TableCell>
                      {employee.lastEvaluationDate
                        ? new Date(employee.lastEvaluationDate).toLocaleDateString("es-MX")
                        : "N/A"}
                    </TableCell>
                  </TableRow>
                ))}
