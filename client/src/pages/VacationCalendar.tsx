...
export default function VacationCalendar() {
  ...
  const { data: deptsData } = trpc.departments.list.useQuery({
    page: 1,
    pageSize: 100,
    isActive: true,
  });
  ...
  const daysInMonth =
    calendarData?.daysInMonth ?? new Date(year, month, 0).getDate();
  ...
  const conflictDays = useMemo(() => {
    ...
    for (const entry of calendarData.entries) {
      ...
      if (!deptTotals[entry.department])
        deptTotals[entry.department] = new Set();
  ...
  const getGanttBar = (startDate: string, endDate: string) => {
    ...
    return {
      left: `${left.toFixed(2)}%`,
      width: `${Math.max(width, 1).toFixed(2)}%`,
    };
  };
  ...
  const getMonthlyEntries = (day: number) => {
    ...
    return calendarData.entries.filter(
      e => String(e.startDate) <= dateStr && String(e.endDate) >= dateStr
    );
  };
    ...
    <div className="container mx-auto py-8 space-y-6">
      ...
      {hasConflicts && (
        <Alert variant="destructive" className="no-print">
          ...
          <AlertTitle>
            Conflictos de ausencias simultáneas detectados
          </AlertTitle>
          <AlertDescription>
            <p className="mb-2 text-sm">
              Los siguientes departamentos tienen días con más del{" "}
              {Math.round(CONFLICT_THRESHOLD * 100)}% de su equipo ausente
              simultáneamente:
            </p>
              ...
              {Object.entries(conflictsByDept).map(([dept, days]) => {
                ...
                return (
                  <div key={dept} className="flex items-center gap-2 text-sm">
                    ...
                    <span>
                      Días {ranges.join(", ")} de {MONTH_NAMES[month - 1]} —
                      hasta {maxPct}% del equipo ausente
                    </span>
                  </div>
                );
              })}
      ...
      <div id="vacation-calendar-print">
        ...
        <div className="hidden print:block mb-4">
          <h1 className="text-xl font-bold">
            Calendario de Vacaciones — {MONTH_NAMES[month - 1]} {year}
          </h1>
          <p className="text-sm text-gray-500">
            Generado el {today.toLocaleDateString("es-MX")}
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12 gap-3">
              ...
              <p className="text-muted-foreground">
                No hay vacaciones registradas para {MONTH_NAMES[month - 1]}{" "}
                {year}
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                Vista Gantt — {MONTH_NAMES[month - 1]} {year}
              </CardTitle>
              <CardDescription>
                {calendarData.entries.length} período(s) en este mes
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <div style={{ minWidth: "700px" }}>
                  ...
                  <div className="flex mb-2">
                    <div className="w-48 shrink-0 text-xs text-muted-foreground font-medium pr-2">
                      Empleado / Depto.
                    </div>
                      ...
                      {month === today.getMonth() + 1 &&
                        year === today.getFullYear() && (
                          <div
                            ...
                            style={{
                              left: `${((today.getDate() - 0.5) / daysInMonth) * 100}%`,
                            }}
                          />
                        )}
                    </div>
                    ...
                    return (
                      <div key={deptName} className="mb-4">
                        ...
                        <div className="flex items-center mb-1">
                          ...
                          <div
                            className={`flex-1 h-px ${color.border} border-t`}
                          />
                        </div>

                        {/* Filas de empleados */}
                        {entries.map((entry: any) => {
                          const bar = getGanttBar(
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex-1 relative h-6 bg-muted/20 rounded">
                                    ...
                                    (_, i) => {
                                      ...
                                      return (
                                        <div
                                          ...
                                          title={
                                            isConflict
                                              ? `⚠️ Conflicto: más del ${Math.round(CONFLICT_THRESHOLD * 100)}% del equipo ausente`
                                              : undefined
                                          }
                                        />
                                      );
                                ...
                                >
                                  <span
                                    className={`text-xs font-medium truncate ${isApproved ? color.text : "text-yellow-700 dark:text-yellow-300"}`}
                                  >
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                Vista Mensual — {MONTH_NAMES[month - 1]} {year}
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              <div className="grid grid-cols-7 gap-px bg-border rounded-lg overflow-hidden">
                {Array.from({ length: firstDayOfWeek }, (_, i) => (
                  <div
                    key={`empty-${i}`}
                    className="bg-muted/30 min-h-[80px] p-1"
                  />
                ))}
                ...
                {Array.from({ length: daysInMonth }, (_, i) => {
                  ...
                  const deptConflicts = deptNames.filter(d =>
                    conflictSet.has(`${d}|${day}`)
                  );
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <div
                          className={`text-xs font-medium w-6 h-6 flex items-center justify-center rounded-full ${isToday ? "bg-primary text-primary-foreground" : "text-muted-foreground"}`}
                        >
                          {day}
                        </div>
                        {hasConflict && (
                          <span
                            title={`Conflicto: ${deptConflicts.join(", ")}`}
                          >
                      ...
                      <div className="space-y-0.5">
                        {entries.slice(0, 3).map((entry: any) => {
                          ...
                          const color =
                            DEPT_COLORS[
                              deptIdx >= 0 ? deptIdx % DEPT_COLORS.length : 0
                            ];
                          const isApproved = entry.status === "approved";
                          return (
                            <div
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-green-600">
                    {
                      calendarData.entries.filter(e => e.status === "approved")
                        .length
                    }
                  </div>
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-yellow-600">
                    {
                      calendarData.entries.filter(e => e.status === "pending")
                        .length
                    }
                  </div>
                  <div className="text-xs text-muted-foreground">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-blue-600">
                    {calendarData.entries.reduce(
                      (sum, e) => sum + e.requestedDays,
                      0
                    )}
                  </div>
                  <div className="text-xs text-muted-foreground">
                ...
                <div className="text-center">
                  <div
                    className={`text-2xl font-bold ${hasConflicts ? "text-red-600" : "text-gray-400"}`}
                  >
                    {conflictDays.length > 0
                      ? Array.from(
                          new Set(conflictDays.map(c => `${c.dept}|${c.day}`))
                        ).length
                      : 0}
                  </div>
                  <div className="text-xs text-muted-foreground">
