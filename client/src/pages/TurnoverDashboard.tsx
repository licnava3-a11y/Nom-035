...
export default function TurnoverDashboard() {
  ...
  const { data: stats, isLoading: statsLoading } =
    trpc.employees.getTurnoverStats.useQuery(dateRange);
  const { data: trends, isLoading: trendsLoading } =
    trpc.employees.getMonthlyTrends.useQuery({ months: 12 });
  const { data: byReason, isLoading: reasonLoading } =
    trpc.employees.getTerminationsByReason.useQuery(dateRange);
  const { data: byDepartment, isLoading: deptLoading } =
    trpc.employees.getTerminationsByDepartment.useQuery(dateRange);

  const isLoading =
    statsLoading || trendsLoading || reasonLoading || deptLoading;
  ...
  const handleExportToExcel = () => {
    ...
    const trendsData = [
      ["Mes", "Bajas"],
      ...monthlyData.map((d: { month: string; bajas: number }) => [
        d.month,
        d.bajas,
      ]),
    ];
    ...
    const reasonDataExport = [
      ["Motivo", "Cantidad"],
      ...reasonData.map((r: { name: string; value: number }) => [
        r.name,
        r.value,
      ]),
    ];
    ...
    const deptDataExport = [
      ["Departamento", "Bajas"],
      ...departmentData.map((d: { name: string; bajas: number }) => [
        d.name,
        d.bajas,
      ]),
    ];
    ...
    XLSX.writeFile(
      wb,
      `dashboard-rotacion-${new Date().toISOString().split("T")[0]}.xlsx`
    );
      ...
      <div className="p-6">
        <div className="flex items-center justify-center h-64">
          <div className="text-muted-foreground">
            Cargando estadísticas de rotación...
          </div>
        </div>
      </div>
    );
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Análisis de bajas y tendencias de personal
          </p>
          {period !== "month" && (
