...
export default function ReportConfigurationPanel() {
  ...
  const [dateRangeType, setDateRangeType] = useState<
    "auto" | "custom" | "last_7_days" | "last_30_days"
  >("auto");
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.reportConfigurations.getAll.useQuery();

  // KPIs para preview — se cargan solo cuando se abre el modal
  const { data: kpiData, isLoading: kpiLoading } =
    trpc.executiveReport.getKPIs.useQuery({}, { enabled: isPreviewOpen });
  ...
  const toggleEnabledMutation =
    trpc.reportConfigurations.toggleEnabled.useMutation({
      onSuccess: data => {
  ...
  const handleCreate = () => {
    const recipientsList = recipients
      ...
      .map((r: any) => r.trim())
      .filter((r: any) => r);
    const ccRecipientsList = ccRecipients
      ? ccRecipients
          .split(",")
          .map((r: any) => r.trim())
          .filter((r: any) => r)
  ...
  const openEditDialog = (config: ReportConfig) => {
    ...
    setCcRecipients(
      config.ccRecipients ? JSON.parse(config.ccRecipients).join(", ") : ""
    );
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold">
            Configuración de Reportes Ejecutivos
          </h1>
          <p className="text-muted-foreground mt-2">
            Gestiona la frecuencia, destinatarios y opciones de reportes
            automatizados
          </p>
        </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {configs?.reduce(
                (sum: any, c: any) => sum + c.executionCount,
                0
              ) || 0}
            </div>
          </CardContent>
                    ...
                    <TableCell>
                      {config.nextExecutionAt ? (
                        <div className="flex items-center gap-2">
                          ...
                          {new Date(config.nextExecutionAt).toLocaleDateString(
                            "es-MX"
                          )}
                        </div>
                      ) : (
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Resumen de KPIs actuales que se incluirán en el próximo envío del
              reporte
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="text-center p-2 bg-muted/40 rounded">
                    <div className="text-2xl font-bold">
                      {kpiData.employees.total}
                    </div>
                    <div className="text-xs text-muted-foreground">Total</div>
                  </div>
                  <div className="text-center p-2 bg-green-50 rounded">
                    <div className="text-2xl font-bold text-green-700">
                      {kpiData.employees.active}
                    </div>
                    <div className="text-xs text-muted-foreground">Activos</div>
                  </div>
                  <div className="text-center p-2 bg-red-50 rounded">
                    <div className="text-2xl font-bold text-red-700">
                      {kpiData.employees.inactive}
                    </div>
                  ...
                  <div className="text-center p-2 bg-orange-50 rounded">
                    <div className="flex items-center justify-center gap-1">
                      <span className="text-2xl font-bold text-orange-700">
                        {kpiData.employees.turnoverRate}%
                      </span>
              ...
              <div className="border rounded-lg p-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="text-center p-2 bg-muted/40 rounded">
                    <div className="text-2xl font-bold">
                      {kpiData.training.totalCourses}
                    </div>
                    <div className="text-xs text-muted-foreground">Cursos</div>
                  </div>
                  <div className="text-center p-2 bg-muted/40 rounded">
                    <div className="text-2xl font-bold">
                      {kpiData.training.totalAssignments}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      Asignaciones
                    </div>
                  ...
                  <div className="text-center p-2 bg-green-50 rounded">
                    <div className="text-2xl font-bold text-green-700">
                      {kpiData.training.completedAssignments}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      Completadas
                    </div>
                ...
                <div className="border border-dashed rounded-lg p-4 bg-muted/20">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <span className="font-medium">
                        {JSON.parse(previewConfig.recipients).length}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="font-medium">
                        {[
                          ...
                          previewConfig.includeRecommendations &&
                            "Recomendaciones",
                        ]
                          .filter(Boolean)
                          .join(", ") || "Solo datos"}
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              <Label htmlFor="ccRecipients">
                CC (opcional, separados por comas)
              </Label>
              <Input
                id="ccRecipients"
                value={ccRecipients}
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Switch
                  checked={includeTrends}
                  onCheckedChange={setIncludeTrends}
                  id="edit-includeTrends"
                />
              </div>
              <div className="flex items-center justify-between">
                <Label htmlFor="edit-includeRecommendations">
                  Incluir Recomendaciones
                </Label>
                <Switch
                  checked={includeRecommendations}
                  onCheckedChange={setIncludeRecommendations}
                  id="edit-includeRecommendations"
                />
              </div>
            </div>
          </div>
      ...
      <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ¿Estás seguro de que deseas eliminar esta configuración de
              reporte? Esta acción no se puede deshacer.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
            ...
            <LoadingButton
              ...
              onClick={() =>
                selectedConfig &&
                deleteMutation.mutate({ id: selectedConfig.id })
              }
