...
async function exportToExcel(data: any) {
  ...
  const resumenRows: any[][] = [
    ...
    [
      ...
      (data.categorias ?? []).filter(
        (c: any) =>
          c.nivel?.labelClass === "alto" || c.nivel?.labelClass === "muy_alto"
      ).length,
    ],
    [
      "Dominios",
      (data.dominios ?? []).length,
      (data.dominios ?? []).filter(
        (d: any) =>
          d.nivel?.labelClass === "alto" || d.nivel?.labelClass === "muy_alto"
      ).length,
    ],
    [
      "Dimensiones",
  ...
  // Hoja 2: Categorías
  const catRows: any[][] = [["CATEGORÍA", "PUNTAJE (%)", "NIVEL DE RIESGO"]];
  ...
  // Hoja 3: Dominios
  const domRows: any[][] = [["DOMINIO", "PUNTAJE (%)", "NIVEL DE RIESGO"]];
  ...
  // Hoja 4: Dimensiones
  const dimRows: any[][] = [["DIMENSIÓN", "PUNTAJE (%)", "NIVEL DE RIESGO"]];
  ...
  if (itemsAlto.length === 0) {
    planRows.push([
      "Sin áreas de riesgo alto o muy alto identificadas",
      "",
      "",
      "",
      "",
    ...
    itemsAlto.forEach((item: any) => {
      planRows.push([
        item.nombre,
        item.nivel?.nivel?.toUpperCase() ?? "",
        "Definir intervención específica",
        "Por asignar",
        "",
        "Pendiente",
      ]);
...
function NivelBadge({
  ...
  return (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold border ${NIVEL_COLORS[labelClass] ?? "bg-gray-100 text-gray-700 border-gray-300"}`}
    >
      {nivel}
    </span>
  );
  ...
  return (
    <div className="flex items-center gap-2 min-w-[140px]">
      <div className="flex-1 bg-gray-100 rounded-full h-2 overflow-hidden">
        <div
          className={`h-2 rounded-full ${NIVEL_BAR_COLORS[labelClass] ?? "bg-gray-400"}`}
...
function CustomTooltip({
  ...
  active?: boolean;
  payload?: Array<{
    payload: { nombre: string; puntaje: number; nivel: { nivel: string } };
  }>;
}) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-lg p-3 text-sm max-w-[220px]">
      <p className="font-semibold text-slate-800 mb-1 leading-tight">
        {d.nombre}
      </p>
      <p className="text-slate-600">
        Puntaje: <strong className="text-slate-900">{d.puntaje}%</strong>
      </p>
...
function DimensionesBarChart({
  items,
}: {
  items: Array<{
    nombre: string;
    puntaje: number;
    nivel: { nivel: string; labelClass: string };
  }>;
}) {
...
export default function NOM035DetailedReport() {
  ...
  const { data, isLoading } = trpc.nom035Admin.getDetailedResults.useQuery(
    queryParams,
    { retry: false }
  );
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Cálculo por <strong>Categoría, Dominio y Dimensión</strong>{" "}
              conforme a la NOM-035-STPS-2018. Fórmula: (PuntajeDirecto /
              MáximoPosible) × 100.
            </p>
          </div>
          <Badge
            variant="outline"
            className="text-xs border-blue-300 text-blue-700 bg-blue-50 whitespace-nowrap"
          >
            NOM-035-STPS-2018
          </Badge>
        </div>
        ...
        <Card className="border-slate-200">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Selecciona qué secciones incluir en el dictamen
            </CardDescription>
          </CardHeader>
              ...
              {[
                {
                  ...
                  value: showCategoria,
                  set: setShowCategoria,
                },
                {
                  label: "Resultados por Dominio",
                  value: showDominio,
                  set: setShowDominio,
                },
                {
                  label: "Resultados por Dimensión",
                  value: showDimension,
                  set: setShowDimension,
                },
                {
                  label: "Recomendaciones (Guía STPS)",
                  value: showRecomendaciones,
                  set: setShowRecomendaciones,
                },
                {
                  label: "Plan de trabajo / Intervención",
                  value: showPlanTrabajo,
                  set: setShowPlanTrabajo,
                },
              ].map(({ label, value, set }) => (
                <label
                  key={label}
                  className="flex items-center gap-2 cursor-pointer bg-slate-50 border border-slate-200 rounded-full px-4 py-2 hover:bg-blue-50 transition-colors"
                >
                  ...
                  onClick={async () => {
                    try {
                      ...
                      const catRows = [
                        new TableRow({
                          children: [
                            makeCell("Dominio", true),
                            makeCell("Puntaje", true),
                            makeCell("Nivel", true),
                          ],
                        }),
                        ...(data.dominios ?? []).map(
                        ...
                        sections: [
                          {
                            children: [
                              new Paragraph({
                                text: "Dictamen NOM-035 \u2014 An\u00e1lisis Extendido",
                              ...
                              new Paragraph({
                                children: [
                                  new TextRun({
                                    text: `Total respuestas: ${data.totalRespuestas}`,
                                  }),
                                ],
                              }),
                              new Paragraph({ text: "" }),
        ...
        {!isLoading && data && data.totalRespuestas === 0 && (
          <Card className="border-dashed border-slate-300">
            <CardContent className="py-12 text-center text-slate-400">
              ...
              <p>
                No hay respuestas completadas. Aplica la encuesta NOM-035
                primero.
              </p>
            </CardContent>
          </Card>
        ...
        {!isLoading && data && data.totalRespuestas > 0 && (
          <div className="space-y-5">
                ...
                <Card>
                  <CardContent className="pt-5">
                    ...
                    <p className="text-3xl font-bold text-slate-900 mt-1">
                      {data.categoriaGeneral.puntaje}%
                    </p>
                    <NivelBadge
                      nivel={data.categoriaGeneral.nivel.nivel}
                      labelClass={data.categoriaGeneral.nivel.labelClass}
                    />
                  </CardContent>
                </Card>
              )}
              <Card
                className={`border-2 ${data.requierePlanObligatorio ? "border-orange-300 bg-orange-50" : "border-green-300 bg-green-50"}`}
              >
                <CardContent className="pt-5">
                  <p
                    className={`text-xs font-semibold uppercase ${data.requierePlanObligatorio ? "text-orange-600" : "text-green-600"}`}
                  >
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span
                      className={`text-sm font-semibold ${data.requierePlanObligatorio ? "text-orange-800" : "text-green-800"}`}
                    >
                      {data.requierePlanObligatorio
                        ? "Obligatorio (NOM-035 §8.4)"
                        : "No requerido"}
                    </span>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader className="pb-3">
                  ...
                  <CardDescription className="text-xs">
                    Barras coloreadas por nivel de riesgo NOM-035. Las líneas
                    verticales marcan los umbrales normativos.
                  </CardDescription>
                  <NivelLeyenda />
                </CardHeader>
                <CardContent>
                          ...
                          {data.dimensiones.map(dim => (
                            ...
                            >
                              ...
                              <td className="py-2.5 px-3 text-slate-500 text-xs">
                                {dim.domain || dim.category || "—"}
                              </td>
                              <td className="py-2.5 px-3">
                                <ScoreBar
                                  puntaje={dim.puntaje}
                                  labelClass={dim.nivel.labelClass}
                                />
                              </td>
                              <td className="py-2.5 px-3">
                                <NivelBadge
                                  nivel={dim.nivel.nivel}
                                  labelClass={dim.nivel.labelClass}
                                />
                              </td>
                        ...
                        {data.dominios.map(dom => (
                          ...
                          >
                            ...
                            <td className="py-2.5 px-3">
                              <ScoreBar
                                puntaje={dom.puntaje}
                                labelClass={dom.nivel.labelClass}
                              />
                            </td>
                            <td className="py-2.5 px-3">
                              <NivelBadge
                                nivel={dom.nivel.nivel}
                                labelClass={dom.nivel.labelClass}
                              />
                            </td>
                            <td className="py-2.5 px-3 text-right text-slate-400 text-xs">
                              {dom.totalDimensiones}
                            </td>
                          </tr>
                        ))}
                      </tbody>
            ...
            {data.recomendaciones.length > 0 &&
              queryParams.showRecomendaciones && (
            ...
            {data.planTrabajo && queryParams.showPlanTrabajo && (
              <Card className="border-orange-200 bg-orange-50">
                ...
                <CardContent className="space-y-3">
                  {[
                    {
                      label: "Nivel 1 — Acción inmediata",
                      value: data.planTrabajo.nivel1,
                    },
                    {
                      label: "Nivel 2 — Intervención organizacional",
                      value: data.planTrabajo.nivel2,
                    },
                    {
                      label: "Nivel 3 — Seguimiento",
                      value: data.planTrabajo.nivel3,
                    },
                    {
                      label: "Comité de seguridad",
                      value: data.planTrabajo.comite,
                    },
                  ].map(({ label, value }) => (
                    <div
                      key={label}
              ...
              <div className="flex items-start gap-3 bg-amber-50 border border-amber-300 rounded-lg p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-700 mt-0.5">
                    Se detectaron niveles <strong>Alto</strong> o{" "}
                    <strong>Muy Alto</strong>. La NOM-035 obliga a un plan de
                    intervención (numeral 8.4).
                  </p>
                </div>
              </div>
            )}
            ...
            <p className="text-xs text-center text-slate-400 pt-2">
              NOM-035-STPS-2018 — Niveles: Nulo (0–5%), Bajo (6–40%), Medio
              (41–60%), Alto (61–85%), Muy Alto (86–100%)
            </p>
          </div>
        )}
      </div>
