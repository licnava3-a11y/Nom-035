...
export default function TrendsCharts() {
  ...
  const { data: surveyCoverageTrends, isLoading: loadingSurveys } = trpc.trends.getSurveyCoverageTrends.useQuery({
    period,
    startDate,
    endDate,
  });
  ...
  const renderTrendBadge = (trend: "up" | "down" | "stable", change: number) => {
    if (trend === "up") {
      return (
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Gráficas de Tendencias Temporales</h1>
          <p className="text-muted-foreground">
            Visualización de evolución de casos NOM-035, cobertura de encuestas y cumplimiento normativo
          </p>
        </div>
      </div>
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Casos abiertos, en investigación y cerrados por período</CardDescription>
                </div>
                {casesTrends?.comparison && (
                  <div className="flex gap-2">
                    <div className="text-right">
                      <p className="text-sm text-muted-foreground">Período Actual</p>
                      <p className="text-2xl font-bold">{casesTrends.comparison.currentTotal}</p>
                    </div>
                    {renderTrendBadge(casesTrends.comparison.trend as "up" | "down" | "stable", casesTrends.comparison.percentageChange)}
                  </div>
                )}
              </div>
            </CardHeader>
              ...
              {loadingCases ? (
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <LineChart data={casesTrends.current}>
                    ...
                    <Line type="monotone" dataKey="enInvestigacion" stroke="#f59e0b" strokeWidth={2} name="En Investigación" />
        ...
        <TabsContent value="surveys" className="space-y-4">
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Encuestas completadas por guía y período</CardDescription>
                </div>
                {surveyCoverageTrends?.comparison && (
                  <div className="flex gap-2">
                    <div className="text-right">
                      <p className="text-sm text-muted-foreground">Período Actual</p>
                      <p className="text-2xl font-bold">{surveyCoverageTrends.comparison.currentTotal}</p>
                    </div>
                    {renderTrendBadge(surveyCoverageTrends.comparison.trend as "up" | "down" | "stable", surveyCoverageTrends.comparison.percentageChange)}
                  </div>
                )}
              </div>
            </CardHeader>
            <CardContent>
              ...
              ) : surveyCoverageTrends?.current && surveyCoverageTrends.current.length > 0 ? (
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={surveyCoverageTrends.current}>
                    ...
                    <Tooltip />
                    <Legend />
                    <Bar dataKey="completadas" fill="#8884d8" name="Total Completadas" />
        ...
        <TabsContent value="compliance" className="space-y-4">
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>Porcentaje de cumplimiento basado en encuestas completadas</CardDescription>
                </div>
                {complianceTrends?.comparison && (
                  <div className="flex gap-2">
                    <div className="text-right">
                      <p className="text-sm text-muted-foreground">Período Actual</p>
                      <p className="text-2xl font-bold">{complianceTrends.comparison.currentPercentage}%</p>
                    </div>
                    {renderTrendBadge(complianceTrends.comparison.trend as "up" | "down" | "stable", complianceTrends.comparison.percentageChange)}
                  </div>
                )}
              </div>
            </CardHeader>
            <CardContent>
              ...
              ) : complianceTrends?.current && complianceTrends.current.length > 0 ? (
                <ResponsiveContainer width="100%" height={400}>
              ...
              {complianceTrends?.totalEmployees && (
                <div className="mt-4 p-4 bg-muted rounded-lg">
                  <p className="text-sm text-muted-foreground">
                    Total de empleados activos: <span className="font-bold">{complianceTrends.totalEmployees}</span>
                  </p>
                </div>
              )}
            </CardContent>
