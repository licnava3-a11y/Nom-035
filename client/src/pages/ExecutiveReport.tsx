...
function RiskComparisonChart({ data }: { data: any }) {
  ...
  useEffect(() => {
    ...
    const prevData = data.comparison.map((r: any) =>
      r.previous.avgScore > 0 ? parseFloat(r.previous.avgScore.toFixed(1)) : 0
    );
    const currData = data.comparison.map((r: any) =>
      r.current.avgScore > 0 ? parseFloat(r.current.avgScore.toFixed(1)) : 0
    );
    chartRef.current = new Chart(canvasRef.current, {
      type: "bar",
      data: {
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              label: (ctx: any) =>
                `${ctx.dataset.label}: ${ctx.parsed.y.toFixed(1)}/140`,
            },
          },
        },
        scales: {
...
export default function ExecutiveReport() {
  ...
  const { data: trends, isLoading: trendsLoading } =
    trpc.executiveReport.getTrends.useQuery({ months: trendMonths });
  const { data: deptRisk = [] } =
    trpc.psychometric.getRiskByDepartment.useQuery(
  ...
  const exportRiskComparisonToExcel = async () => {
    ...
    try {
      ...
      const trendLabel = (t: string) =>
        t === "up" ? "Sube" : t === "down" ? "Baja" : "Estable";
      const rows = riskComparison.comparison.map((row: any) => ({
        Departamento: row.departmentName,
        [`Puntaje ${riskComparison.previousMonthLabel}`]:
          row.previous.avgScore > 0
            ? row.previous.avgScore.toFixed(1)
            : "Sin datos",
        [`Puntaje ${riskComparison.currentMonthLabel}`]:
          row.current.avgScore > 0
            ? row.current.avgScore.toFixed(1)
            : "Sin datos",
        "Δ Puntaje":
          row.deltaScore !== 0
            ? row.deltaScore > 0
              ? `+${row.deltaScore.toFixed(1)}`
              : row.deltaScore.toFixed(1)
            : "0",
  ...
  const { data: companiesList = [] } =
    trpc.superAdmin.listCompaniesSimple.useQuery();
  ...
  async function exportToExcel() {
    ...
    if (kpis) {
      const kpiRows = [
        ...
        [
          ...
          kpis.cases.highRisk,
          "Casos con prioridad alta o crítica",
        ],
        [
          "Mensajes Buzón Interno",
          kpis.mailbox.total,
        ...
        [
          ...
          kpis.psychometric.total,
          "Total de evaluaciones realizadas",
        ],
        [
          "Alto Riesgo Psicométrico",
          kpis.psychometric.highRisk,
          "Evaluaciones con riesgo alto o muy alto",
        ],
      ];
    ...
    XLSX.writeFile(
      wb,
      `Reporte_Ejecutivo_NOM035_${new Date().toISOString().slice(0, 10)}.xlsx`
    );
  }
  ...
  const exportToPDFWithCharts = async () => {
    try {
      ...
      doc.setFontSize(9);
      doc.text("Factores de Riesgo Psicosocial en el Trabajo", pageW / 2, 110, {
        align: "center",
      });
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        companyInfo?.company_name || "[Nombre de la Empresa]",
        65,
        coverY
      );
      ...
      doc.setFont("helvetica", "normal");
      doc.text(
        companyInfo?.company_rfc || "[RFC de la Empresa]",
        65,
        coverY + 10
      );
      ...
      doc.text(
        "Documento generado automáticamente por la Plataforma de Cumplimiento NOM-035 STPS 2018",
        pageW / 2,
        pageH - 10,
        { align: "center" }
      );
      ...
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.text("Reporte Ejecutivo Consolidado NOM-035 STPS", pageW / 2, 10, {
        align: "center",
      });
      doc.setFontSize(9);
      doc.setFont("helvetica", "normal");
      ...
      if (trendsContainerRef.current) {
        ...
        y += 4;
        const canvas = await html2canvas(trendsContainerRef.current, {
          scale: 1.5,
          useCORS: true,
      ...
      doc.save(
        `reporte-ejecutivo-nom035-${new Date().toISOString().slice(0, 10)}.pdf`
      );
  ...
  const exportToWord = async () => {
    try {
      ...
      const kpiTableRows = kpis
        ? [
            new TableRow({
              children: [makeCell("KPI", true), makeCell("Valor", true)],
            }),
            new TableRow({
              children: [
                makeCell("Total Empleados"),
                makeCell(String(kpis.employees.total)),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Empleados Activos"),
                makeCell(String(kpis.employees.active)),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Tasa de Rotaci\u00f3n"),
                makeCell(`${kpis.employees.turnoverRate}%`),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Cursos Totales"),
                makeCell(String(kpis.training.totalCourses)),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Tasa de Completaci\u00f3n"),
                makeCell(`${kpis.training.completionRate}%`),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Vacaciones Pendientes"),
                makeCell(String(kpis.vacations.pending)),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Casos NOM-035 Abiertos"),
                makeCell(String(kpis.cases.open)),
              ],
            }),
            new TableRow({
              children: [
                makeCell("Casos Alto Riesgo"),
                makeCell(String(kpis.cases.highRisk)),
              ],
            }),
          ]
        : [];
        ...
        sections: [
          {
            children: [
              new Paragraph({
                text: "Reporte Ejecutivo Consolidado NOM-035 STPS",
        ...
        {kpis && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
              {[
                ...
                },
                {
                  ...
                  total: kpis.vacations.total,
                  icon: <Calendar className="h-5 w-5 text-orange-600" />,
                  color: "text-orange-700",
                  bg: "bg-orange-50",
                },
                {
                  label: "Casos Abiertos NOM-035",
                  value: kpis.cases.open,
                  total: kpis.cases.total,
                    ...
                    <TrendChart
                      ...
                      datasets={[
                        ...
                        },
                        {
                          ...
                          data: trends.trainingCompletions,
                          color: "#16a34a",
                        },
                      ]}
                    />
                      ...
                      datasets={[
                        {
                          ...
                          data: trends.employeeExits,
                          color: "#1d4ed8",
                        },
                        {
                          label: "Evaluaciones psicométricas",
                          data: trends.psychometricAssessments,
                          color: "#7c3aed",
                        },
                      ]}
                    />
                    ...
                    <Card>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-semibold flex items-center gap-2">
                          ...
                          Resumen del Período ({trendMonths} meses)
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        {[
                          {
                            label: "Total casos NOM-035",
                            value: trends.cases.reduce((a, b) => a + b, 0),
                            color: "text-red-600",
                          },
                          {
                            label: "Capacitaciones completadas",
                            value: trends.trainingCompletions.reduce(
                              (a, b) => a + b,
                              0
                            ),
                          ...
                          {
                            label: "Salidas de empleados",
                            value: trends.employeeExits.reduce(
                              (a, b) => a + b,
                              0
                            ),
                            color: "text-blue-600",
                          ...
                          {
                            label: "Evaluaciones psicométricas",
                            value: trends.psychometricAssessments.reduce(
                              (a, b) => a + b,
                              0
                            ),
                          ...
                          {
                            label: "Mensajes en buzón",
                            value: trends.mailboxMessages.reduce(
                              (a, b) => a + b,
                              0
                            ),
                            color: "text-teal-600",
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  {[
                    ...
                    {
                      label: "Empleados activos",
                      value: `${kpis.employees.active} (${kpis.employees.total > 0 ? Math.round((kpis.employees.active / kpis.employees.total) * 100) : 0}%)`,
                    },
                    {
                      label: "Empleados inactivos",
                      value: `${kpis.employees.inactive} (${kpis.employees.total > 0 ? Math.round((kpis.employees.inactive / kpis.employees.total) * 100) : 0}%)`,
                    },
                    {
                      label: "Tasa de rotación",
                      value: `${kpis.employees.turnoverRate}%`,
                    },
                  ].map(row => (
                    <div
                      key={row.label}
                  ...
                  {kpis.employees.total > 0 && (
                    <div className="mt-2">
                      <div className="flex justify-between text-xs text-muted-foreground mb-1">
                        ...
                        <span>
                          {Math.round(
                            (kpis.employees.active / kpis.employees.total) * 100
                          )}
                          %
                        </span>
                      </div>
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(kpis.employees.active / kpis.employees.total) * 100}%`,
                          }}
                        />
                      </div>
                    </div>
                ...
                <CardContent className="space-y-3">
                  {[
                    ...
                    },
                    {
                      ...
                      value: kpis.training.completedAssignments,
                    },
                    {
                  ...
                  {kpis.training.totalAssignments > 0 && (
                    <div className="mt-2">
                      <div className="flex justify-between text-xs text-muted-foreground mb-1">
                        ...
                        <span>{kpis.training.completionRate}%</span>
                      </div>
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          className="h-2 bg-purple-500 rounded-full"
                          style={{ width: `${kpis.training.completionRate}%` }}
                        />
                      </div>
                    </div>
                  )}
              ...
              <Card>
                <CardHeader className="pb-3">
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    Casos NOM-035 y Riesgo Psicosocial
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  {[
                    ...
                    },
                    {
                      ...
                      value: kpis.cases.open,
                      alert: kpis.cases.open > 0,
                    },
                    {
                      label: "Casos de alto riesgo",
                      value: kpis.cases.highRisk,
                      alert: kpis.cases.highRisk > 0,
                    },
                    {
                      label: "Evaluaciones psicométricas",
                      value: kpis.psychometric.total,
                      alert: false,
                    },
                    {
                      label: "Riesgo alto/muy alto",
                      value: kpis.psychometric.highRisk,
                      alert: kpis.psychometric.highRisk > 0,
                    },
                  ].map(row => (
                    <div
                      key={row.label}
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-semibold ${row.alert ? "text-red-600" : ""}`}
                      >
                        {row.value}
                      </span>
                    </div>
                  ...
                  {[
                    {
                      ...
                      value: kpis.mailbox.total,
                      alert: false,
                    },
                    {
                      label: "Mensajes pendientes",
                      value: kpis.mailbox.pending,
                      alert: kpis.mailbox.pending > 0,
                    },
                    {
                      label: "Solicitudes de vacaciones",
                      value: kpis.vacations.total,
                      alert: false,
                    },
                    {
                      label: "Vacaciones pendientes",
                      value: kpis.vacations.pending,
                      alert: kpis.vacations.pending > 0,
                    },
                    {
                      label: "Vacaciones aprobadas",
                      value: kpis.vacations.approved,
                      alert: false,
                    },
                  ].map(row => (
                    <div
                    ...
                    >
                      ...
                      <span
                        className={`text-sm font-semibold ${row.alert ? "text-orange-600" : ""}`}
                      >
                        {row.value}
                      </span>
                    </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Basado en la última evaluación NOM-035 Guía III de cada
                      empleado activo
                    </p>
                  </div>
              ...
              <CardContent>
                {(deptRisk as any[]).length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-4">
                    Sin datos de evaluaciones psicométricas aún
                  </p>
                ) : (
                    ...
                    {(deptRisk as any[]).map((dept: any) => {
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center gap-3 p-2 rounded-lg ${bgColor}`}
                        >
                          <div
                            className="w-36 text-xs font-medium truncate"
                            title={dept.departmentName}
                          >
                            {dept.departmentName || "Sin departamento"}
                          </div>
                          <div className="flex-1 bg-gray-200 rounded-full h-3 overflow-hidden">
                            <div
                              className={`h-3 rounded-full transition-all ${riskColor}`}
                              style={{
                                width: `${Math.min((avg / 140) * 100, 100)}%`,
                              }}
                            />
                          </div>
              ...
              <Card>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        ...
                        <span className="font-semibold">
                          {riskComparison.currentMonthLabel}
                        </span>{" "}
                        vs.{" "}
                        <span className="font-semibold">
                          {riskComparison.previousMonthLabel}
                        </span>
                      </p>
                    </div>
                    <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground print:hidden">
                      <span className="flex items-center gap-1">
                        <ArrowUp className="h-3 w-3 text-red-500" /> Riesgo
                        aumentó
                      </span>
                      <span className="flex items-center gap-1">
                        <ArrowDown className="h-3 w-3 text-green-500" /> Riesgo
                        bajó
                      </span>
                      <span className="flex items-center gap-1">
                        <Minus className="h-3 w-3 text-gray-400" /> Sin cambio
                      </span>
                      <div className="flex items-center gap-1 ml-1 border-l pl-2">
                        ...
                        <select
                          ...
                          onChange={e =>
                            setCompareMonthsAgo(Number(e.target.value))
                          }
                ...
                <CardContent>
                          ...
                          return (
                            ...
                            >
                              <td className="py-2 pr-4">
                                ...
                                <span className="text-xs text-muted-foreground">
                                  {row.current.totalAssessed} eval. actuales
                                </span>
                              </td>
                              <td className="text-right py-2 px-3">
                                <span className="text-muted-foreground">
                                  {prevAvg > 0 ? prevAvg.toFixed(1) : "—"}
                                </span>
                                {prevAvg > 0 && (
                                  <span className="text-xs text-muted-foreground">
                                    /140
                              ...
                              <td className="text-right py-2 px-3">
                                <span
                                  className={`font-semibold ${deltaHighRiskColor}`}
                                >
                  ...
                  <p className="text-xs text-muted-foreground mt-3 border-t pt-2">
                    Δ Puntaje: diferencia de puntaje promedio NOM-035 (escala
                    0–140). Valores positivos indican mayor riesgo psicosocial.
                    Umbral de tendencia: ±2 puntos.
                  </p>
                  {/* Grouped bar chart */}
                  <div className="mt-4 border-t pt-4">
                    <p className="text-xs font-semibold text-muted-foreground mb-3">
                      Gráfica comparativa por departamento
                    </p>
            ...
            <div className="hidden print:block border-t pt-4 mt-6 text-xs text-gray-500">
              <p>
                Plataforma de Capacitación NOM-035 STPS 2018 — Reporte generado
                el {new Date().toLocaleString("es-MX")}
              </p>
              <p className="mt-1">
                Documento confidencial — uso exclusivo para auditoría interna y
                cumplimiento STPS.
              </p>
            </div>
          </>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b">
              <div>
                <h2 className="font-semibold text-slate-800 text-base">
                  Vista Previa del Reporte Ejecutivo
                </h2>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Revisa la portada y los datos antes de descargar
                </p>
              </div>
              <div className="flex items-center gap-2">
                <Button
                ...
                <button
                  onClick={() => setPdfPreviewUrl(null)}
                  className="ml-2 text-slate-400 hover:text-slate-700 text-xl font-bold leading-none"
                  aria-label="Cerrar"
                >
                  &times;
                </button>
