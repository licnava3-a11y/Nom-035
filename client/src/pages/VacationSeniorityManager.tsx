...
export default function VacationSeniorityManager() {
  ...
  const {
    ...
    refetch,
    isLoading,
  } = trpc.vacations.getSeniorityTable.useQuery();
  const updateMut = trpc.vacations.updateSeniorityTable.useMutation({
    onSuccess: () => {
      toast({
        title: "Tabla actualizada",
        description: "La tabla de antigüedad se guardó correctamente.",
      });
      refetch();
    },
    onError: e =>
  ...
  const handleAddRow = () => {
    ...
    setRows([
      ...effectiveRows,
      { yearsMin: newMin, yearsMax: null, vacationDays: 30 },
    ]);
    setIsDirty(true);
  };
  ...
  const handleRestoreLFT = () => {
    ...
    setIsDirty(true);
    toast({
      title: "Tabla LFT restaurada",
      description:
        "Se cargó la tabla predeterminada de la Ley Federal del Trabajo.",
    });
  };

  const handleSave = () => {
    // Validar que no haya filas con días = 0
    const invalid = effectiveRows.some(
      r => r.vacationDays <= 0 || r.yearsMin < 0
    );
    if (invalid) {
      toast({
        title: "Datos inválidos",
        description:
          "Todos los días deben ser mayores a 0 y los años deben ser positivos.",
        variant: "destructive",
      });
      return;
    }
    ...
    <DashboardLayout>
      <div className="space-y-6 p-6">
        ...
        <div className="flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-3 mb-1">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Configura los días de vacaciones según los años de servicio
                  del empleado
                </p>
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              onClick={handleSave}
              disabled={!isAdmin || updateMut.isPending || !isDirty}
            >
          ...
          <div className="flex items-start gap-3 p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 rounded-lg">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300">
                Solo los administradores pueden modificar esta tabla. Contacta
                al administrador del sistema para realizar cambios.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="flex items-start gap-3 p-4 bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          ...
          <div className="text-sm">
            ...
            <p className="text-blue-700 dark:text-blue-300 mt-0.5">
              La LFT establece un mínimo de 12 días para el primer año,
              incrementando 2 días por cada año subsiguiente hasta el quinto
              año, y 2 días adicionales por cada 5 años de servicio a partir del
              sexto año. Puedes personalizar esta tabla según la política
              interna de tu empresa, siempre respetando los mínimos legales.
            </p>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="text-base">
                  Configuración de Días por Antigüedad
                </CardTitle>
                <CardDescription>
                  Define los rangos de años y los días de vacaciones
                  correspondientes
                </CardDescription>
              </div>
              {isDirty && (
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                ...
                {effectiveRows.map((row, idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-12 gap-2 items-center px-3 py-2 rounded-lg border border-border/50 hover:bg-muted/20 transition-colors"
                  >
                    ...
                    <div className="col-span-3">
                      <div className="flex items-center gap-1">
                        <Input
                          ...
                          value={row.yearsMin}
                          onChange={e =>
                            handleChange(idx, "yearsMin", e.target.value)
                          }
                          disabled={!isAdmin}
                    ...
                    <div className="col-span-3">
                      <div className="flex items-center gap-1">
                        <Input
                          ...
                          onChange={e =>
                            handleChange(idx, "yearsMax", e.target.value)
                          }
                          disabled={!isAdmin}
                          className="h-8 text-sm"
                        />
                    ...
                    <div className="col-span-4">
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          onChange={e =>
                            handleChange(idx, "vacationDays", e.target.value)
                          }
                          disabled={!isAdmin}
                          className="h-8 text-sm"
                        />
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base">
              Vista Previa — Tabla Vigente
            </CardTitle>
            <CardDescription>
              Así se verá la tabla de vacaciones para los empleados
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  {effectiveRows.map((row, idx) => (
                    ...
                    >
                      ...
                      <td className="py-2 px-3 text-muted-foreground text-xs">
                        {DEFAULT_LFT_TABLE.find(
                          d =>
                            d.yearsMin === row.yearsMin &&
                            d.vacationDays === row.vacationDays
                        ) ? (
                          <Badge
                            variant="outline"
                            className="text-green-600 border-green-400 text-xs"
                          >
                            Conforme LFT
                          </Badge>
                        ) : row.vacationDays >=
                          (DEFAULT_LFT_TABLE.find(
                            d =>
                              d.yearsMin <= row.yearsMin &&
                              (d.yearsMax === null ||
                                d.yearsMax >= row.yearsMin)
                          )?.vacationDays ?? 0) ? (
                          <Badge
                            variant="outline"
                            className="text-blue-600 border-blue-400 text-xs"
                          >
                            Por encima de LFT
                          </Badge>
                        ) : (
