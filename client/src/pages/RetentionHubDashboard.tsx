...
import {
  TrendingDown, Users, AlertTriangle, Target, DollarSign, Zap,
  BarChart2, Activity, CheckCircle2, Clock, ArrowUpRight
} from "lucide-react";
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle className="text-sm">Tendencia mensual de bajas (12 meses)</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={trends ?? []}>
                ...
                <Tooltip />
                <Legend />
                <Line type="monotone" dataKey="terminations" stroke="#ef4444" name="Bajas" />
                <Line type="monotone" dataKey="hires" stroke="#22c55e" name="Altas" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle className="text-sm">Distribución por motivo</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie data={byReason ?? []} dataKey="count" nameKey="reason" cx="50%" cy="50%" outerRadius={80} label={({ reason, percent }: any) => `${reason} ${(percent * 100).toFixed(0)}%`}>
                  {(byReason ?? []).map((_: any, i: number) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer>
      ...
      {byDept && byDept.length > 0 && (
        <Card>
          <CardHeader><CardTitle className="text-sm">Bajas por departamento</CardTitle></CardHeader>
          <CardContent>
...
function TabRiesgo() {
  ...
  const getRiskBadge = (score: number) => {
    if (score >= 0.8) return <Badge className="bg-red-100 text-red-800">Crítico</Badge>;
    if (score >= 0.6) return <Badge className="bg-orange-100 text-orange-800">Alto</Badge>;
                  ...
                  <TableRow key={emp.employeeId}>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">{(emp.riskScore * 100).toFixed(0)}%</span>
                      </div>
                    </TableCell>
                    <TableCell className="text-xs text-muted-foreground max-w-[200px] truncate">{emp.riskFactors?.join(", ")}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
...
function TabIntervenciones() {
  const { data: interventions = [] } = trpc.retentionInterventions.getInterventions.useQuery({ limit: 50 });
  const { data: highRisk = [] } = trpc.predictiveTurnoverDashboard.getHighRiskEmployees.useQuery({});
  const { data: payrollData = [] } = trpc.payrollIntegration.getAllPayrollData.useQuery();

  const activeCount = interventions.filter((i: any) => i.outcome === "pending").length;
  const retainedCount = interventions.filter((i: any) => i.outcome === "retained").length;
  const totalCost = interventions.reduce((s: number, i: any) => s + parseFloat(i.cost || "0"), 0);
  const roi = retainedCount > 0 && totalCost > 0 ? ((retainedCount * 50000 - totalCost) / totalCost * 100).toFixed(1) : "0";

  const priorityData = (highRisk as any[]).slice(0, 10).map((emp: any) => {
    const payroll = (payrollData as any[]).find((p: any) => p.employeeId === emp.employeeId);
    return {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Intervenciones activas", value: activeCount, cls: "text-blue-600" },
          { label: "Empleados retenidos", value: retainedCount, cls: "text-green-600" },
          { label: "Costo total", value: `$${totalCost.toLocaleString()}`, cls: "" },
          { label: "ROI estimado", value: `${roi}%`, cls: parseFloat(roi) > 0 ? "text-green-600" : "text-red-600" },
        ].map(({ label, value, cls }) => (
          <Card key={label}><CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">{label}</CardTitle></CardHeader><CardContent><div className={`text-2xl font-bold ${cls}`}>{value}</div></CardContent></Card>
        ))}
      </div>

      <div className="grid grid-cols-2 gap-4">
        <Card>
          <CardHeader><CardTitle className="text-sm">Priorización: riesgo vs brecha salarial</CardTitle><CardDescription>Top 10 empleados de alto riesgo</CardDescription></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={220}>
              <ScatterChart>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="riesgo" name="Riesgo" tick={{ fontSize: 11 }} label={{ value: "Riesgo (%)", position: "insideBottom", offset: -5, fontSize: 11 }} />
                <YAxis dataKey="brecha" name="Brecha salarial" tick={{ fontSize: 11 }} label={{ value: "Brecha (%)", angle: -90, position: "insideLeft", fontSize: 11 }} />
        ...
        <Card>
          <CardHeader><CardTitle className="text-sm">Historial de intervenciones</CardTitle></CardHeader>
          <CardContent>
              ...
              {interventions.slice(0, 15).map((i: any) => (
                <div key={i.id} className="flex items-center justify-between p-2 rounded border text-sm">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{i.interventionType}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Badge variant={i.outcome === "retained" ? "default" : i.outcome === "resigned" ? "destructive" : "secondary"} className="text-xs">
                      {i.outcome === "retained" ? "Retenido" : i.outcome === "resigned" ? "Renunció" : "Pendiente"}
                    </Badge>
                    <span className="text-xs text-muted-foreground">${parseFloat(i.cost || "0").toLocaleString()}</span>
                  </div>
                </div>
              ))}
              {!interventions.length && <p className="text-center text-muted-foreground py-4">Sin intervenciones registradas</p>}
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {(predictions as any[])?.length ? (
            <div className="space-y-2">
              {(predictions as any[]).slice(0, 15).map((emp: any) => (
                <div key={emp.employeeId} className="flex items-center gap-3 p-2 rounded border">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-32 bg-gray-200 rounded-full h-2">
                      <div className="h-2 rounded-full" style={{ width: `${parseFloat(emp.turnoverProbability) * 100}%`, backgroundColor: parseFloat(emp.turnoverProbability) >= 0.8 ? "#ef4444" : parseFloat(emp.turnoverProbability) >= 0.6 ? "#f97316" : "#eab308" }} />
                    </div>
                    <span className="text-sm font-medium w-12 text-right">{(parseFloat(emp.turnoverProbability) * 100).toFixed(0)}%</span>
                  </div>
                </div>
              ))}
            </div>
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Vista unificada: rotación histórica, empleados en riesgo, intervenciones y análisis predictivo
        </p>
      </div>

      <Tabs defaultValue="rotacion">
        <TabsList className="grid grid-cols-4 w-full max-w-2xl">
          <TabsTrigger value="rotacion" className="gap-1.5"><BarChart2 className="h-4 w-4" />Rotación</TabsTrigger>
          <TabsTrigger value="riesgo" className="gap-1.5"><AlertTriangle className="h-4 w-4" />En Riesgo</TabsTrigger>
          <TabsTrigger value="intervenciones" className="gap-1.5"><Target className="h-4 w-4" />Intervenciones</TabsTrigger>
          <TabsTrigger value="predictivo" className="gap-1.5"><Activity className="h-4 w-4" />Predictivo</TabsTrigger>
        </TabsList>

        <TabsContent value="rotacion"><TabRotacion /></TabsContent>
        <TabsContent value="riesgo"><TabRiesgo /></TabsContent>
        <TabsContent value="intervenciones"><TabIntervenciones /></TabsContent>
        <TabsContent value="predictivo"><TabPredictivo /></TabsContent>
      </Tabs>
    </div>
  );
}
