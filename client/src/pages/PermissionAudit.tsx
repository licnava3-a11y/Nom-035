...
export default function PermissionAudit() {
  ...
  const { data: historyData, isLoading } =
    trpc.permissionAudit.getHistory.useQuery({
      userId,
      changeType,
      startDate,
      endDate,
  ...
  const { data: trendsData } = trpc.permissionAudit.getChangesTrends.useQuery({
    months: trendMonths,
  });
  const { data: monthlyChanges } =
    trpc.permissionAudit.getMonthlyChangesCount.useQuery();
  const { data: customPermissionsCount } =
    trpc.permissionAudit.getUsersWithCustomPermissionsCount.useQuery();
  const { data: topAdmins } =
    trpc.permissionAudit.getTopAdministrators.useQuery();
  const { data: criticalChanges } =
    trpc.permissionAudit.getRecentCriticalChanges.useQuery();
  ...
  useEffect(() => {
    ...
    const labels = stats.map((stat: any) => {
      ...
      if (stat.changeType === "custom_permission_update")
        ...
        datasets: [
          {
            ...
            borderWidth: stats.map((s: any) =>
              changeType === s.changeType ? 4 : 2
            ),
            borderColor: "#ffffff",
            hoverBorderWidth: 4,
          },
  ...
  useEffect(() => {
    ...
    const months = Array.from(
      new Set(trendsData.map((t: any) => t.month))
    ).sort();
    const roleChanges = months.map((month: any) => {
      const item = trendsData.find(
        (t: any) => t.month === month && t.changeType === "role_change"
      );
    ...
    const permissionUpdates = months.map((month: any) => {
      const item = trendsData.find(
        (t: any) =>
          t.month === month && t.changeType === "custom_permission_update"
      );
      return item ? Number(item.count) : 0;
    });
    const permissionResets = months.map((month: any) => {
      const item = trendsData.find(
        (t: any) =>
          t.month === month && t.changeType === "custom_permission_reset"
      );
      return item ? Number(item.count) : 0;
    });
    ...
    <div className="container mx-auto py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">
                Cambios de Rol (Mes Actual)
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex items-center justify-between">
                <div className="text-2xl font-bold">
                  {monthlyChanges.currentCount}
                </div>
                {monthlyChanges.trend === "up" && (
                  <Badge className="bg-green-600">
                    ↑{" "}
                    {monthlyChanges.currentCount - monthlyChanges.previousCount}
                  </Badge>
                )}
                {monthlyChanges.trend === "down" && (
                  <Badge className="bg-red-600">
                    ↓{" "}
                    {monthlyChanges.previousCount - monthlyChanges.currentCount}
                  </Badge>
                )}
        ...
        {topAdmins && topAdmins.length > 0 && (
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium">
                Administrador Más Activo
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-lg font-bold truncate">
                {topAdmins[0].adminName}
              </div>
              <p className="text-xs text-muted-foreground mt-2">
              ...
              <Card key={stat.changeType}>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm font-medium">
                    ...
                    {stat.changeType === "custom_permission_update" &&
                      "Actualizaciones de Permisos"}
                    {stat.changeType === "custom_permission_reset" &&
                      "Resets de Permisos"}
                  </CardTitle>
                </CardHeader>
                <CardContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Evolución mensual de cambios de roles y permisos
              </CardDescription>
            </div>
            <div className="flex gap-2">
              ...
              <Button
                ...
                size="sm"
                onClick={() =>
                  downloadChartAsPNG(chartRef.current, "tendencias-cambios.png")
                }
              >
                <Download className="h-4 w-4 mr-2" />
                Descargar PNG
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filtrar historial por usuario, tipo de cambio o fecha
          </CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="custom_permission_update">
                    Actualización de Permisos
                  </SelectItem>
                  <SelectItem value="custom_permission_reset">
                    Reset de Permisos
                  </SelectItem>
                </SelectContent>
              </Select>
            ...
            <>
              ...
              {historyData.pagination.totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-muted-foreground">
                    Página {historyData.pagination.page} de{" "}
