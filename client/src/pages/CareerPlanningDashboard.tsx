...
export default function CareerPlanningDashboard() {
  ...
  const { data: vacancyProjections } =
    trpc.careerPlanning.getVacancyProjections.useQuery();

  const updateMilestoneMutation =
    trpc.careerPlanning.updateMilestone.useMutation();
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      {path.positions.map((pos: any, idx: number) => (
                        ...
                        >
                          <span>
                            Nivel {pos.level}: {pos.positionName}
                          </span>
                          <Badge variant="outline">
                            {pos.estimatedTimeMonths} meses
                          </Badge>
                        </div>
                      ))}
                    </div>
                  ...
                  {path.minimumEducation && (
                    <div className="text-sm">
                      <span className="font-medium">Educación mínima:</span>{" "}
                      {path.minimumEducation}
                    </div>
                  )}
                  ...
                  {path.minimumExperience && (
                    <div className="text-sm">
                      <span className="font-medium">Experiencia mínima:</span>{" "}
                      {Math.floor(path.minimumExperience / 12)} años
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              {suggestions?.map((suggestion, idx) => (
                <div key={idx} className="border rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge
                      variant={
                        suggestion.matchScore >= 70 ? "default" : "secondary"
                      }
                    >
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <div className="flex items-center gap-1">
                      ...
                      <span>
                        Duración estimada:{" "}
                        {Math.floor(suggestion.estimatedDuration / 12)} años
                      </span>
                    </div>
                    <div className="flex items-center gap-1">
            ...
            <>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Nivel actual: {employeePlan.currentLevel} → Nivel objetivo:{" "}
                    {employeePlan.targetLevel}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={
                        (employeePlan.currentLevel / employeePlan.targetLevel) *
                        100
                      }
                      className="h-3"
                    />
                  ...
                  {employeePlan.competencyGaps &&
                    employeePlan.competencyGaps.length > 0 && (
                      <div className="space-y-3">
                        <h3 className="font-semibold">
                          Brechas de Competencias
                        </h3>
                        <div className="space-y-2">
                          {employeePlan.competencyGaps.map(
                            (gap: any, idx: number) => (
                              <div
                                key={idx}
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="font-semibold text-lg mb-2">
                  No tienes un plan de carrera activo
                </h3>
              ...
              <div className="h-[400px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={vacancyProjections || []}>
                    ...
                    <Legend />
                    <Bar
                      dataKey="estimatedVacancies"
                      fill="#3b82f6"
                      name="Vacantes Estimadas"
                    />
                  </BarChart>
                </ResponsiveContainer>
              </div>
                    ...
                    >
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground">
                          Fecha estimada:{" "}
                          {new Date(
                            proj.estimatedOpeningDate
                          ).toLocaleDateString()}
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-semibold">
                          {proj.estimatedVacancies} vacantes
                        </div>
                        <Badge variant="outline">
                          {Math.round(proj.probability)}% probabilidad
                        </Badge>
                      </div>
                    </div>
                  ))}
