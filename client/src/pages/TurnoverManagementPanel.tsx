...
export default function TurnoverManagementPanel() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.turnoverManagement.getAllTurnoverRecords.useQuery({
    limit: 50,
    offset: 0,
  });
  ...
  const createMutation =
    trpc.turnoverManagement.createTurnoverRecord.useMutation({
      onSuccess: () => {
  ...
  const updateMutation =
    trpc.turnoverManagement.updateTurnoverRecord.useMutation({
      onSuccess: () => {
  ...
  const deleteMutation =
    trpc.turnoverManagement.deleteTurnoverRecord.useMutation({
      onSuccess: () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      exitReason: formData.exitReason as
        | "voluntary"
        | "involuntary"
        | "retirement",
    ...
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-3xl font-bold">
            Gestión de Rotación de Empleados
          </h1>
          <p className="text-muted-foreground">
            Registra manualmente empleados que han rotado para mejorar la
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Completa los campos para agregar un nuevo registro de rotación
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={e =>
                    setFormData({
                      ...formData,
                      riskScoreAtExit: parseInt(e.target.value) || 0,
                    })
                  }
                />
                <p className="text-xs text-muted-foreground">
                  {formData.riskScoreAtExit >= 70
                    ? "Alto riesgo (≥70)"
                    : "Bajo riesgo (<70)"}
                </p>
              </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Registros de Rotación ({recordsData?.total || 0})
          </CardTitle>
          <CardDescription>
            Historial de empleados que han rotado
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
                ...
                <TableBody>
                  {recordsData?.records.map((record: any) => (
                    <TableRow key={record.id}>
                      ...
                      <TableCell>
                        {new Date(record.exitDate).toLocaleDateString("es-MX")}
                      </TableCell>
                      <TableCell>
                        {getExitReasonLabel(record.exitReason)}
                      </TableCell>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Modifica los datos del registro de rotación
            </DialogDescription>
          </DialogHeader>
          {editingRecord && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  value={editingRecord.exitReason}
                  onValueChange={value =>
                    setEditingRecord({ ...editingRecord, exitReason: value })
                  }
                >
