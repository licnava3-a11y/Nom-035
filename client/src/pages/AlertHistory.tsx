...
function TrendChart() {
  ...
  const { data: trendData, isLoading } =
    trpc.alerts.getMonthlyByPriority.useQuery({ months });
    ...
    return (
      <div className="h-64 flex flex-col items-center justify-center text-muted-foreground">
        ...
        <p className="text-xs mt-1">
          Los datos aparecerán conforme se registren alertas en el sistema
        </p>
      </div>
    );
  }
...
export default function AlertHistory() {
  ...
  const handleAlertTypeChange = (v: AlertType) => {
    setAlertType(v);
    setCurrentPage(1);
  };
  const handleStatusChange = (v: AlertStatus) => {
    setStatus(v);
    setCurrentPage(1);
  };
  const handlePriorityChange = (v: AlertPriority) => {
    setPriority(v);
    setCurrentPage(1);
  };
  ...
  const handleExportToWord = async () => {
    ...
    try {
        ...
        (alert: any) =>
          new TableRow({
            children: [
              ...
              new TableCell({
                children: [new Paragraph(getAlertTypeLabel(alert.alertType))],
              }),
              new TableCell({
                children: [
                  new Paragraph(
              ...
              new TableCell({
                children: [new Paragraph(alert.description ?? "")],
              }),
              new TableCell({
                children: [
                  new Paragraph(
        ...
        sections: [
          {
            children: [
              new Paragraph({
                text: "Histórico de Alertas — NOM-035 STPS",
      ...
      saveAs(
        blob,
        `historico_alertas_${new Date().toISOString().split("T")[0]}.docx`
      );
      toast({
        title: "Word exportado",
        description: "El archivo .docx fue descargado.",
      });
    } catch {
      toast({
        title: "Error",
  ...
  const handleExportToPDF = () => {
    ...
    printWindow.document
      .write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Histórico de Alertas</title>
  ...
  const handleExportToExcel = async () => {
    ...
    const metadata = [
      ...
      [
        "  Tipo de Alerta:",
        alertType === "all" ? "Todas" : getAlertTypeLabel(alertType),
      ],
      [
        "  Estado:",
        status === "all"
      ...
      [
        "  Alertas Activas (página):",
        alerts.filter((a: any) => a.status === "active").length,
      ],
      [
        "  Alertas Resueltas (página):",
    ...
    const data = alerts.map((alert: any) => ({
      ...
      "Fecha Resolución": alert.resolvedAt
        ? new Date(alert.resolvedAt).toLocaleString("es-MX")
        : "N/A",
  ...
  const handleExportAllToExcel = async () => {
    ...
    try {
      const allAlerts =
        await utils.client.systemSettings.getAllAlertsForExport.query({
      ...
      const metadata = [
        ...
        [
          "  Tipo:",
          alertType === "all" ? "Todos" : getAlertTypeLabel(alertType),
        ],
      ...
      XLSX.writeFile(
        wb,
        `historico_completo_alertas_${new Date().toISOString().split("T")[0]}.xlsx`
      );
      toast({
        title: "Exportación completa",
        description: `${allAlerts.length} alertas exportadas a Excel.`,
      });
    } catch (err: any) {
      toast({
        title: "Error al exportar",
        description:
          err.message || "No se pudo exportar el historial completo.",
        variant: "destructive",
      });
    } finally {
    ...
    <div className="container py-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground mt-2">
          Registro completo de alertas del sistema para auditoría de
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Filtra el histórico por tipo de alerta, estado, prioridad y rango de
            fechas
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="flex-1">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="excellent_compliance">
            ...
            <div className="flex-1">
              ...
              <Select
                value={priority}
                onValueChange={v => handlePriorityChange(v as AlertPriority)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
      ...
      {total > 0 && (
        <div className="grid grid-cols-3 gap-3">
          <button
            onClick={() =>
              handlePriorityChange(priority === "critical" ? "all" : "critical")
            }
          ...
          <button
            onClick={() =>
              handlePriorityChange(priority === "warning" ? "all" : "warning")
            }
            className={`flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${
              priority === "warning"
          ...
          <button
            onClick={() =>
              handlePriorityChange(priority === "info" ? "all" : "info")
            }
            className={`flex items-center gap-3 p-3 rounded-lg border transition-all text-left ${
              priority === "info"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Evolución de alertas críticas, advertencias e informativas en los
            últimos 12 meses
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {total} alerta{total !== 1 ? "s" : ""} encontrada
                {total !== 1 ? "s" : ""}
                {priority !== "all" &&
                  ` — filtro: ${priority === "critical" ? "Crítica" : priority === "warning" ? "Advertencia" : "Informativa"}`}
      ...
      <Dialog open={resolveDialogOpen} onOpenChange={setResolveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Agrega notas sobre las acciones tomadas para resolver esta alerta
              (opcional)
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
