...
const PRIORITY_BADGE: Record<string, React.ReactElement> = {
  critical: (
    <Badge className="bg-red-100 text-red-800 border-red-300">Crítico</Badge>
  ),
  high: (
  ...
  low: (
    <Badge className="bg-blue-100 text-blue-800 border-blue-300">Bajo</Badge>
  ),
};
...
function TabAlertas() {
  ...
  const silenceAlert = trpc.alertsDashboard.silenceAlert.useMutation({
    onSuccess: () => {
      toast.success("Alerta silenciada 24h");
      refetch();
    },
    onError: e => toast.error(e.message),
  });
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <Card className="border-orange-200 bg-orange-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-orange-700">Altas</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="border-yellow-200 bg-yellow-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-yellow-700">Medias</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Alertas ({data?.total ?? 0})</CardTitle>
        </CardHeader>
        <CardContent>
          {data?.alerts?.length ? (
            <Table>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-1">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            silenceAlert.mutate({ alertId: a.id, duration: 24 })
                          }
        ...
        {[
          ...
          },
          {
            ...
            value: stats?.totalAlerts
              ? `${Math.round(((stats?.resolvedAlerts ?? 0) / stats.totalAlerts) * 100)}%`
              : "0%",
            cls: "text-blue-600",
          },
        ].map(({ label, value, cls }) => (
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie
                  data={[
                    { priority: "Críticos", count: stats?.criticalCases ?? 0 },
                    {
                      priority: "Baja cobertura",
                      count: stats?.lowCoverage ?? 0,
                    },
                    {
                      priority: "Excelente",
                      count: stats?.excellentCompliance ?? 0,
                    },
                  ]}
                  dataKey="count"
                  nameKey="priority"
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart
                data={
                  resolution?.byType
                    ? Object.entries(
                        resolution.byType as Record<string, number>
                      ).map(([type, avgHours]) => ({ type, avgHours }))
                    : []
                }
              >
                <CartesianGrid strokeDasharray="3 3" />
...
function TabIA() {
  ...
  const [filterType, setFilterType] = useState<
    "all" | "anomaly" | "pattern" | "prediction"
  >("all");
  const [filterSeverity, setFilterSeverity] = useState<
    "all" | "low" | "medium" | "high" | "critical"
  >("all");
  const { data: dashboard } = trpc.intelligentAlerts.getDashboard.useQuery();
  const { data: alerts, refetch } = trpc.intelligentAlerts.list.useQuery({
    alertType: filterType === "all" ? undefined : (filterType as any),
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          Detección proactiva de patrones de riesgo emergentes mediante IA
        </p>
        <Button
        ...
        >
          ...
          {runAnalysis.isPending ? "Analizando..." : "Ejecutar análisis IA"}
        </Button>
      </div>
      <div className="grid grid-cols-4 gap-4">
        {[
                ...
                <div className="flex gap-1 shrink-0">
                  <Button
                    ...
                    onClick={() =>
                      resolveAlert.mutate({
                        id: alert.id,
                        resolutionNotes: "Resuelta desde dashboard",
                      })
                    }
                    disabled={resolveAlert.isPending}
                  >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      dismissAlert.mutate({
                        id: alert.id,
                        reason: "Descartada desde dashboard",
                      })
                    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Configura los valores que disparan cada tipo de alerta
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loadingThresholds ? (
              ...
              {thresholds.map((t: any) => (
                <div key={t.id} className="flex items-center gap-4">
                  <Label className="flex-1 text-sm">
                    {ALERT_TYPE_LABELS[t.alertType] ?? t.alertType}
                  </Label>
                  <Input
                    type="number"
                    className="w-28"
                    value={thresholdValues[t.alertType] ?? t.threshold}
                    onChange={e =>
                      setThresholdValues(p => ({
                        ...p,
                        [t.alertType]: parseFloat(e.target.value),
                      }))
                    }
                  />
                  <Button
                    ...
                    onClick={() =>
                      updateThreshold.mutate({
                        ...
                        threshold: thresholdValues[t.alertType] ?? t.threshold,
                      })
                    }
          ...
          <div className="space-y-3">
            {FREQ_OPTIONS.filter(o => o.value !== "disabled").map(opt => (
              <div key={opt.value} className="flex items-center gap-4">
                ...
                <Select
                  value={scheduleValues[opt.value] ?? "disabled"}
                  onValueChange={v =>
                    setScheduleValues(p => ({ ...p, [opt.value]: v }))
                  }
                >
            ...
            <Input
              ...
              onKeyDown={e => {
                if (e.key === "Enter" && newRecipient.trim()) {
                  setRecipients(p => [...p, newRecipient.trim()]);
                  setNewRecipient("");
                }
              }}
            />
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Vista unificada: alertas operativas, métricas, IA predictiva y
          configuración
        </p>
      </div>
