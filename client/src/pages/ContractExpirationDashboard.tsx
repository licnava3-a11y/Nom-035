...
function getUrgencyBadge(days: number | null) {
  ...
  if (days < 0)
    return <Badge className="bg-gray-200 text-gray-700 text-xs">Vencido</Badge>;
  if (days <= 2)
    return (
  ...
  if (days <= 7)
    return (
      <Badge className="bg-red-100 text-red-700 text-xs">🔴 {days}d</Badge>
    );
  if (days <= 15)
    return (
  ...
  return (
    <Badge className="bg-green-100 text-green-700 text-xs">🟢 {days}d</Badge>
  );
}
...
export default function ContractExpirationDashboard() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.hiring.getExpiringContracts.useQuery(
    { daysAhead: daysFilter },
    { refetchOnWindowFocus: false }
  );

  const sendReportMutation =
    trpc.hiring.sendExpiringContractsReport.useMutation({
      onSuccess: () =>
        toast.success("Reporte enviado a RH por correo electrónico"),
  ...
  const stats = useMemo(
    () => ({
      ...
      warning: filteredRows.filter(
        r => r.daysUntil !== null && r.daysUntil > 7 && r.daysUntil <= 15
      ).length,
      safe: filteredRows.filter(r => r.daysUntil !== null && r.daysUntil > 15)
        .length,
    }),
    [filteredRows]
  );
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
          ...
          {isLoading ? (
            ...
            <div className="py-12 text-center text-muted-foreground">
              ...
              <p className="font-medium">
                Sin vencimientos en los próximos {daysFilter} días
              </p>
              <p className="text-xs mt-1">
                Todos los contratos están al corriente.
              </p>
            </div>
          ) : (
                  ...
                  {filteredRows.map((row, idx) => (
                    <tr
                      ...
                      className={`border-b transition-colors hover:bg-muted/20 ${
                        row.daysUntil !== null && row.daysUntil <= 7
                          ? "bg-red-50/40"
                          : row.daysUntil !== null && row.daysUntil <= 15
                            ? "bg-amber-50/30"
                            : ""
                      }`}
                    >
      ...
      <p className="text-xs text-muted-foreground text-center">
        Datos actualizados en tiempo real · Exportación en formato XLSX para
        auditoría STPS · Alertas automáticas enviadas a RH 7 días antes del
        vencimiento
      </p>
    </div>
  );
