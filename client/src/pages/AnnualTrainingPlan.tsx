...
function ItemStatusBadge({ status }: { status: string }) {
  const s = ITEM_STATUS_LABELS[status] ?? {
    label: status,
    color: "bg-slate-100 text-slate-700",
    icon: Clock,
  };
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium ${s.color}`}
    >
...
// ─── Exportar PAC a XLSX ──────────────────────────────────────────────────────
async function exportXLSX(plan: any, items: any[]) {
  ...
  const rows = items.map(item => ({
    ...
    "Fecha Planeada": item.plannedDate
      ? new Date(item.plannedDate).toLocaleDateString("es-MX")
      : "",
    "Fecha Real": item.actualDate
      ? new Date(item.actualDate).toLocaleDateString("es-MX")
      : "",
  ...
  const resumen = [
    ["Plan", plan.title ?? ""],
    ["Año", plan.year ?? ""],
    ["Departamento", plan.departmentName ?? "General"],
    ["Estatus", plan.status ?? ""],
  ...
  XLSX.writeFile(
    wb,
    `PAC_${plan.year}_${(plan.title ?? "plan").replace(/\s+/g, "_")}_${new Date().toISOString().slice(0, 10)}.xlsx`
  );
}
...
async function exportPDF(plan: any, items: any[]) {
  ...
  doc.setFontSize(10);
  doc.text("PROGRAMA ANUAL DE CAPACITACIÓN (PAC)", pageW / 2, 9, {
    align: "center",
  });
  ...
  doc.text(
    `Año: ${plan.year}  |  Departamento: ${plan.departmentName ?? "General"}  |  Estatus: ${STATUS_LABELS[plan.status]?.label ?? plan.status}`,
    pageW / 2,
    y,
    { align: "center" }
  );
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(
      `Generado: ${new Date().toLocaleDateString("es-MX")}  |  Pág. ${i}/${totalPages}`,
      pageW - 15,
      290,
      { align: "right" }
    );
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <Input
          ...
          placeholder="Ej: PAC 2025 - NOM-035 STPS"
        />
      </div>
      <div>
...
function ItemForm({
  ...
  const [form, setForm] = useState({
    ...
    plannedDate: initial?.plannedDate
      ? new Date(initial.plannedDate).toISOString().split("T")[0]
      : "",
    instructor: initial?.instructor ?? "",
    estimatedCost: initial?.estimatedCost ? String(initial.estimatedCost) : "",
    participantsTarget: initial?.participantsTarget
      ? String(initial.participantsTarget)
      : "",
    ...
    participantsActual: initial?.participantsActual
      ? String(initial.participantsActual)
      : "",
    completedDate: initial?.completedDate
      ? new Date(initial.completedDate).toISOString().split("T")[0]
      : "",
    dncId: initial?.dncId ? String(initial.dncId) : "",
  });
  const set = (k: string, v: string) => setForm(p => ({ ...p, [k]: v }));
  const { data: dncNeeds } = trpc.annualTrainingPlan.listDncNeeds.useQuery(
    {},
    { retry: false }
  );
    ...
    <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          ...
          <Input
            type="number"
            value={form.participantsTarget}
            onChange={e => set("participantsTarget", e.target.value)}
          />
        </div>
        <div>
          ...
          <Input
            ...
            onChange={e => set("participantsActual", e.target.value)}
          />
        </div>
      </div>
      <div>
      ...
      <div>
        ...
        <Input
          value={form.normativeReference}
          onChange={e => set("normativeReference", e.target.value)}
          placeholder="Ej: NOM-035-STPS-2018, Art. 8"
        />
      </div>
      <div>
      ...
      <div>
        <Label>
          Vincular a DNC (Detección de Necesidades){" "}
          <span className="text-slate-400 font-normal text-xs">— opcional</span>
        </Label>
        <Select
          value={form.dncId || "__none__"}
          onValueChange={v => set("dncId", v === "__none__" ? "" : v)}
        >
          <SelectTrigger>
            <SelectValue placeholder="Sin vinculación DNC" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="__none__">Sin vinculación DNC</SelectItem>
            {(dncNeeds ?? []).map(n => (
              <SelectItem key={n.id} value={String(n.id)}>
                [{n.priority.toUpperCase()}] {n.competencyName} — brecha:{" "}
                {n.gap}
              </SelectItem>
            ))}
          </SelectContent>
        ...
        <Button
          onClick={() =>
            onSave({
              ...
              participantsTarget: form.participantsTarget
                ? Number(form.participantsTarget)
                : undefined,
              participantsActual: form.participantsActual
                ? Number(form.participantsActual)
                : undefined,
...
function PlanDetail({
  ...
  const { data, isLoading } = trpc.annualTrainingPlan.getById.useQuery(
    { id: planId },
    { retry: false }
  );
  const { data: stats } = trpc.annualTrainingPlan.getStats.useQuery(
    { planId },
    { retry: false }
  );
  ...
  const addItem = trpc.annualTrainingPlan.addItem.useMutation({
    onSuccess: () => {
      utils.annualTrainingPlan.getById.invalidate({ id: planId });
      utils.annualTrainingPlan.getStats.invalidate({ planId });
      setShowAddItem(false);
      toast({ title: "Curso agregado" });
    },
    onError: e =>
  ...
  if (!data)
    return (
      <div className="text-center text-slate-500 py-12">Plan no encontrado</div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex gap-2">
          <Button
            ...
            variant="outline"
            className="gap-1.5"
            onClick={() => exportPDF(data, data.items)}
          >
          ...
          <Button
            ...
            onClick={() => exportXLSX(data, data.items)}
          >
            ...
            <TableBody>
              {data.items.length === 0 && (
                <TableRow>
                  ...
                  >
                    Sin cursos registrados. Haz clic en "Agregar curso".
                  </TableCell>
                </TableRow>
              )}
              {data.items.map((item: any) => (
                <TableRow key={item.id} className="hover:bg-slate-50">
                  ...
                  <TableCell className="text-sm">
                    {item.participantsActual ?? "-"}/
                    {item.participantsTarget ?? "-"}
                  </TableCell>
                  ...
                  <TableCell className="text-center">
                    <StaleBadge dias={diasSinActualizar(item.updatedAt)} />
                  </TableCell>
                  <TableCell>
      ...
      <Dialog open={showAddItem} onOpenChange={setShowAddItem}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Agregar Curso al PAC</DialogTitle>
          </DialogHeader>
          <ItemForm
            planId={planId}
            onSave={d => addItem.mutate(d)}
        ...
        <DialogContent className="max-w-lg">
          ...
          {editItem && (
            <ItemForm
              planId={planId}
              initial={editItem}
              onSave={d => updItem.mutate({ id: editItem.id, ...d })}
      ...
      >
        <DialogContent className="max-w-sm">
          ...
          <p className="text-sm text-slate-600">
            ¿Confirmas que deseas eliminar este curso del PAC? Esta acción no se
            puede deshacer.
          </p>
          <DialogFooter>
            ...
            <Button
              variant="destructive"
              onClick={() =>
                deleteItemId && delItem.mutate({ id: deleteItemId })
              }
            >
              Eliminar
            </Button>
...
export default function AnnualTrainingPlan() {
  ...
  const { data: availableYears } =
    trpc.annualTrainingPlan.getAvailableYears.useQuery(undefined, {
      retry: false,
    });
  ...
  const createMut = trpc.annualTrainingPlan.create.useMutation({
    onSuccess: () => {
      utils.annualTrainingPlan.list.invalidate();
      setShowCreate(false);
      toast({ title: "PAC creado exitosamente" });
    },
    onError: e =>
      toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
      ...
      <div className="p-6 max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Gestión de planes anuales de capacitación conforme a NOM-035 STPS
              2018.
            </p>
          </div>
                ...
                <TableBody>
                  {(() => {
                    const filteredPlans = onlyStalePlans
                      ? (data?.plans ?? []).filter(
                          (p: any) => Number(p.staleItemsCount) > 0
                        )
                      : (data?.plans ?? []);
                    if (filteredPlans.length === 0)
                      return (
                        <TableRow>
                          ...
                          >
                            {onlyStalePlans
                              ? "No hay planes con cursos rezagados."
                              : 'No se encontraron planes. Crea el primero con "Nuevo PAC".'}
                          </TableCell>
                        </TableRow>
                      );
                    return filteredPlans.map((plan: any) => (
                      <TableRow key={plan.id} className="hover:bg-slate-50">
                        ...
                        <TableCell className="text-sm text-slate-600">
                          {plan.responsibleFirstName
                            ? `${plan.responsibleFirstName} ${plan.responsibleLastName ?? ""}`
                            : "-"}
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-sm">
                          {plan.totalBudget
                            ? `$${Number(plan.totalBudget).toLocaleString("es-MX")}`
                            : "-"}
                        </TableCell>
                        <TableCell>
        ...
        {data && data.total > 15 && (
          <div className="flex items-center justify-between text-sm text-slate-500">
            <span>
              Mostrando {Math.min((page - 1) * 15 + 1, data.total)}–
              {Math.min(page * 15, data.total)} de {data.total}
            </span>
            <div className="flex gap-2">
              <Button
                size="sm"
        ...
        <Dialog open={showCreate} onOpenChange={setShowCreate}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Nuevo Programa Anual de Capacitación</DialogTitle>
            </DialogHeader>
            <PlanForm
        ...
        <Dialog open={!!editPlan} onOpenChange={o => !o && setEditPlan(null)}>
          <DialogContent className="max-w-lg">
            ...
            {editPlan && (
              <PlanForm
                initial={editPlan}
                onSave={d => updateMut.mutate({ id: editPlan.id, ...d })}
        ...
        >
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle>Eliminar PAC</DialogTitle>
            </DialogHeader>
            <p className="text-sm text-slate-600">
              ¿Confirmas que deseas eliminar este Programa Anual de Capacitación
              y todos sus cursos? Esta acción no se puede deshacer.
            </p>
            <DialogFooter>
              ...
              <Button
                variant="destructive"
                onClick={() =>
                  deletePlanId && deleteMut.mutate({ id: deletePlanId })
                }
              >
                Eliminar
              </Button>
