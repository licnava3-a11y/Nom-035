...
function CompactDepartmentNode({
  data,
}: NodeProps<{
  name: string;
  code: string;
...
export default function OrganizationChart() {
  const { data: hierarchy, isLoading } =
    trpc.departments.getHierarchy.useQuery();
  ...
  const { data: historicalHierarchy, isLoading: isLoadingHistorical } =
    trpc.departments.getHierarchyAtDate.useQuery(
      { date: selectedDate },
  ...
  useEffect(() => {
    ...
    getLayoutedElements(
      activeHierarchy as DepartmentNode[],
      isCompactMode,
      orientation
    )
  ...
  const goToPreviousMatch = () => {
    ...
    setCurrentMatchIndex(
      prev => (prev - 1 + matchedNodeIds.length) % matchedNodeIds.length
    );
  };
      ...
      toPng(flowElement, {
        ...
        .then(dataUrl => {
          ...
          const orientationLabel =
            orientation === "DOWN" ? "vertical" : "horizontal";
    ...
    const flattenHierarchy = (
      ...
      depts.forEach((dept: any) => {
        result.push({
          ...
          "Jefe del Departamento": dept.managerId
            ? `ID: ${dept.managerId}`
            : "Sin asignar",
        });
  ...
  return (
    <div
      className={`container py-8 space-y-6 ${isPrintMode ? "print-mode" : ""}`}
    >
      {/* Header */}
      <div className="flex items-center justify-between gap-4 print:hidden">
        <div className="flex-1">
          ...
          <p className="text-muted-foreground mt-2">
            Visualización interactiva de la estructura jerárquica de
            departamentos
          </p>
        </div>
        ...
        <CardContent className="p-0 h-full print:p-4">
          {nodes.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <p className="text-muted-foreground">
                No hay departamentos para mostrar. Crea departamentos en la
                sección de Gestión de Talento.
              </p>
            </div>
          ) : (
            <ReactFlow
            ...
            >
              ...
              <Panel
                position="top-right"
                className="bg-white p-3 rounded-lg shadow-md border print:hidden"
              >
                <div className="text-sm space-y-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    • Click en nodo para detalles
                  </p>
                  <div className="border-t pt-2 mt-2 space-y-1">
                    <p className="text-xs text-muted-foreground">
                      <strong>Modo:</strong>{" "}
                      {isCompactMode ? "Compacto" : "Completo"}
                    </p>
                    <p className="text-xs text-muted-foreground">
