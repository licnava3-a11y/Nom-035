...
export default function MinuteRecipients() {
  ...
  const { data: recipients = [], isLoading } =
    trpc.minuteRecipients.list.useQuery({
  ...
  const createMutation = trpc.minuteRecipients.create.useMutation({
    onSuccess: () => {
      ...
      toast({ title: "Destinatario creado" });
      handleCloseForm();
    },
    onError: err =>
      toast({
  ...
  const handleOpenEdit = (r: Recipient) => {
    setEditingId(r.id);
    setFormData({
      name: r.name,
      email: r.email,
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    rawRows.forEach((row, i) => {
      ...
      if (!n.email || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(n.email)) {
        errors.push(`Fila ${i + 2}: correo inválido (${n.email})`);
        return;
      }
      if (!n.position) {
        errors.push(`Fila ${i + 2}: cargo vacío`);
        return;
      }
      validRows.push(n);
    });
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2 flex-1">
          ...
          <Tooltip>
            ...
            <TooltipContent>
              {onlyActive
                ? "Mostrando solo activos. Clic para ver todos."
                : "Clic para mostrar solo activos."}
            </TooltipContent>
          </Tooltip>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <Tooltip>
            ...
            <TooltipContent>
              Cargar destinatarios desde archivo Excel
            </TooltipContent>
          </Tooltip>
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        ...
                        >
                          <Badge
                            variant={
                              recipient.isActive ? "default" : "secondary"
                            }
                            className={`cursor-pointer select-none transition-opacity ${recipient.isActive ? "bg-green-100 text-green-700 border-green-200 hover:bg-green-200" : "opacity-60 hover:opacity-80"}`}
                          >
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button
                            ...
                            onClick={() =>
                              setLocation(
                                `/committee/minute-recipients/${recipient.id}/history`
                              )
                            }
                          >
        ...
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              Importar Destinatarios desde Excel
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="rounded-lg border bg-muted/30 p-4 space-y-2">
              ...
              <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                <li>
                  ...
                  <strong>nombre, correo, cargo, area</strong>
                </li>
                <li>
                  Los registros existentes (mismo correo) serán actualizados
                  automáticamente
                </li>
              ...
              <div className="flex items-center gap-2">
                ...
                {importPreviewReady && (
                  <span className="text-sm text-muted-foreground">
                    {importRows.length} fila(s) válidas cargadas
                  </span>
                )}
              </div>
            </div>
            {importErrors.length > 0 && (
              <div className="rounded-lg border border-destructive/30 bg-destructive/5 p-3 space-y-1">
                <div className="flex items-center gap-2 text-destructive text-sm font-medium">
                  ...
                  {importErrors.length} fila(s) con errores (serán omitidas):
                </div>
                <ul className="text-xs text-destructive/80 space-y-0.5 list-disc list-inside">
                  ...
                  {importErrors.length > 5 && (
                    <li>... y {importErrors.length - 5} más</li>
                  )}
                </ul>
              </div>
            )}
            {importPreviewReady && importRows.length > 0 && (
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-sm font-medium text-green-700">
                  ...
                  Vista previa — {importRows.length} destinatario(s) a importar:
                </div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              onClick={() => bulkImportMutation.mutate({ rows: importRows })}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. El destinatario será removido
