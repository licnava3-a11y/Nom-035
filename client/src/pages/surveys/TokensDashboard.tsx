...
export default function TokensDashboard() {
  ...
  const chartData = useMemo(() => {
    ...
    const positionChart = {
      labels: posData.map((p: any) =>
        p.position.length > 25 ? p.position.substring(0, 25) + "…" : p.position
      ),
      datasets: [
        {
          label: "Completados",
          data: posData.map((p: any) => p.completed),
          backgroundColor: COLORS.completed,
        },
        {
          label: "Pendientes",
          data: posData.map((p: any) => p.pending),
          backgroundColor: COLORS.pending,
        },
        {
          label: "Expirados",
          data: posData.map((p: any) => p.expired),
          backgroundColor: COLORS.expired,
        },
      ],
    };
      ...
      datasets: [
        {
          ...
          backgroundColor: genderData.map(
            (g: any) => GENDER_COLORS[g.sexo] ?? "#9CA3AF"
          ),
          borderWidth: 2,
          borderColor: "#fff",
        },
    ...
    const genderCompletionChart = {
      ...
      datasets: [
        {
          label: "Completados",
          data: genderData.map((g: any) => g.completed),
          backgroundColor: COLORS.completed,
        },
        {
          label: "Pendientes",
          data: genderData.map((g: any) => g.pending),
          backgroundColor: COLORS.pending,
        },
        {
          label: "Expirados",
          data: genderData.map((g: any) => g.expired),
          backgroundColor: COLORS.expired,
        },
      ],
    };
      ...
      datasets: [
        {
          ...
          backgroundColor: AGE_COLORS.slice(0, ageData.length).map(
            c => c + "CC"
          ),
      ...
      datasets: [
        {
          ...
          backgroundColor: ageData.map(
            (_: any, i: number) => AGE_COLORS[i % AGE_COLORS.length] + "99"
          ),
          borderColor: ageData.map(
            (_: any, i: number) => AGE_COLORS[i % AGE_COLORS.length]
          ),
          borderWidth: 2,
        },
      ],
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Select
              ...
              onValueChange={value =>
                setSelectedSurvey(value === "all" ? undefined : parseInt(value))
              }
            >
              <SelectTrigger>
      ...
      <Card className="p-5">
        ...
        <div className="h-5 bg-muted rounded-full overflow-hidden flex">
          <div
            ...
            style={{
              width: `${stats.totalTokens > 0 ? (stats.completedTokens / stats.totalTokens) * 100 : 0}%`,
            }}
          />
          <div
            className="h-full bg-yellow-400 transition-all"
            style={{
              width: `${stats.totalTokens > 0 ? (stats.pendingTokens / stats.totalTokens) * 100 : 0}%`,
            }}
          />
          <div
            className="h-full bg-red-400 transition-all"
            style={{
              width: `${stats.totalTokens > 0 ? (stats.expiredTokens / stats.totalTokens) * 100 : 0}%`,
            }}
          />
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-6 pt-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {ageData.length > 0 && (
              <Card className="p-6">
                <h3 className="font-semibold mb-4">
                  Distribución por Rango de Edad (NOM-035)
                </h3>
                <div className="h-[280px]">
                  <PolarArea
                ...
                <p className="text-xs text-muted-foreground mt-2 text-center">
                  Clasificación de grupos etarios según criterios
                  NOM-035-STPS-2018
                </p>
              </Card>
            )}
            ...
            {ageData.length > 0 && (
              <Card className="p-6">
                <h3 className="font-semibold mb-4">
                  Tasa de Completado por Rango de Edad
                </h3>
                <div className="h-[280px]">
                  <Bar
                    data={chartData.ageCompletionChart}
                    options={percentBarOptions(true) as any}
                  />
                </div>
              </Card>
            )}
            ...
            <Card className="p-6">
              ...
              <div className="h-[350px]">
                <Bar
                  ...
                  options={barOptions(true, true) as any}
                />
              </div>
            </Card>
        ...
        <TabsContent value="demo" className="space-y-6 pt-4">
          {genderData.length > 0 && (
            <Card className="p-6">
              <h3 className="font-semibold mb-4">
                Estado de Participación por Género
              </h3>
              <div className="h-[280px]">
                <Bar
                  data={chartData.genderCompletionChart}
                  options={barOptions(false, true) as any}
                />
              </div>
            </Card>
          )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card className="p-6">
                ...
                <p className="text-xs text-muted-foreground mb-4">
                  Grupos etarios según NOM-035-STPS-2018
                </p>
                    ...
                    <div key={a.group}>
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span className="text-muted-foreground">
                          {a.total} trabajadores ({a.completionRate.toFixed(0)}%
                          completado)
                        </span>
                      </div>
              ...
              <Card className="p-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {stats.byDepartment.filter(d => d.completionRate < 50)
                      .length > 0 ? (
                        ...
                        {stats.byDepartment
                          ...
                          .map(d => (
                            <div
                              ...
                              className="flex items-center justify-between text-sm"
                            >
                              <span className="text-red-700 font-medium truncate max-w-[180px]">
                                {d.department}
                              </span>
                      ...
                      <p className="text-sm text-green-700">
                        ✓ Todos los departamentos superan el 50% de cobertura
                      </p>
                    )}
                  </div>
                </div>
        ...
        <TabsContent value="detail" className="space-y-6 pt-4">
          ...
          {stats.pendingTokens > 0 && (
            <Card className="p-6">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                ...
                {
                  stats.tokens.filter((t: any) => t.status === "pendiente")
                    .length
                }
                )
              </h3>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {stats.tokens
                      ...
                      .sort((a: any, b: any) => {
                        const dA = a.expiresAt
                          ? Math.ceil(
                              (new Date(a.expiresAt).getTime() - Date.now()) /
                                86400000
                            )
                          : 0;
                        const dB = b.expiresAt
                          ? Math.ceil(
                      ...
                      .map((token: any) => {
                        ...
                        return (
                          <tr
                            key={token.tokenId}
                            className={`border-b hover:bg-muted/50 ${urgent ? "bg-red-50" : ""}`}
                          >
          ...
          <Card className="p-6">
            <h3 className="font-semibold mb-4">
              Todos los Tokens ({filteredTokens.length})
            </h3>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
