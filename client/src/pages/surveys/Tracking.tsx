...
export default function SurveysTracking() {
  ...
  const { data: departmentCoverage, isLoading: isLoadingDepts } = (
    trpc as any
  ).surveys.getCoverageByDepartment.useQuery(surveyId);
  ...
  const { data: pendingWorkers, isLoading: isLoadingPending } = (
    trpc as any
  ).surveys.getPendingWorkers.useQuery({
    surveyId,
  ...
  const generatePDF = (
    trpc as any
  ).surveys.generatePendingWorkersPDF.useMutation();

  // Mutation para enviar recordatorios
  const sendReminders = (
    trpc as any
  ).surveys.sendPendingWorkersReminders.useMutation();
  ...
  const totalWorkers = stats?.totalResponses
    ? Math.round(
        stats.totalResponses /
          (stats.completionRate ? parseFloat(stats.completionRate) / 100 : 1)
      )
    : 0;
  const coveragePercentage = stats?.completionRate
    ? parseFloat(stats.completionRate)
    : 0;
    ...
    <div className="container mx-auto py-8 space-y-6">
      <div>
        <h1 className="text-3xl font-bold mb-2">
          Seguimiento de Encuestas NOM-035
        </h1>
      ...
      {coverageStatus === "critical" && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            La cobertura actual ({coveragePercentage.toFixed(1)}%) está por
            debajo del mínimo recomendado. Se requiere acción inmediata para
      ...
      {isLoading ? (
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-orange-600">
                {totalWorkers - (stats?.totalResponses || 0)}
              </div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                Cobertura (Ecuación 1)
              </CardTitle>
              <div
                className={`h-8 w-8 rounded-full flex items-center justify-center ${coverageColor}`}
              >
                  ...
                  <div key={dept.department} className="space-y-2">
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex items-center gap-4 text-muted-foreground">
                        <span>
                          {dept.responded}/{dept.total}
                        </span>
                        <Badge
                          variant={
                            deptCoverageStatus === "excellent" ||
                            deptCoverageStatus === "good"
                              ? "default"
                              : "destructive"
                          }
                        >
          ...
          {isLoadingPending ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mt-2">
                Cargando trabajadores pendientes...
              </p>
            </div>
            ...
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">
                Todos los trabajadores han completado la encuesta
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2 text-sm">
            <p>
              <strong>Niveles de cumplimiento:</strong>
            </p>
            <ul className="space-y-1 ml-4">
              <li className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full bg-green-600"></div>
                <span>
                  <strong>Excelente:</strong> ≥ 90% - Cumplimiento óptimo
                </span>
              </li>
              <li className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full bg-blue-600"></div>
                <span>
                  <strong>Bueno:</strong> 70-89% - Cumplimiento aceptable
                </span>
              </li>
              <li className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full bg-yellow-600"></div>
                <span>
                  <strong>Requiere atención:</strong> 50-69% - Mejorar cobertura
                </span>
              </li>
              <li className="flex items-center gap-2">
                <div className="h-3 w-3 rounded-full bg-red-600"></div>
                <span>
                  <strong>Crítico:</strong> &lt; 50% - Acción inmediata
                  requerida
                </span>
              </li>
            </ul>
