...
import { Download, TrendingUp, Users, Briefcase, Calendar, Heart, UserCheck, Clock, FileText, Award } from 'lucide-react';
import {
  Chart as ChartJS,
  CategoryScale,
  LinearScale,
...
export default function ActionPlan() {
  ...
  const { data: deptAnalysis, isLoading: loadingDept } = trpc.actionPlan.getDepartmentalAnalysis.useQuery({ surveyId });
  const { data: posAnalysis, isLoading: loadingPos } = trpc.actionPlan.getPositionAnalysis.useQuery({ surveyId });
  const { data: ageAnalysis, isLoading: loadingAge } = trpc.actionPlan.getAgeRangeAnalysis.useQuery({ surveyId });
  const { data: genderAnalysis, isLoading: loadingGender } = trpc.actionPlan.getGenderAnalysis.useQuery({ surveyId });
  const { data: maritalAnalysis, isLoading: loadingMarital } = trpc.actionPlan.getMaritalStatusAnalysis.useQuery({ surveyId });
  const { data: scheduleAnalysis, isLoading: loadingSchedule } = trpc.actionPlan.getWorkScheduleAnalysis.useQuery({ surveyId });
  const { data: contractAnalysis, isLoading: loadingContract } = trpc.actionPlan.getContractTypeAnalysis.useQuery({ surveyId });
  const { data: tenureAnalysis, isLoading: loadingTenure } = trpc.actionPlan.getTenureAnalysis.useQuery({ surveyId });
      ...
      <Tabs defaultValue="organizational" className="space-y-6" onValueChange={setCurrentTab}>
        ...
        <TabsContent value="organizational">
          {loadingOrg ? (
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground">Cargando análisis organizacional...</p>
            </Card>
          ) : orgAnalysis ? (
            <AnalysisView data={[orgAnalysis]} title="Análisis Organizacional" />
          ) : (
            <Card className="p-8 text-center">
        ...
        <TabsContent value="departmental">
          {loadingDept ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis departamental...</p>
            </Card>
          ) : deptAnalysis && deptAnalysis.length > 0 ? (
            <AnalysisView data={deptAnalysis} title="Análisis por Departamento" />
          ) : (
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No hay datos disponibles</p>
            </Card>
        ...
        <TabsContent value="position">
          {loadingPos ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por puesto...</p>
            </Card>
        ...
        <TabsContent value="age">
          {loadingAge ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por edad...</p>
            </Card>
          ) : ageAnalysis && ageAnalysis.length > 0 ? (
            <AnalysisView data={ageAnalysis} title="Análisis por Rango de Edad" />
          ) : (
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No hay datos disponibles</p>
            </Card>
        ...
        <TabsContent value="gender">
          {loadingGender ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por género...</p>
            </Card>
        ...
        <TabsContent value="marital">
          {loadingMarital ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por estado civil...</p>
            </Card>
          ) : maritalAnalysis && maritalAnalysis.length > 0 ? (
            <AnalysisView data={maritalAnalysis} title="Análisis por Estado Civil" />
          ) : (
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No hay datos disponibles</p>
            </Card>
        ...
        <TabsContent value="schedule">
          {loadingSchedule ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por jornada...</p>
            </Card>
          ) : scheduleAnalysis && scheduleAnalysis.length > 0 ? (
            <AnalysisView data={scheduleAnalysis} title="Análisis por Jornada Laboral" />
          ) : (
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No hay datos disponibles</p>
            </Card>
        ...
        <TabsContent value="contract">
          {loadingContract ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por contrato...</p>
            </Card>
          ) : contractAnalysis && contractAnalysis.length > 0 ? (
            <AnalysisView data={contractAnalysis} title="Análisis por Tipo de Contrato" />
          ) : (
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No hay datos disponibles</p>
            </Card>
        ...
        <TabsContent value="tenure">
          {loadingTenure ? (
            <Card className="p-8 text-center">
              <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4"></div>
              <p className="text-muted-foreground">Cargando análisis por antigüedad...</p>
            </Card>
          ) : tenureAnalysis && tenureAnalysis.length > 0 ? (
            <AnalysisView data={tenureAnalysis} title="Análisis por Antigüedad" />
          ) : (
            <Card className="p-8 text-center">
              <p className="text-muted-foreground">No hay datos disponibles</p>
            </Card>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="text-center">
            ...
            <p className="text-sm text-muted-foreground">Segmentos Analizados</p>
          </div>
          <div className="text-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Score Promedio por Segmento</h3>
          <Bar
        ...
        <Card className="p-6">
          <h3 className="text-lg font-semibold mb-4">Respuestas por Segmento</h3>
          <Bar
            ...
            <tbody>
              {data.map((segment, idx) => (
                <tr key={idx} className="border-b">
                  ...
                  <td className="text-center p-3 font-semibold">{segment.avgScore.toFixed(1)}</td>
                  <td className="text-center p-3">
                    <span className="inline-block px-2 py-1 rounded text-sm" style={{ backgroundColor: RISK_COLORS.nulo, color: 'white' }}>
                      {segment.riskDistribution.nulo}
                    </span>
                  </td>
                  <td className="text-center p-3">
                    <span className="inline-block px-2 py-1 rounded text-sm" style={{ backgroundColor: RISK_COLORS.bajo, color: 'white' }}>
                      {segment.riskDistribution.bajo}
                    </span>
                  </td>
                  <td className="text-center p-3">
                    <span className="inline-block px-2 py-1 rounded text-sm" style={{ backgroundColor: RISK_COLORS.medio, color: 'white' }}>
                      {segment.riskDistribution.medio}
                    </span>
                  </td>
                  <td className="text-center p-3">
                    <span className="inline-block px-2 py-1 rounded text-sm" style={{ backgroundColor: RISK_COLORS.alto, color: 'white' }}>
                      {segment.riskDistribution.alto}
                    </span>
                  </td>
                  <td className="text-center p-3">
                    <span className="inline-block px-2 py-1 rounded text-sm" style={{ backgroundColor: RISK_COLORS.muy_alto, color: 'white' }}>
