...
export function AssignCommitteeModal({
  ...
  const { data: committeeMembers, isLoading: loadingMembers } = trpc.cases.getCommitteeMembers.useQuery() as any;

  // Obtener distribución de carga de trabajo
  const { data: workload, isLoading: loadingWorkload } = trpc.cases.getCommitteeWorkload.useQuery() as any;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecciona un miembro del comité para asignar el caso <strong>{caseNumber}</strong>.
            La distribución de carga de trabajo te ayudará a equilibrar la asignación.
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-4">
            {committeeMembers && committeeMembers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-8 text-center">
                ...
                <p className="text-gray-600">No hay miembros del comité disponibles</p>
                              ...
                              <div className="flex items-center gap-4 mt-3">
                                <div className="flex items-center gap-2">
                                  ...
                                  <span className="text-sm text-gray-600">
                                    <strong>{activeCases}</strong> caso{activeCases !== 1 ? 's' : ''} activo{activeCases !== 1 ? 's' : ''}
                                  </span>
                                </div>
            ...
            {workload && workload.length > 0 && (
              <Card className="bg-blue-50 border-blue-200">
                <CardContent className="p-4">
                  <h4 className="font-medium text-blue-900 mb-2">Distribución de Carga de Trabajo</h4>
                  <div className="grid grid-cols-3 gap-4 text-sm">
                    <div>
                      <p className="text-blue-700 font-medium">Total de casos activos</p>
                      <p className="text-2xl font-bold text-blue-900">
                        {workload.reduce((sum: number, w: any) => sum + w.activeCases, 0)}
                      </p>
                    </div>
                    <div>
                      <p className="text-blue-700 font-medium">Promedio por miembro</p>
                      <p className="text-2xl font-bold text-blue-900">
                        {(workload.reduce((sum: number, w: any) => sum + w.activeCases, 0) / workload.length).toFixed(1)}
                      </p>
                    </div>
                    <div>
                      <p className="text-blue-700 font-medium">Miembros disponibles</p>
                      <p className="text-2xl font-bold text-blue-900">
                        {workload.filter((w: any) => w.activeCases === 0).length}
                      </p>
                    </div>
                  </div>
                </CardContent>
