...
export const EmployeesBarChart = forwardRef<
  BarChartHandle,
...
>(function EmployeesBarChart({ positions }, ref) {
  ...
  useEffect(() => {
    ...
    const loadChart = async () => {
      ...
      chartInst.current = new Chart(canvasRef.current, {
        type: "bar",
        data: {
          labels: sorted.map(position =>
            position.title.length > 28
              ? `${position.title.slice(0, 26)}…`
              : position.title
          ),
          datasets: [
            {
              ...
              backgroundColor: sorted.map(
                position => riskColors[position.riskLevel] ?? "#3b82f6"
              ),
              borderRadius: 4,
              borderSkipped: false,
            },
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: {
                label: context =>
                  ` ${context.parsed.x} empleado${context.parsed.x !== 1 ? "s" : ""}`,
                afterLabel: context => {
  ...
  return (
    <DeferredChartFrame loading={!chartReady} type="bar" heightClass="h-64">
