...
export function HistoryTrendChart({ data }: { data: HistoryPoint[] }) {
  ...
  useEffect(() => {
    ...
    const loadChart = async () => {
      ...
      const labels = sorted.map(record =>
        new Date(record.analyzedAt).toLocaleDateString("es-MX", {
          day: "2-digit",
          month: "short",
        })
      );
      ...
      const pointColors = values.map(value =>
        value >= 3.5 ? "#dc2626" : value >= 2.5 ? "#d97706" : "#16a34a"
      );
      chartInst.current = new Chart(canvasRef.current, {
        ...
        options: {
          ...
          plugins: {
            ...
            tooltip: {
              callbacks: { label: context => ` Índice: ${context.parsed.y}/5` },
            },
          },
          scales: {
            y: {
  ...
  return (
    <DeferredChartFrame loading={!chartReady} type="line" heightClass="h-36">
