...
export function BulkConvertToLeadModal({
  ...
  const [deselectedEvents, setDeselectedEvents] = useState<Set<number>>(
    new Set()
  );
  ...
  const convertibleEvents = selectedEvents.filter(
    e => e.conversionStatus !== "converted"
  );
  const alreadyConvertedCount =
    selectedEvents.length - convertibleEvents.length;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Estás a punto de convertir {selectedCount} evento
            {selectedCount !== 1 ? "s" : ""} de WhatsApp en leads.
            {alreadyConvertedCount > 0 && (
              <span className="block mt-2 text-yellow-600">
                <AlertCircle className="inline h-4 w-4 mr-1" />
                {alreadyConvertedCount} evento
                {alreadyConvertedCount !== 1 ? "s" : ""} ya{" "}
        ...
        <div className="space-y-3 mt-4">
          {selectedEvents.map((event: any) => {
            ...
            const userData =
              event.userData && typeof event.userData === "object"
                ? (event.userData as any)
                : {};
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    {event.normativas &&
                      Array.isArray(event.normativas) &&
                      event.normativas.length > 0 && (
