...
test.describe('Calendario de Deadlines', () => {
  ...
  test('Navegación entre meses funciona correctamente', async ({ page }) => {
    // Navegar al calendario
    await page.click('text=Calendario');
    await page.waitForURL('**/calendar');

    // Verificar que el calendario se renderizó
    await expect(page.locator('.fc-toolbar, [class*="calendar"]')).toBeVisible();

    // Obtener mes actual
    const currentMonth = await page.locator('.fc-toolbar-title, [class*="month-title"]').textContent();
    
    // Click en "Siguiente mes"
    await page.click('button[aria-label="Next month"], button:has-text("›"), button:has-text("Siguiente")');
    
    // Verificar que el mes cambió
    const nextMonth = await page.locator('.fc-toolbar-title, [class*="month-title"]').textContent();
    expect(nextMonth).not.toBe(currentMonth);

    // Click en "Mes anterior"
    await page.click('button[aria-label="Previous month"], button:has-text("‹"), button:has-text("Anterior")');
    
    // Verificar que volvió al mes original
    const backToMonth = await page.locator('.fc-toolbar-title, [class*="month-title"]').textContent();
    expect(backToMonth).toBe(currentMonth);
  });

  test('Filtros por tipo de evento funcionan', async ({ page }) => {
    await page.click('text=Calendario');
    await page.waitForURL('**/calendar');

    // Verificar que hay eventos visibles inicialmente
    const initialEvents = await page.locator('.fc-event, [class*="event"]').count();
    expect(initialEvents).toBeGreaterThan(0);

    // Aplicar filtro (ejemplo: solo cursos)
    const filterSelect = page.locator('select[name="eventType"], select:has-text("Tipo de evento")');
    if (await filterSelect.isVisible()) {
      ...
      const filteredEvents = await page.locator('.fc-event, [class*="event"]').count();
  ...
  test('Click en evento muestra detalles', async ({ page }) => {
    await page.click('text=Calendario');
    await page.waitForURL('**/calendar');

    // Esperar a que los eventos carguen
    await page.waitForSelector('.fc-event, [class*="event"]', { timeout: 5000 });
  ...
  test('Eventos se renderizan en las fechas correctas', async ({ page }) => {
    await page.click('text=Calendario');
    await page.waitForURL('**/calendar');
...
test.describe('Gráficos del Dashboard', () => {
  ...
  test('Gráficos de Chart.js se renderizan correctamente', async ({ page }) => {
  ...
  test('Interacción con gráficos (hover) funciona', async ({ page }) => {
  ...
  test('Gráfico de casos por mes muestra datos correctos', async ({ page }) => {
    ...
    await expect(page.locator('text=Casos por Mes, text=Tendencia')).toBeVisible();

    // Verificar que hay métricas numéricas
    await expect(page.locator('text=/\\d+/')).toBeVisible(); // Cualquier número
  });

  test('Gráfico de distribución por tipo funciona', async ({ page }) => {
    await page.click('text=Prevención de Riesgos');
    await page.click('text=Métricas de Casos');
    await page.waitForURL('**/cases-metrics');
  ...
  test('Gráficos de NMX-025 se renderizan correctamente', async ({ page }) => {
  ...
  test('Gráficos responden a cambios de datos', async ({ page }) => {
    await page.click('text=Prevención de Riesgos');
    await page.click('text=Métricas de Casos');
    ...
    const dateFilter = page.locator('select[name="period"], select:has-text("Período")');
...
test.describe('Compatibilidad Multi-Navegador', () => {
  test('Dashboard funciona en diferentes viewports', async ({ page }) => {
    // Test en desktop
