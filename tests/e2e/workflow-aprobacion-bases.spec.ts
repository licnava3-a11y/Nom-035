...
test.describe("Workflow de Aprobación de Bases de Funcionamiento", () => {
  ...
  test("Crear y aprobar base de funcionamiento completa", async ({
    mockedAuthPage: page,
  }) => {
    ...
    await expect(
      page.locator("h1, h2").filter({ hasText: "Bases de Funcionamiento" })
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Crear Nueva Base de Funcionamiento")
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=debe tener al menos 50 caracteres")
    ).toBeVisible({ timeout: 1000 });

    // Completar con texto válido
    await objectivesField.fill(
      "Establecer los lineamientos y procedimientos para el funcionamiento efectivo del comité de prevención de riesgos psicosociales, garantizando el cumplimiento de la NOM-035-STPS-2018 y promoviendo un entorno laboral saludable."
    );
    ...
    await page
      .locator('textarea[name="structure"]')
      .fill(
        "El comité estará conformado por representantes de la empresa y de los trabajadores, con un mínimo de 3 integrantes y un máximo de 10, designados de acuerdo con los criterios establecidos en la normativa vigente."
      );

    // Completar roles y responsabilidades
    await page
      .locator('textarea[name="roles"]')
      .fill(
        "Coordinador: Dirigir las sesiones y dar seguimiento a acuerdos. Secretario: Elaborar minutas y mantener archivo. Vocales: Participar en investigaciones y proponer medidas preventivas."
      );

    // Completar quórum
    await page
      .locator('textarea[name="quorum"]')
      .fill(
        "Se requiere la presencia de al menos el 50% más uno de los integrantes del comité para sesionar válidamente. Las decisiones se tomarán por mayoría simple de los presentes."
      );

    // Completar manejo de casos
    await page
      .locator('textarea[name="caseHandling"]')
      .fill(
        "Los casos se recibirán de forma confidencial, se investigarán en un plazo máximo de 30 días hábiles, y se emitirán recomendaciones para su atención y seguimiento."
      );

    // Completar confidencialidad
    await page
      .locator('textarea[name="confidentiality"]')
      .fill(
        "Todos los integrantes del comité firmarán un compromiso de confidencialidad. La información de los casos se manejará con estricta reserva y solo será compartida con personal autorizado."
      );

    // 7. Verificar indicador de guardado automático
    await expect(page.locator("text=Guardando..., text=Guardado")).toBeVisible({
      timeout: 35000,
    });
    ...
    await expect(
      page.locator("text=Base de funcionamiento creada exitosamente")
    ).toBeVisible({ timeout: 5000 });

    // 10. Verificar que aparece en la lista
    await expect(
      page.locator("text=Establecer los lineamientos")
    ).toBeVisible();
    ...
    await expect(
      page.locator("text=Base de funcionamiento aprobada")
    ).toBeVisible({ timeout: 3000 });
  });

  test("Validación en tiempo real funciona correctamente", async ({
    mockedAuthPage: page,
  }) => {
    // Login y navegación
    const loginButton = page.locator("text=Acceder a la Plataforma");
    ...
    // Corregir texto
    await objectivesField.fill(
      "Este es un texto suficientemente largo para cumplir con la validación mínima de cincuenta caracteres requeridos por el sistema."
    );
  ...
  test("Confirmación de salida con cambios sin guardar", async ({
    mockedAuthPage: page,
  }) => {
    // Login y navegación
    const loginButton = page.locator("text=Acceder a la Plataforma");
    ...
    // Hacer cambios
    await page
      .locator('textarea[name="objectives"]')
      .fill(
        "Este es un cambio sin guardar que debería activar la confirmación de salida del formulario."
      );
  ...
  test("Recuperación de borradores al volver", async ({
    mockedAuthPage: page,
  }) => {
    // Login y navegación
    const loginButton = page.locator("text=Acceder a la Plataforma");
    ...
    const testText =
      "Este es un borrador que debería recuperarse automáticamente cuando vuelva a abrir el formulario de creación.";
