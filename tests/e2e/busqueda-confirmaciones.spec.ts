...
test.describe("Búsqueda Global (Ctrl+K)", () => {
  ...
  test("Abrir búsqueda con Ctrl+K", async ({ mockedAuthPage: page }) => {
    ...
    await expect(
      page.locator(
        '[role="dialog"]:has-text("Buscar"), [class*="search-dialog"]'
      )
  ...
  test("Búsqueda funciona correctamente", async ({ mockedAuthPage: page }) => {
    ...
    await expect(
      page.locator('[class*="search-result"], [role="option"]')
    ).toBeVisible({ timeout: 3000 });

    // Verificar que hay al menos un resultado
    const results = await page
      .locator('[class*="search-result"], [role="option"]')
      .count();
  ...
  test("Navegación a resultados funciona", async ({ mockedAuthPage: page }) => {
    ...
    await expect(
      page.locator('[role="dialog"]:has-text("Buscar")')
    ).not.toBeVisible();
  });
  ...
  test("Búsqueda vacía muestra mensaje apropiado", async ({
    mockedAuthPage: page,
  }) => {
    await page.keyboard.press("Control+K");
    ...
    await expect(
      page.locator("text=No se encontraron resultados, text=Sin resultados")
    ).toBeVisible({ timeout: 3000 });
  });
});
...
test.describe("Confirmaciones en Acciones Destructivas", () => {
  ...
  test("Confirmación antes de eliminar minuta del comité", async ({
    mockedAuthPage: page,
  }) => {
    // Navegar a minutas
    ...
    const deleteButton = page
      .locator('button[aria-label*="Eliminar"], button:has(svg.lucide-trash)')
      .first();
    ...
    if (await deleteButton.isVisible()) {
      ...
      await expect(
        page.locator(
          '[role="alertdialog"], [role="dialog"]:has-text("Eliminar")'
        )
      ).toBeVisible({ timeout: 2000 });

      // Verificar mensaje de impacto
      await expect(
        page.locator("text=no se puede deshacer, text=permanentemente")
      ).toBeVisible();
  ...
  test("Confirmación antes de eliminar departamento", async ({
    mockedAuthPage: page,
  }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      await expect(
        page.locator('[role="alertdialog"]:has-text("Eliminar departamento")')
      ).toBeVisible({ timeout: 2000 });

      // Verificar mensaje de impacto (empleados afectados)
      await expect(
        page.locator("text=empleados, text=afectados")
      ).toBeVisible();
    }
  });

  test("Confirmación antes de eliminar evaluación", async ({
    mockedAuthPage: page,
  }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      // Verificar confirmación
      await expect(
        page.locator('[role="alertdialog"]:has-text("Eliminar evaluación")')
      ).toBeVisible({ timeout: 2000 });
  ...
  test("Confirmación antes de eliminar solicitud de gasto", async ({
    mockedAuthPage: page,
  }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      // Verificar confirmación
      await expect(
        page.locator('[role="alertdialog"]:has-text("Eliminar solicitud")')
      ).toBeVisible({ timeout: 2000 });
  ...
  test("Confirmación antes de eliminar certificado digital", async ({
    mockedAuthPage: page,
  }) => {
    ...
    if (await deleteButton.isVisible()) {
      ...
      // Verificar confirmación
      await expect(
        page.locator('[role="alertdialog"]:has-text("Eliminar certificado")')
      ).toBeVisible({ timeout: 2000 });

      // Verificar mensaje sobre llave privada
      await expect(
        page.locator("text=llave privada, text=validación")
      ).toBeVisible();
    }
  });

  test("Confirmar eliminación ejecuta la acción", async ({
    mockedAuthPage: page,
  }) => {
    // Navegar a minutas
    await page.click("text=Prevención de Riesgos");
    ...
    if ((await deleteButton.isVisible()) && initialCount > 0) {
      ...
      await expect(page.locator("text=eliminad, text=éxito")).toBeVisible({
        timeout: 5000,
      });
  ...
  test("Componente ConfirmDialog es reutilizable", async ({
    mockedAuthPage: page,
  }) => {
    // Verificar que el mismo componente se usa en múltiples páginas
    const pages = [
      {
        menu: "Prevención de Riesgos",
        submenu: "Minutas del Comité",
        url: "committee-minutes",
      },
      {
        menu: "Gestión de Talento",
        submenu: "Departamentos",
        url: "departments",
      },
      {
        menu: "Evaluación de Desempeño",
        submenu: "Gestión de Evaluaciones",
        url: "assessments",
      },
    ];
...
test.describe("Accesibilidad de Confirmaciones", () => {
  ...
  test("Dialog de confirmación tiene roles ARIA correctos", async ({
    mockedAuthPage: page,
  }) => {
    await page.click("text=Prevención de Riesgos");
    await page.click("text=Minutas del Comité");
  ...
  test("Focus trap funciona en dialog de confirmación", async ({
    mockedAuthPage: page,
  }) => {
    await page.click("text=Prevención de Riesgos");
    await page.click("text=Minutas del Comité");
  ...
  test("Escape cierra el dialog de confirmación", async ({
    mockedAuthPage: page,
  }) => {
    await page.click("text=Prevención de Riesgos");
    await page.click("text=Minutas del Comité");
