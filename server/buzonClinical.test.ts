...
describe("Buzón de Comunicación Interna", () => {
  it("genera folio con formato correcto", () => {
    ...
    expect(generateFolio("solicitud_capacitacion", 7)).toMatch(
      /^DNC-\d{4}-00007$/
    );
  ...
  it("verifica que una solicitud anónima no expone datos del empleado", () => {
    ...
    const namedReq = sanitizeRequest({
      isAnonymous: false,
      employeeId: 42,
      employeeName: "Juan Pérez",
    });
...
describe("Expediente Clínico Psicométrico", () => {
  ...
  it("verifica que las notas de sesión tienen los campos requeridos", () => {
    ...
    expect(
      validateNote({ content: "Sin fecha", sessionType: "seguimiento" })
    ).toBe(false);
    expect(
      validateNote({ sessionDate: new Date(), sessionType: "seguimiento" })
    ).toBe(false);
  });
});
...
describe("Segmentación demográfica NOM-035", () => {
  it("clasifica correctamente los rangos de edad según NOM-035", () => {
    ...
    const now = new Date();
    const age25 = new Date(
      now.getFullYear() - 25,
      now.getMonth(),
      now.getDate()
    );
    const age35 = new Date(
      now.getFullYear() - 35,
      now.getMonth(),
      now.getDate()
    );
    const age45 = new Date(
      now.getFullYear() - 45,
      now.getMonth(),
      now.getDate()
    );
  ...
  it("agrupa tokens por género correctamente", () => {
    ...
    const grouped = tokens.reduce(
      (acc: Record<string, { total: number; completed: number }>, t) => {
