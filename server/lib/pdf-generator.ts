...
async function addSignatures(
  ...
  const spacing =
    (pageWidth - firmas.length * signatureWidth) / (firmas.length + 1);
...
export async function generateActaConstitutivaPDF(data: {
  ...
  doc.setFontSize(12);
  doc.text(
    "Comité de Atención a Factores de Riesgo Psicosocial",
    doc.internal.pageSize.getWidth() / 2,
    yPos,
    {
      align: "center",
...
export async function generateFuncionesComitePDF(data: {
  ...
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text("FUNCIONES DEL COMITÉ", doc.internal.pageSize.getWidth() / 2, yPos, {
    align: "center",
  });
  yPos += 10;

  doc.setFontSize(12);
  doc.text("NOM-035-STPS-2018", doc.internal.pageSize.getWidth() / 2, yPos, {
    align: "center",
  });
  yPos += 15;
...
export async function generateAceptacionCargoPDF(data: {
  ...
  doc.setFontSize(16);
  doc.setFont("helvetica", "bold");
  doc.text("ACEPTACIÓN DE CARGO", doc.internal.pageSize.getWidth() / 2, yPos, {
    align: "center",
  });
  yPos += 10;
