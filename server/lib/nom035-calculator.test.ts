...
describe('NOM-035 Calculator', () => {
  describe('calculateAnswerScore', () => {
    it('should calculate normal scoring correctly', () => {
      expect(calculator.calculateAnswerScore('Siempre', false, 'guia_ii')).toBe(0);
      expect(calculator.calculateAnswerScore('Casi siempre', false, 'guia_ii')).toBe(1);
      expect(calculator.calculateAnswerScore('Algunas veces', false, 'guia_ii')).toBe(2);
      expect(calculator.calculateAnswerScore('Casi nunca', false, 'guia_ii')).toBe(3);
      expect(calculator.calculateAnswerScore('Nunca', false, 'guia_ii')).toBe(4);
    });

    it('should calculate inverse scoring correctly', () => {
      expect(calculator.calculateAnswerScore('Siempre', true, 'guia_ii')).toBe(4);
      expect(calculator.calculateAnswerScore('Casi siempre', true, 'guia_ii')).toBe(3);
      expect(calculator.calculateAnswerScore('Algunas veces', true, 'guia_ii')).toBe(2);
      expect(calculator.calculateAnswerScore('Casi nunca', true, 'guia_ii')).toBe(1);
  ...
  describe('calculateCategoryScore', () => {
    it('should calculate category score correctly', () => {
      const answers = [
        { questionId: 1, answer: 'Siempre', isReverseScored: true, category: 'Ambiente de trabajo' },
        { questionId: 2, answer: 'Nunca', isReverseScored: false, category: 'Ambiente de trabajo' },
        { questionId: 3, answer: 'Casi siempre', isReverseScored: true, category: 'Factores propios de la actividad' },
      ];
      
      const result = calculator.calculateCategoryScore(answers, 'Ambiente de trabajo', 'guia_ii');
