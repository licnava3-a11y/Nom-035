...
export async function generateNom035MatrixPdf(
  ...
  return new Promise<Buffer>((resolve, reject) => {
    ...
    doc.fillColor("white")
      .fontSize(22)
      .font("Helvetica-Bold")
      .text("SECRETARÍA DEL TRABAJO Y PREVISIÓN SOCIAL", 50, 60, { width: pageWidth, align: "center" });
    ...
    doc.moveDown(0.5)
      .fontSize(14)
      .text("Factores de Riesgo Psicosocial en el Trabajo", { align: "center" });

    doc.fillColor(COLOR_PRIMARY)
      .fontSize(20)
      .font("Helvetica-Bold")
      .text("REPORTE DE SEGUIMIENTO", 50, 220, { width: pageWidth, align: "center" });
    ...
    const dataRows = [
      ...
      ["Nivel de aplicación:", NIVEL_LABELS[plan.nivelAplicacion] || plan.nivelAplicacion],
    ...
    for (let i = 0; i < dataRows.length; i++) {
      doc.fillColor(COLOR_GRAY).text(dataRows[i][0], col1X, rowY, { width: 100, continued: false });
      doc.fillColor("#111827").text(dataRows[i][1], col1X + 105, rowY, { width: pageWidth / 2 - 120 });
      doc.fillColor(COLOR_GRAY).text(dataRows2[i][0], col2X, rowY, { width: 100, continued: false });
      doc.fillColor("#111827").text(dataRows2[i][1], col2X + 105, rowY, { width: pageWidth / 2 - 120 });
      rowY += 18;
    }
    ...
    doc.text(`Generado el ${formatDate(now)} a las ${now.toLocaleTimeString("es-MX")}`, { align: "center" });
    ...
    doc.moveDown();
    doc.strokeColor("#d1d5db").lineWidth(1).moveTo(50, doc.y).lineTo(50 + pageWidth, doc.y).stroke();
    doc.moveDown(0.5);
    ...
    const pctEvidencia = total > 0 ? Math.round((conEvidencia / total) * 100) : 0;
    ...
    const cards = [
      ...
      { label: "% Cumplimiento", value: `${pctCumplimiento}%`, color: pctCumplimiento >= 80 ? COLOR_SUCCESS : pctCumplimiento >= 50 ? COLOR_WARNING : COLOR_DANGER },
      { label: "En Proceso", value: enProceso.toString(), color: COLOR_WARNING },
      { label: "Vencidas", value: vencidas.toString(), color: COLOR_DANGER },
      { label: "Con Evidencia", value: `${conEvidencia} (${pctEvidencia}%)`, color: COLOR_ACCENT },
    ];
    ...
    for (let i = 0; i < cards.length; i++) {
      ...
      doc.fillColor(cards[i].color).fontSize(22).font("Helvetica-Bold")
    ...
    if (pctCumplimiento > 0) {
      doc.rect(50, barY, barW * (pctCumplimiento / 100), 16)
        .fill(pctCumplimiento >= 80 ? COLOR_SUCCESS : pctCumplimiento >= 50 ? COLOR_WARNING : COLOR_DANGER);
    }
    doc.fillColor("white").fontSize(9).font("Helvetica-Bold")
      .text(`${pctCumplimiento}% de cumplimiento`, 50, barY + 3, { width: barW, align: "center" });
    ...
    for (const [tipo, tipoActions] of Object.entries(byTipo)) {
      ...
      const headers = ["ID", "Objetivo", "Estado", "Prioridad", "Responsable", "Plazo"];
      let tableX = 50;
      const headerY = doc.y;
      ...
      for (let i = 0; i < headers.length; i++) {
        doc.text(headers[i], tableX + 3, headerY + 4, { width: colWidths[i] - 6 });
        tableX += colWidths[i];
      }
      ...
      for (const action of tipoActions) {
        ...
        tableX = 50;
        doc.fillColor(COLOR_PRIMARY).fontSize(7).font("Helvetica-Bold")
          .text(action.accionId, tableX + 3, rowY2 + 6, { width: colWidths[0] - 6 });
        tableX += colWidths[0];

        doc.fillColor("#111827").font("Helvetica")
          .text(truncate(action.objetivo, 60), tableX + 3, rowY2 + 6, { width: colWidths[1] - 6 });
        tableX += colWidths[1];

        doc.fillColor(estadoColor)
          .text(ESTADO_LABELS[action.estado] || action.estado, tableX + 3, rowY2 + 6, { width: colWidths[2] - 6 });
        tableX += colWidths[2];

        doc.fillColor("#111827")
          .text(action.prioridad.charAt(0).toUpperCase() + action.prioridad.slice(1), tableX + 3, rowY2 + 6, { width: colWidths[3] - 6 });
        tableX += colWidths[3];

        doc.text(truncate(action.responsable || "—", 20), tableX + 3, rowY2 + 6, { width: colWidths[4] - 6 });
        tableX += colWidths[4];

        doc.text(formatDate(action.plazo), tableX + 3, rowY2 + 6, { width: colWidths[5] - 6 });

        // Línea separadora
        doc.strokeColor("#e5e7eb").lineWidth(0.5)
          .moveTo(50, rowY2 + rowH).lineTo(50 + pageWidth, rowY2 + rowH).stroke();
    ...
    const generateActionDetail = async () => {
      for (const action of actions) {
        ...
        doc.fillColor("white")
          .fontSize(10)
          .font("Helvetica-Bold")
          .text(`${action.accionId} — ${truncate(action.objetivo, 80)}`, 58, accionY + 6, { width: pageWidth - 16 });

        doc.moveDown(0.2);

        // Datos de la acción
        const detailY = doc.y;
        doc.rect(50, detailY, pageWidth, 80).fillAndStroke(COLOR_LIGHT, "#e5e7eb");
        ...
        doc.text("Acción:", col1, dy, { continued: true }).fillColor("#111827").font("Helvetica-Bold")
          .text(` ${truncate(action.accion, 80)}`, { width: pageWidth / 2 - 20 });
        doc.fillColor(COLOR_GRAY).font("Helvetica")
          .text("Indicador:", col2, dy, { continued: true }).fillColor("#111827").font("Helvetica-Bold")
          .text(` ${truncate(action.indicador || "—", 60)}`, { width: pageWidth / 2 - 20 });

        dy += 16;
        doc.fillColor(COLOR_GRAY).font("Helvetica")
          .text("Responsable:", col1, dy, { continued: true }).fillColor("#111827").font("Helvetica-Bold")
          .text(` ${action.responsable || "Sin asignar"}`, { width: pageWidth / 2 - 20 });
        doc.fillColor(COLOR_GRAY).font("Helvetica")
          .text("Plazo:", col2, dy, { continued: true }).fillColor("#111827").font("Helvetica-Bold")
          .text(` ${formatDate(action.plazo)}`, { width: pageWidth / 2 - 20 });

        dy += 16;
        const estadoColor2 = {
          cumplida: COLOR_SUCCESS, en_proceso: COLOR_ACCENT,
          vencida: COLOR_DANGER, no_iniciada: COLOR_GRAY, cancelada: COLOR_GRAY,
        }[action.estado] || COLOR_GRAY;
        doc.fillColor(COLOR_GRAY).font("Helvetica")
          .text("Estado:", col1, dy, { continued: true }).fillColor(estadoColor2).font("Helvetica-Bold")
          .text(` ${ESTADO_LABELS[action.estado] || action.estado}`, { width: pageWidth / 2 - 20 });
        doc.fillColor(COLOR_GRAY).font("Helvetica")
          .text("Prioridad:", col2, dy, { continued: true }).fillColor("#111827").font("Helvetica-Bold")
          .text(` ${action.prioridad.charAt(0).toUpperCase() + action.prioridad.slice(1)}`, { width: pageWidth / 2 - 20 });

        dy += 16;
        if (action.observaciones) {
          doc.fillColor(COLOR_GRAY).font("Helvetica")
            .text("Observaciones:", col1, dy, { continued: true }).fillColor("#111827").font("Helvetica")
            .text(` ${truncate(action.observaciones, 120)}`, { width: pageWidth - 20 });
        }
        ...
        if (action.evidencias.length === 0) {
          doc.fillColor(COLOR_DANGER).fontSize(8).font("Helvetica-Bold")
            .text("⚠ Sin evidencias registradas", { indent: 10 });
          doc.moveDown(0.5);
        } else {
          doc.fillColor(COLOR_SUCCESS).fontSize(8).font("Helvetica-Bold")
            .text(`✓ ${action.evidencias.length} evidencia(s) registrada(s):`, { indent: 10 });
          doc.moveDown(0.2);
          ...
          if (options.includeEvidenceThumbnails !== false) {
            ...
            if (imageEvidences.length > 0) {
              ...
              for (let i = 0; i < Math.min(imageEvidences.length, 6); i++) {
                ...
                try {
                  const imgBuffer = await fetchImageBuffer(imageEvidences[i].fileUrl);
                  if (imgBuffer) {
                    doc.rect(thumbX - 1, thumbY - 1, thumbW + 2, thumbH + 2).stroke("#d1d5db");
                    doc.image(imgBuffer, thumbX, thumbY, { width: thumbW, height: thumbH, fit: [thumbW, thumbH] });
                    doc.fillColor(COLOR_GRAY).fontSize(6)
                      .text(truncate(imageEvidences[i].nombreArchivo, 20), thumbX, thumbY + thumbH + 2, { width: thumbW });
                  }
                } catch {
                  // Si no se puede cargar la imagen, mostrar placeholder
                  doc.rect(thumbX, thumbY, thumbW, thumbH).fillAndStroke("#f3f4f6", "#d1d5db");
                  doc.fillColor(COLOR_GRAY).fontSize(7)
                    .text("Imagen no disponible", thumbX, thumbY + thumbH / 2 - 5, { width: thumbW, align: "center" });
                }
          ...
          const nonImageEvidences = action.evidencias.filter(e => !e.tipoArchivo.startsWith("image/"));
          for (const ev of nonImageEvidences) {
            ...
            doc.fillColor(COLOR_GRAY)
              .text(`  [${TIPO_EVIDENCIA_LABELS[ev.tipoEvidencia] || ev.tipoEvidencia}]  ${formatDate(ev.fechaSubida)}`);
            if (ev.descripcion) {
              doc.fillColor(COLOR_GRAY).fontSize(7)
    ...
    generateActionDetail().then(() => {
      ...
      doc.fillColor(COLOR_GRAY).fontSize(8).font("Helvetica")
        .text("RESPONSABLE NOM-035", 55, sigY + 8, { width: sigW - 10, align: "center" });

      if (plan.firmaNombreResponsable) {
        doc.fillColor(COLOR_PRIMARY).fontSize(10).font("Helvetica-Bold")
          .text(plan.firmaNombreResponsable, 55, sigY + 30, { width: sigW - 10, align: "center" });
        doc.fillColor(COLOR_GRAY).fontSize(8).font("Helvetica")
          .text(plan.firmaCargoResponsable || "Responsable", 55, sigY + 48, { width: sigW - 10, align: "center" });
      } else {
        // Línea de firma en blanco
        doc.strokeColor("#9ca3af").lineWidth(1)
          .moveTo(70, sigY + 60).lineTo(50 + sigW - 20, sigY + 60).stroke();
        doc.fillColor(COLOR_GRAY).fontSize(8)
          .text("Nombre y firma", 55, sigY + 65, { width: sigW - 10, align: "center" });
        doc.text("Cargo / Puesto", 55, sigY + 78, { width: sigW - 10, align: "center" });
      }

      doc.fillColor(COLOR_GRAY).fontSize(7)
        .text(`Fecha: ${plan.firmaFecha ? formatDate(plan.firmaFecha) : "_______________"}`, 55, sigY + 88, { width: sigW - 10, align: "center" });

      // Firma 2: Representante Legal / Dirección
      const sig2X = 50 + sigW + 40;
      doc.rect(sig2X, sigY, sigW, 100).fillAndStroke(COLOR_LIGHT, "#d1d5db");
      doc.fillColor(COLOR_GRAY).fontSize(8).font("Helvetica")
        .text("REPRESENTANTE LEGAL / DIRECCIÓN GENERAL", sig2X + 5, sigY + 8, { width: sigW - 10, align: "center" });

      if (plan.firmaNombreRepLegal) {
        doc.fillColor(COLOR_PRIMARY).fontSize(10).font("Helvetica-Bold")
          .text(plan.firmaNombreRepLegal, sig2X + 5, sigY + 30, { width: sigW - 10, align: "center" });
        doc.fillColor(COLOR_GRAY).fontSize(8).font("Helvetica")
          .text(plan.firmaCargoRepLegal || "Representante Legal", sig2X + 5, sigY + 48, { width: sigW - 10, align: "center" });
      } else {
        doc.strokeColor("#9ca3af").lineWidth(1)
          .moveTo(sig2X + 20, sigY + 60).lineTo(sig2X + sigW - 20, sigY + 60).stroke();
        doc.fillColor(COLOR_GRAY).fontSize(8)
          .text("Nombre y firma", sig2X + 5, sigY + 65, { width: sigW - 10, align: "center" });
        doc.text("Cargo / Puesto", sig2X + 5, sigY + 78, { width: sigW - 10, align: "center" });
      }

      doc.fillColor(COLOR_GRAY).fontSize(7)
        .text(`Fecha: ${plan.firmaFecha ? formatDate(plan.firmaFecha) : "_______________"}`, sig2X + 5, sigY + 88, { width: sigW - 10, align: "center" });
      ...
      doc.strokeColor("#d1d5db").lineWidth(1)
        .moveTo(50, footerY).lineTo(50 + pageWidth, footerY).stroke();
