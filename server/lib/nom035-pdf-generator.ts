...
function addRiskDistributionTable(
  ...
  const tableData = [
    ...
    {
      nivel: "Muy Alto",
      cantidad: distribution.muyAlto,
      color: COLORS.muyAlto,
    },
  ];
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor("#000000")
    .text("Distribución de Riesgo:", 50, yPosition);
  yPosition += 20;
  ...
  tableData.forEach((row: any) => {
    doc
      .fillColor(row.color)
      .circle(60, yPosition + 5, 5)
      .fill();
    doc
      .fillColor("#000000")
      .text(
        `${row.nivel}: ${row.cantidad} respuestas (${((row.cantidad / (distribution.nulo + distribution.bajo + distribution.medio + distribution.alto + distribution.muyAlto)) * 100).toFixed(1)}%)`,
        75,
        yPosition
      );
    yPosition += 20;
...
function addHeader(
  ...
  doc
    ...
    .text(
      `Fecha: ${reportDate.toLocaleDateString("es-MX")}`,
      doc.page.width - 200,
      55
    );
 ...
 */
async function addSurveyResultsSection(
  doc: PDFKit.PDFDocument,
  surveyResults: SurveyResult[]
) {
  doc
 ...
 */
async function addMultilevelAnalysisSection(
  doc: PDFKit.PDFDocument,
  multilevelAnalysis: MultilevelAnalysis[]
) {
  doc.addPage();
  ...
  for (const analysis of multilevelAnalysis) {
    ...
    if (analysis.segments.length > 0) {
      ...
      analysis.segments.forEach((segment: any) => {
        const riesgoAlto =
          segment.riskDistribution.alto + segment.riskDistribution.muyAlto;

        doc
          .text(segment.name.substring(0, 25), col1, doc.y)
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `Este reporte consolida los resultados de las encuestas NOM-035 STPS 2018 aplicadas en ${data.companyName}.`,
          50,
          doc.y,
          {
      ...
      const totalResponses = data.surveyResults.reduce(
        (sum: any, r: any) => sum + r.totalResponses,
        0
      );
      doc.text(`Total de respuestas recopiladas: ${totalResponses}`, 50, doc.y);
      doc.text(
        `Encuestas aplicadas: ${data.surveyResults.map(r => r.surveyTitle).join(", ")}`,
        50,
        doc.y
      );
