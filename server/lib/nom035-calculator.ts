...
const GUIDE_II_SCORING = {
  ...
  inverse: { 'Siempre': 4, 'Casi siempre': 3, 'Algunas veces': 2, 'Casi nunca': 1, 'Nunca': 0 },
};
...
const GUIDE_II_FINAL_RANGES = {
  nulo: { min: 0, max: 19, color: 'blue' as RiskColor, label: 'Nulo o despreciable' },
  ...
  muy_alto: { min: 90, max: Infinity, color: 'red' as RiskColor, label: 'Muy alto' },
};
...
const GUIDE_III_FINAL_RANGES = {
  nulo: { min: 0, max: 49, color: 'blue' as RiskColor, label: 'Nulo o despreciable' },
  ...
  muy_alto: { min: 140, max: Infinity, color: 'red' as RiskColor, label: 'Muy alto' },
};
...
const GUIDE_II_ACTIONS = {
  ...
  bajo: ['Es necesario una mayor difusión de la política de prevención de riesgos psicosociales y revisar que la misma se esté aplicando.'],
  medio: [
...
export function calculateAnswerScore(
  ...
  const scoringTable = guideType === 'guia_ii' ? GUIDE_II_SCORING : GUIDE_III_SCORING;
...
export function determineRiskLevel(
  ...
  const ranges = guideType === 'guia_ii' ? GUIDE_II_FINAL_RANGES : GUIDE_III_FINAL_RANGES;
...
export function getRecommendedActions(
  ...
  const actions = guideType === 'guia_ii' ? GUIDE_II_ACTIONS : GUIDE_III_ACTIONS;
...
export function calculateFinalScore(
  answers: Array<{ questionId: number; answer: string; isReverseScored: boolean }>,
  guideType: 'guia_ii' | 'guia_iii'
): number {
  let totalScore = 0;
...
export function calculateCategoryScore(
  answers: Array<{ questionId: number; answer: string; isReverseScored: boolean; category: string }>,
  category: string,
  guideType: 'guia_ii' | 'guia_iii'
): CategoryScore {
...
export function calculateDomainScore(
  answers: Array<{ questionId: number; answer: string; isReverseScored: boolean; domain: string }>,
  domain: string,
  guideType: 'guia_ii' | 'guia_iii'
): DomainScore {
...
export function calculateDimensionScore(
  answers: Array<{ questionId: number; answer: string; isReverseScored: boolean; dimension: string | null }>,
  dimension: string,
  guideType: 'guia_ii' | 'guia_iii'
): DimensionScore {
...
export function calculateSurveyResult(
  ...
  const dimensions = Array.from(new Set(answers.map(a => a.dimension).filter(d => d !== null))) as string[];
