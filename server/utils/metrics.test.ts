...
export function calculateCaseDistribution(
  cases: Case[]
...
export function calculateCaseDistributionPercentage(
  cases: Case[]
): Record<string, number> {
...
export function calculateCasesByMonth(
  cases: Case[],
  year: number
): Record<string, number> {
...
export function identifyHighRiskCases(
  cases: Case[],
  now: Date = new Date()
): Case[] {
...
export function calculateWorkloadByType(
  cases: Case[]
): Record<string, { open: number; total: number; percentage: number }> {
  const workload: Record<
    string,
    { open: number; total: number; percentage: number }
  > = {};
  ...
  Object.keys(workload).forEach(type => {
    ...
    workload[type].percentage =
      total > 0 ? Math.round((open / total) * 100 * 10) / 10 : 0;
  });
  return workload;
}
