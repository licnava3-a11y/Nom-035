...
function addFooter(
  ...
  // Número de página
  doc.text(`Página ${pageNumber} de ${totalPages}`, 105, pageHeight - 10, {
    align: "center",
  });
}
 ...
 */
async function addSignaturesSection(
  doc: jsPDF,
  signatures: Signature[],
  startY: number
): Promise<number> {
  ...
  for (const signature of signatures) {
    ...
    if (signature.signatureHash) {
      doc.setFontSize(8);
      doc.text(
        `Hash: ${signature.signatureHash.substring(0, 32)}...`,
        20,
        currentY + 45
      );
    }
 ...
 */
export async function generateActaRecorridoPDF(
  data: ActaRecorridoData
): Promise<Buffer> {
  const doc = new jsPDF({
  ...
  // Área inspeccionada
  doc.text(
    `Área Inspeccionada: ${data.content.areaInspeccionada}`,
    20,
    currentY
  );
  currentY += 10;
  ...
  currentY += 6;
  doc.setFont("helvetica", "normal");
  const recomendacionesLines = doc.splitTextToSize(
 ...
 */
export async function generateActaFinalResultadosPDF(
  data: ActaFinalResultadosData
): Promise<Buffer> {
  const doc = new jsPDF({
    orientation: "portrait",
    unit: "mm",
