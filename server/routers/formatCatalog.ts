...
export const formatCatalogRouter = router({
  ...
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const rows = await db.select().from(formatCatalog)
  ...
  getActive: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [active] = await db.select().from(formatCatalog)
        .where(and(
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (fields.versionDate !== undefined) updateData.versionDate = fields.versionDate;
      if (fields.reference !== undefined) updateData.reference = fields.reference;
      if (fields.changeNotes !== undefined) updateData.changeNotes = fields.changeNotes;

      if (Object.keys(updateData).length === 0) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "No hay campos para actualizar" });
      }

      await db.update(formatCatalog).set(updateData).where(eq(formatCatalog.id, id));
      return { success: true };
    }),
  ...
  setActive: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [entry] = await db.select({ code: formatCatalog.code })
        .from(formatCatalog).where(eq(formatCatalog.id, input.id));

      if (!entry) throw new TRPCError({ code: "NOT_FOUND", message: "Versión no encontrada" });
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [entry] = await db.select({ isActive: formatCatalog.isActive })
        .from(formatCatalog).where(eq(formatCatalog.id, input.id));

      if (!entry) throw new TRPCError({ code: "NOT_FOUND", message: "Versión no encontrada" });
      if (entry.isActive) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "No se puede eliminar la versión activa. Active otra versión primero." });
      }
