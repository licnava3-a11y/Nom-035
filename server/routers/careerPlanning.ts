...
export const careerPlanningRouter = router({
    ...
    .input(
      z.object({
          ...
          .array(
            z.object({
              ...
              requiredCompetencies: z.array(
                z.object({
                  ...
                  minimumLevel: z
                    ...
                    .max(5, "El nivel mínimo no puede exceder 5"),
                })
              ),
              estimatedTimeMonths: z
                ...
                .max(120, "El tiempo estimado no puede exceder 120 meses"),
            })
          )
        ...
        minimumExperience: z
          .number()
          .int()
          .nonnegative("La experiencia mínima no puede ser negativa")
          .max(50, "La experiencia mínima no puede exceder 50 años")
          .optional(),
      })
    )
    ...
    .query(async ({ input }) => {
      ...
      const suggestions = paths.map(path => {
        ...
        if (path.minimumEducation && employee.ultimoGradoEstudios) {
          const educationMatch = employee.ultimoGradoEstudios.includes(
        ...
        if (path.minimumExperience) {
          const experienceRatio = Math.min(
            experienceMonths / path.minimumExperience,
            1
          );
        ...
        const currentDepartmentMatch = path.positions.some((pos: any) =>
          pos.positionName
            .toLowerCase()
            .includes(employee.departamento?.toLowerCase() || "")
        );
        ...
        const currentPositionMatch = path.positions.some(
          (pos: any) =>
            pos.positionName.toLowerCase() === employee.puesto?.toLowerCase()
        );
        ...
        return {
          ...
          estimatedDuration: path.positions.reduce(
            (sum: number, pos: any) => sum + pos.estimatedTimeMonths,
            0
          ),
    ...
    .input(
      z.object({
        ...
        currentLevel: z
          .number()
          .int()
          .min(1, "El nivel actual debe ser al menos 1"),
        targetLevel: z
          .number()
          .int()
          .min(1, "El nivel objetivo debe ser al menos 1"),
        competencyGaps: z
          .array(
            z.object({
              competencyId: commonValidators.positiveId,
              ...
              currentLevel: z
                .number()
                .int()
                .min(0, "El nivel actual no puede ser negativo")
                .max(5, "El nivel actual no puede exceder 5"),
              requiredLevel: z
                .number()
                .int()
                .min(1, "El nivel requerido debe ser al menos 1")
                .max(5, "El nivel requerido no puede exceder 5"),
  ...
  getVacancyProjections: protectedProcedure.query(async () => {
    ...
    const projections = turnoverData.map((item: any) => ({
      ...
      estimatedOpeningDate: new Date(Date.now() + 180 * 24 * 60 * 60 * 1000)
        .toISOString()
        .split("T")[0],
    }));
