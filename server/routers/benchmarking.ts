  ...
  getComparison: protectedProcedure
    ...
    .query(async ({ input }: { input: { sectorId: number } }) => {
      ...
      const highRiskCount = await db
        ...
        .from(surveyResults)
        .where(
          sql`${surveyResults.riskLevel} = 'very_high' OR ${surveyResults.riskLevel} = 'high'`
        );
      ...
      const comparisons = [
        {
          ...
          gap: parseFloat(
            (
              casesPerHundred -
              (benchmarkMap["avg_cases_per_100_employees"] || 0)
            ).toFixed(2)
          ),
          status:
            casesPerHundred <=
        ...
        },
        {
          ...
          gap: parseFloat(
            (
              highRiskPercentage -
              (benchmarkMap["avg_high_risk_percentage"] || 0)
            ).toFixed(2)
          ),
          status:
            highRiskPercentage <=
        ...
        },
        {
          ...
          gap: parseFloat(
            (
              avgResolutionDays - (benchmarkMap["avg_resolution_days"] || 0)
            ).toFixed(2)
          ),
          status:
            avgResolutionDays <= (benchmarkMap["avg_resolution_days"] || 0)
        ...
        },
        {
          ...
          gap: parseFloat(
            (
              ...
              (benchmarkMap["avg_training_satisfaction"] || 0)
            ).toFixed(2)
          ),
          status:
            trainingSatisfaction >=
        ...
        },
        {
          ...
          unit: "porcentaje",
          gap: parseFloat(
            (burnoutRate - (benchmarkMap["avg_burnout_rate"] || 0)).toFixed(2)
          ),
          status:
            burnoutRate <= (benchmarkMap["avg_burnout_rate"] || 0)
              ? "better"
              : "worse",
        },
        {
          ...
          unit: "porcentaje",
          gap: parseFloat(
            (turnoverRate - (benchmarkMap["avg_turnover_rate"] || 0)).toFixed(2)
          ),
          status:
            turnoverRate <= (benchmarkMap["avg_turnover_rate"] || 0)
              ? "better"
              : "worse",
        },
      ];
...
Genera 3-5 recomendaciones específicas y accionables para cerrar las brechas identificadas, priorizando las áreas donde la organización está por debajo del estándar sectorial.`;
        ...
        messages: [
          {
            role: "system",
            content:
              "Eres un experto en riesgos psicosociales y cumplimiento NOM-035 STPS.",
          },
                ...
                recommendations: {
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      targetMetric: {
                        type: "string",
                        description: "Métrica objetivo que mejorará",
                      },
                    },
                    required: [
                      "title",
  ...
  getDashboard: protectedProcedure
    ...
    .query(async ({ input }: { input: { sectorId: number } }) => {
      ...
      const sector = await db
        ...
        .where(eq(industrySectors.id, input.sectorId))
        .limit(1);
      ...
      // Calcular resumen
      const betterCount: number = comparison.comparisons.filter(
        (c: any) => c.status === "better"
      ).length;
      const worseCount: number = comparison.comparisons.filter(
        (c: any) => c.status === "worse"
      ).length;
      ...
      return {
        ...
        performanceScore: parseFloat(
          ((betterCount / totalMetrics) * 100).toFixed(2)
        ),
  ...
  generatePDF: protectedProcedure
    .input(z.object({ sectorId: z.number() }))
    .mutation(
      async ({ input, ctx }: { input: { sectorId: number }; ctx: any }) => {
        const db = await getDb();
        if (!db) throw new Error("Database connection failed");
        ...
        const casesPerEmployee =
          employeeCount > 0 ? (caseCount / employeeCount) * 100 : 0;

        const highRiskCases = await db
          .select({ count: sql<number>`COUNT(*)` })
          .from(surveyResults)
          .where(sql`${surveyResults.riskLevel} IN ('high', 'very_high')`);
        const highRiskCount = Number(highRiskCases[0]?.count || 0);
        const highRiskPercentage =
          employeeCount > 0 ? (highRiskCount / employeeCount) * 100 : 0;
        ...
        const satisfactionScore = parseFloat(
          (avgSatisfaction[0]?.avg || 0).toFixed(2)
        );
        ...
        doc
          .fontSize(24)
          .fillColor("#1e40af")
          .text("Análisis de Benchmarking Sectorial", { align: "center" });
        doc.moveDown(0.5);
        doc
          .fontSize(16)
          .fillColor("#374151")
          .text(`Sector: ${sectorName}`, { align: "center" });
        doc.moveDown(0.3);
        doc
          .fontSize(12)
          .fillColor("#6b7280")
        ...
        doc
          .fontSize(18)
          .fillColor("#1e40af")
          .text("Comparación Detallada de Métricas", { underline: true });
        doc.moveDown(0.5);
        ...
        headers.forEach((header: any, i: number) => {
          ...
          doc
            .fillColor("#ffffff")
            .text(header, xPos + 5, tableTop + 5, { width: colWidths[i] - 10 });
          xPos += colWidths[i];
        });
        ...
        doc
          .fontSize(8)
          .fillColor("#9ca3af")
          .text(`Folio: ${folio}`, 50, doc.page.height - 50, {
            align: "center",
          });
