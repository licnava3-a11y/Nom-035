...
function normalizeRow(
  ...
  // Normalizar género
  const sexoRaw = String(normalized.sexo ?? "")
    .toLowerCase()
    .trim();
  ...
  if (normalized.rfc)
    normalized.rfc = String(normalized.rfc).toUpperCase().trim();
  if (normalized.curp)
    normalized.curp = String(normalized.curp).toUpperCase().trim();
  if (normalized.nss) normalized.nss = String(normalized.nss).trim();
  if (normalized.employeeNumber)
    normalized.employeeNumber = String(normalized.employeeNumber).trim();
...
function detectUnmappedColumns(rows: Record<string, any>[]): string[] {
  ...
  rows.forEach(r =>
    Object.keys(r)
      ...
      .forEach(k => allKeys.add(k.replace("_raw_", "")))
  );
...
export const hrIntegrationRouter = router({
  ...
  previewImport: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const rawData: Record<string, any>[] = xlsx.utils.sheet_to_json(
        worksheet,
        { defval: "" }
      );
  ...
  confirmImport: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const deptMap = new Map(
        allDepartments.map((d: any) => [d.name.toLowerCase().trim(), d.id])
      );
      const posMap = new Map(
        allPositions.map((p: any) => [p.title.toLowerCase().trim(), p.id])
      );
      ...
      for (let i = 0; i < rawData.length; i++) {
        ...
        try {
          ...
          if (!firstName) {
            throw new Error(
              "Nombre vacío — no se puede crear el empleado sin nombre."
            );
          }
          if (!email) {
          ...
          const deptRaw = String(row.departamento ?? row.department ?? "")
            .toLowerCase()
            .trim();
          const posRaw = String(row.puesto ?? row.position ?? "")
            .toLowerCase()
            .trim();
          ...
          const edRaw = String(
            row.educationLevel ?? row.nivelEducativo ?? ""
          ).toLowerCase();
  ...
  exportForContpaqi: protectedProcedure.mutation(async () => {
    ...
    const { employees, departments, positions } = await import(
      "../../drizzle/schema"
    );
