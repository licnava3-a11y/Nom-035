...
export const salaryHistoryRouter = router({
  ...
  add: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      if (
        ctx.user.role !== "admin" &&
        ctx.user.role !== "rh" &&
        ctx.user.role !== "recursos_humanos"
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
            "Solo RH/Administradores pueden registrar cambios salariales",
        });
      }
      const db = await getDb();
      if (!db)
      ...
      if (input.previousSalary && input.previousSalary > 0) {
        const pct =
          ((input.newSalary - input.previousSalary) / input.previousSalary) *
          100;
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Solo administradores pueden eliminar registros",
        });
      }
      const db = await getDb();
      if (!db)
  ...
  equityStats: protectedProcedure.query(async () => {
    ...
    return {
      ...
      byPosition: Object.values(byPosition).sort(
