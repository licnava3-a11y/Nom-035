...
export const dc3RemoteSignRouter = router({
  ...
  createToken: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      }).from(dc3Records).where(eq(dc3Records.id, input.dc3RecordId));

      if (!record) throw new TRPCError({ code: "NOT_FOUND", message: "Registro DC-3 no encontrado" });
      if (record.status === "cancelled") {
        throw new TRPCError({ code: "BAD_REQUEST", message: "No se puede solicitar firma en una constancia cancelada" });
      }
  ...
  getToken: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [tokenRow] = await db.select().from(dc3RemoteSignTokens)
        .where(eq(dc3RemoteSignTokens.token, input.token));

      if (!tokenRow) return { status: "not_found" as const };
      if (tokenRow.usedAt) return { status: "used" as const, usedAt: tokenRow.usedAt };
      if (new Date() > tokenRow.expiresAt) return { status: "expired" as const, expiresAt: tokenRow.expiresAt };
      ...
      const [record] = await db.select({
        ...
        status: dc3Records.status,
      }).from(dc3Records).where(eq(dc3Records.id, tokenRow.dc3RecordId));
  ...
  submitSignature: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!tokenRow) {
        throw new TRPCError({ code: "NOT_FOUND", message: "Token inválido o expirado" });
      }
      if (tokenRow.usedAt) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Este enlace de firma ya fue utilizado" });
      }
      ...
      const { url: signatureUrl } = await storagePut(fileKey, signatureBuffer, "image/png");
      ...
      await db.update(dc3Records).set(updateData as any).where(eq(dc3Records.id, tokenRow.dc3RecordId));

      // Marcar el token como usado
      await db.update(dc3RemoteSignTokens)
        .set({ usedAt: new Date() })
  ...
  renewToken: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [oldToken] = await db.select().from(dc3RemoteSignTokens)
        .where(eq(dc3RemoteSignTokens.id, input.tokenId));

      if (!oldToken) throw new TRPCError({ code: "NOT_FOUND", message: "Token no encontrado" });
      ...
      if (!isExpired && !isUsed) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "El token aún es válido. Solo se puede renovar un token expirado o ya utilizado.",
        });
      }
      ...
      const [record] = await db.select({
        ...
        companyName: dc3Records.companyName,
        status: dc3Records.status,
      }).from(dc3Records).where(eq(dc3Records.id, oldToken.dc3RecordId));

      if (!record) throw new TRPCError({ code: "NOT_FOUND", message: "Registro DC-3 no encontrado" });
      if (record.status === "cancelled") {
        throw new TRPCError({ code: "BAD_REQUEST", message: "No se puede renovar firma en una constancia cancelada" });
      }
      ...
      const signerEmail = input.signerEmail ?? oldToken.signerEmail ?? undefined;
