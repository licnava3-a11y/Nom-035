...
export const exitInterviewsRouter = router({
  ...
  initDefaultQuestions: protectedProcedure.mutation(async ({ ctx }) => {
    if (ctx.user.role !== "admin") {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "Solo administradores pueden inicializar preguntas",
      });
    }
    ...
    if (existing.length > 0) {
      return {
        initialized: false,
        message: "Las preguntas ya están inicializadas",
      };
    }
  ...
  registerTermination: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Solo administradores pueden registrar bajas",
        });
      }
      const db = await getDb();
      if (!db)
      ...
      const [termination] = await (db.insert(employeeTerminations) as any)
        .values({
    ...
    .query(async ({ input, ctx }) => {
      ...
      const [rows, [{ total }]] = await Promise.all([
        db
          ...
          .leftJoin(
            employeeTerminations,
            eq(exitInterviews.terminationId, employeeTerminations.id)
          )
          .where(whereClause)
    ...
    .query(async ({ input, ctx }) => {
      ...
      if (ctx.user.role !== "admin") {
        ...
        if (interview.employeeId !== employeeId) {
          throw new TRPCError({
            code: "FORBIDDEN",
            message: "No tienes acceso a esta entrevista",
          });
        }
      }
      ...
      const responses = await db
        ...
        .leftJoin(
          exitInterviewQuestions,
          eq(exitInterviewResponses.questionId, exitInterviewQuestions.id)
        )
  ...
  submitResponses: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (interview.status === "completed")
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "La entrevista ya fue completada",
        });
      if (ctx.user.role !== "admin") {
        const employeeId = await getAuthenticatedEmployeeId(db, ctx.user.id);
        if (interview.employeeId !== employeeId) {
      ...
      await db
        ...
        .where(eq(exitInterviewResponses.exitInterviewId, input.interviewId));
  ...
  getAnalytics: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Solo administradores pueden ver el análisis",
        });
      }
      const db = await getDb();
      if (!db)
      ...
      if (mainReasonQuestion.length > 0) {
        mainReasonDistribution = await db
          ...
          .from(exitInterviewResponses)
          .where(
            eq(exitInterviewResponses.questionId, mainReasonQuestion[0].id)
          )
      ...
      if (recommendQuestion.length > 0) {
        ...
        const positive = recs
          .filter(r => r.response.includes("sí") || r.response.includes("Sí"))
      ...
      const quarterlyTrend = await db
        ...
        .groupBy(
          sql`CONCAT(YEAR(${exitInterviews.completedAt}), '-Q', QUARTER(${exitInterviews.completedAt}))`
        )
        .orderBy(
          sql`CONCAT(YEAR(${exitInterviews.completedAt}), '-Q', QUARTER(${exitInterviews.completedAt}))`
        );
  ...
  createActionPlan: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      if (ctx.user.role !== "admin") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Solo administradores pueden crear planes de acción",
        });
      }
      const db = await getDb();
      if (!db)
      ...
      const [plan] = await (db.insert(turnoverActionPlans) as any)
        .values({
          title: input.title,
  ...
  addQuestion: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [{ maxOrder }] = await db
        .select({
          maxOrder: sql<number>`COALESCE(MAX(${exitInterviewQuestions.order}), 0)`,
        })
        .from(exitInterviewQuestions);
      await (db.insert(exitInterviewQuestions) as any).values({
  ...
  updateQuestion: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const { id, ...data } = input;
      await (db.update(exitInterviewQuestions) as any)
        .set({ ...data, updatedAt: new Date() })
        .where(eq(exitInterviewQuestions.id, id));
      return { success: true };
    }),
  ...
  deleteQuestion: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await (db.update(exitInterviewQuestions) as any)
        .set({ isActive: false })
        .where(eq(exitInterviewQuestions.id, input.id));
      return { success: true };
    }),
  ...
  listActionPlans: protectedProcedure.query(async ({ ctx }) => {
    if (ctx.user.role !== "admin") {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "Solo administradores pueden ver los planes de acción",
      });
    }
    const db = await getDb();
    if (!db)
  ...
  importQuestions: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      for (let i = 0; i < input.questions.length; i++) {
        ...
        const cat = VALID_CATEGORIES.includes(q.category ?? "")
          ? q.category!
          : "Otro";
