...
export const annualTrainingPlanRouter = router({
  ...
  list: protectedProcedure
    .input(
      z.object({
        ...
        status: z
          .enum(["borrador", "aprobado", "en_ejecucion", "cerrado"])
          .optional(),
    ...
    .query(async ({ input }) => {
      ...
      if (departmentId)
        conditions.push(eq(annualTrainingPlans.departmentId, departmentId));
      if (status) conditions.push(eq(annualTrainingPlans.status, status));
      if (search)
        conditions.push(like(annualTrainingPlans.title, `%${search}%`));
      const whereClause =
      ...
      const plans = await db
        ...
        .leftJoin(
          departments,
          eq(annualTrainingPlans.departmentId, departments.id)
        )
        .leftJoin(
          employees,
          eq(annualTrainingPlans.responsibleId, employees.id)
        )
        .where(whereClause)
        .orderBy(
          desc(annualTrainingPlans.year),
          desc(annualTrainingPlans.createdAt)
        )
        .limit(pageSize)
        .offset(offset);
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db
        .update(annualTrainingPlans)
        .set(updateData)
        .where(eq(annualTrainingPlans.id, id));
      return { success: true };
    }),
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db
        .delete(annualTrainingPlanItems)
        .where(eq(annualTrainingPlanItems.planId, input.id));
      await db
        .delete(annualTrainingPlans)
        .where(eq(annualTrainingPlans.id, input.id));
      return { success: true };
    }),
  ...
  addItem: protectedProcedure
    .input(
      z.object({
        ...
        modality: z
          .enum(["presencial", "virtual", "mixta", "e_learning"])
          .default("presencial"),
  ...
  updateItem: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db
        .update(annualTrainingPlanItems)
        .set(updateData)
        .where(eq(annualTrainingPlanItems.id, id));
      return { success: true };
    }),
  ...
  deleteItem: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db
        .delete(annualTrainingPlanItems)
        .where(eq(annualTrainingPlanItems.id, input.id));
      return { success: true };
    }),
  ...
  getStats: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const totalActualCost = items.reduce(
        (s, i) => s + (i.actualCost ?? 0),
        0
      );
      const totalHours = items.reduce((s, i) => s + (i.durationHours ?? 0), 0);
      const totalParticipantsTarget = items.reduce(
        (s, i) => s + (i.participantsTarget ?? 0),
        0
      );
      const totalParticipantsActual = items.reduce(
        (s, i) => s + (i.participantsActual ?? 0),
        0
      );
