...
import {
  committeeTrainingPrograms,
  committeeTrainingSessions,
  committeeTrainingAttendance,
  committeeMembers,
  employees,
  users,
} from "../../drizzle/schema";
  ...
  createProgram: protectedProcedure
    ...
    .mutation(
      async ({
        ...
        const [program] = await (
          db.insert(committeeTrainingPrograms) as any
        ).values({
          title: input.title,
  ...
  listPrograms: protectedProcedure
    ...
    .query(
      async ({
        ...
        const programs = await query.orderBy(
          desc(committeeTrainingPrograms.createdAt)
        );
        return programs;
      }
    ),
  ...
  createSession: protectedProcedure
    ...
    .mutation(
      async ({
        ...
        if (!db) throw new Error("Database connection failed");
        const [session] = await (
          db.insert(committeeTrainingSessions) as any
        ).values({
  ...
  listSessions: protectedProcedure
    ...
    .query(
      async ({
        ...
        input: { programId?: number; startDate?: string; endDate?: string };
      }) => {
        const db = await getDb();
        if (!db) throw new Error("Database connection failed");
        ...
        if (input.programId) {
          conditions.push(
            eq(committeeTrainingSessions.programId, input.programId)
          );
        }
        if (input.startDate) {
          conditions.push(
            gte(
              committeeTrainingSessions.sessionDate,
        ...
        if (input.endDate) {
          conditions.push(
            lte(committeeTrainingSessions.sessionDate, new Date(input.endDate))
          );
        }
        ...
        const sessions = await query.orderBy(
          desc(committeeTrainingSessions.sessionDate)
        );
        return sessions;
      }
    ),
        ...
        const existing = await db
          ...
          .where(
            and(
              ...
              eq(
                committeeTrainingAttendance.committeeMemberId,
  ...
  generateCertificate: protectedProcedure
    ...
    .mutation(
      async ({
        ...
        input: { sessionId: number; committeeMemberId: number };
      }) => {
        const db = await getDb();
        if (!db) throw new Error("Database connection failed");
        ...
        if (!member || !member.employeeId) {
          throw new Error(
            "Miembro del comité no encontrado o sin empleado asociado"
          );
        }
        ...
        try {
          ...
          if (user && user.email) {
            const { sendEmail, getCertificateGeneratedTemplate } = await import(
          ...
          console.error(
            "[Generate Certificate] Error al enviar email:",
            emailError
          );
  ...
  getAttendanceReport: protectedProcedure
    ...
    .query(
      async ({
        ...
        input: { programId?: number; sessionId?: number };
      }) => {
        const db = await getDb();
        if (!db) throw new Error("Database connection failed");
