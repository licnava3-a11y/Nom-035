...
export const documentsRouter = router({
  ...
  saveActaConstitutiva: protectedProcedure
    .input(actaConstitutivaSchema)
    .mutation(async ({ ctx, input }) => {
      ...
      let [catalog] = await db
        .select()
        .from(formatCatalog)
        .where(eq(formatCatalog.code, "AC"))
        .limit(1);
      ...
      const [lastDoc] = await db
        .select()
        .from(documents)
        .where(eq(documents.type, "acta_constitutiva"))
        .orderBy(desc(documents.id))
        .limit(1);

      const consecutivo = lastDoc
        ? parseInt(lastDoc.folio.split("-")[1].split("/")[0]) + 1
        : 1;
  ...
  saveFuncionesComite: protectedProcedure
    .input(funcionesComiteSchema)
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db)
        throw new TRPCError({
      ...
      let [catalog] = await db
        .select()
        .from(formatCatalog)
        .where(eq(formatCatalog.code, "FC"))
        .limit(1);
      ...
      const [lastDoc] = await db
        .select()
        .from(documents)
        .where(eq(documents.type, "funciones_comite"))
        .orderBy(desc(documents.id))
        .limit(1);
  ...
  saveAceptacionCargo: protectedProcedure
    .input(aceptacionCargoSchema)
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db)
        throw new TRPCError({
      ...
      let [catalog] = await db
        .select()
        .from(formatCatalog)
        .where(eq(formatCatalog.code, "ACC"))
        .limit(1);
      ...
      const [lastDoc] = await db
        .select()
        .from(documents)
        .where(eq(documents.type, "aceptacion_cargo"))
        .orderBy(desc(documents.id))
        .limit(1);
  ...
  saveActaRecorrido: protectedProcedure
    .input(actaRecorridoSchema)
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db)
        throw new TRPCError({
      ...
      let [catalog] = await db
        .select()
        .from(formatCatalog)
        .where(eq(formatCatalog.code, "AR"))
        .limit(1);
      ...
      const [lastDoc] = await db
        .select()
        .from(documents)
        .where(eq(documents.type, "acta_recorrido"))
        .orderBy(desc(documents.id))
        .limit(1);
  ...
  saveActaFinalResultados: protectedProcedure
    .input(actaFinalResultadosSchema)
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
      if (!db)
        throw new TRPCError({
      ...
      let [catalog] = await db
        .select()
        .from(formatCatalog)
        .where(eq(formatCatalog.code, "AFR"))
        .limit(1);
      ...
      const [lastDoc] = await db
        .select()
        .from(documents)
        .where(eq(documents.type, "acta_final_resultados"))
        .orderBy(desc(documents.id))
        .limit(1);
  ...
  generateActaRecorridoPDF: protectedProcedure
    ...
    .mutation(async ({ input: documentId }) => {
      ...
      if (doc.type !== "acta_recorrido") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "El documento no es un Acta de Recorrido",
        });
      }
  ...
  generateActaFinalResultadosPDF: protectedProcedure
    ...
    .mutation(async ({ input: documentId }) => {
      ...
      if (doc.type !== "acta_final_resultados") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "El documento no es un Acta Final de Resultados",
        });
      }
      ...
      // Importar generador PDF
      const { generateActaFinalResultadosPDF } = await import(
        "../pdfGenerator"
      );
