    ...
    .input(
      z.object({
        category: z
          ...
          .default("all"),
        status: z
          .enum(["nuevo", "en_proceso", "resuelto", "cerrado", "all"])
          .default("all"),
  ...
  respond: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (msg && msg.senderId && !msg.isAnonymous) {
        ...
        const notifId = Array.isArray(inserted)
          ? inserted[0]?.id
          : (inserted as any)?.id;
        if (notifId) {
  ...
  getNotificationHistory: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [msg] = await db
        .select()
        .from(internalMessages)
        .where(eq(internalMessages.id, input.messageId))
        .limit(1);
  ...
  getLastNotification: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [msg] = await db
        ...
        .limit(1);
      if (!msg || !msg.senderId)
        return { lastNotification: null, blockedUntil: null, isBlocked: false };
      ...
      return {
        lastNotification: last.createdAt,
  ...
  notifyEmployee: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (msg.isAnonymous || !msg.senderId) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "No se puede notificar a un remitente anónimo",
        });
      }
      ...
      if (recentNotif) {
        const blockedUntil = new Date(
          new Date(recentNotif.createdAt).getTime() + 24 * 60 * 60 * 1000
        );
        throw new TRPCError({
