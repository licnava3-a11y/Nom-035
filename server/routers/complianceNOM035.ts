...
export const complianceNOM035Router = router({
   ...
   */
  getComplianceByNumeral: protectedProcedure.query(async () => {
    try {
      ...
      const checksMap = new Map(
        checks.map(c => [
          c.checklistItemId,
          { isCompliant: c.isCompliant, dueDate: c.dueDate },
        ])
      );

      // Agrupar por numeral (fundamento)
      const byNumeral: Record<
        string,
        { total: number; completed: number; items: any[] }
      > = {};
      ...
      const result = Object.entries(byNumeral).map(
        ([numeral, data]: [string, any]) => ({
          numeral,
          total: data.total,
      ...
      console.error(
        "[ComplianceNOM035] Error getting compliance by numeral:",
        error
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
      ...
      try {
        ...
        throw new TRPCError({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Error al asignar fecha de vencimiento",
        });
      }
    }),
    ...
    .mutation(async ({ input }) => {
      try {
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message:
            error instanceof Error ? error.message : "Error al desmarcar",
        });
      }
    }),
  ...
  generateComplianceReport: protectedProcedure.mutation(async ({ ctx }) => {
    try {
      ...
      // Calcular porcentajes
      const numerals = Object.entries(byNumeral).map(
        ([numeral, data]: [string, any]) => ({
          numeral,
          total: data.total,
          completed: data.completed,
      ...
      doc
        .fontSize(24)
        .font("Helvetica-Bold")
        .text("Reporte de Cumplimiento", { align: "center" });
      ...
      doc.text(
        `Porcentaje de cumplimiento global: ${globalPercentage.toFixed(1)}%`
      );
      doc.moveDown(2);
      ...
      doc
        .fontSize(16)
        .font("Helvetica-Bold")
        .text("Cumplimiento por Numeral", { underline: true });
      doc.moveDown(1);
      ...
      numerals.forEach((numeral: any) => {
        ...
        doc.fontSize(12).font("Helvetica");
        doc.text(
          `Completados: ${numeral.completed}/${numeral.total} (${numeral.percentage.toFixed(1)}%)`
        );
        doc.moveDown(0.5);

        // Listar items pendientes
        const pendingItems = numeral.items.filter(
        ...
        if (pendingItems.length > 0) {
          doc
            .fontSize(10)
            .font("Helvetica-Bold")
            .text("Requisitos pendientes:");
          pendingItems.forEach((item: any) => {
            doc.fontSize(9).font("Helvetica");
            doc.text(`  • ${item.itemCode}: ${item.requirement}`, {
              indent: 20,
            });
            if (item.dueDate) {
              ...
              doc.text(
                `    Vence: ${new Date(item.dueDate).toLocaleDateString()} (${daysUntil} días)`,
                { indent: 20 }
              );
            }
          });
      ...
      doc
        .fontSize(16)
        .font("Helvetica-Bold")
        .text("Plan de Acción", { underline: true });
      doc.moveDown(1);
      doc.fontSize(12).font("Helvetica");
      doc.text(
        "Recomendaciones para completar el cumplimiento de la NOM-035-STPS-2018:"
      );
      doc.moveDown(1);

      const allPendingItems = numerals.flatMap((n: any) =>
        n.items.filter((item: any) => !item.isCompleted)
      );
      if (allPendingItems.length > 0) {
        ...
        doc.text(
          "3. Establecer fechas de vencimiento para requisitos sin fecha"
        );
        doc.text("4. Revisar evidencias requeridas y comenzar su recopilación");
        doc.text(
          "5. Programar revisiones periódicas del avance de cumplimiento"
        );
      } else {
      ...
      console.error(
        "[ComplianceNOM035] Error generating compliance report:",
        error
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
