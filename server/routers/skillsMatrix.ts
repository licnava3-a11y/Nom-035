...
export const skillsMatrixRouter = router({
  ...
  getMatrix: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.employeeName) {
        filters.push(
          sql`CONCAT(${employees.firstName}, ' ', ${employees.lastName}) LIKE ${`%${input.employeeName}%`}`
        );
      }
      ...
      const competenciesList = await db
        .select()
        .from(competencies)
        .orderBy(competencies.type, competencies.name);
      ...
      const departmentScores = matrixEntries
        ...
        .map(
          (e: typeof skillsMatrix.$inferSelect) =>
            levelValues[e.level as keyof typeof levelValues]
        );
      const departmentAverage =
        departmentScores.length > 0
          ? departmentScores.reduce((a: number, b: number) => a + b, 0) /
            departmentScores.length
          : 0;

      // Calculate per-competency averages
      const competencyAverages = competenciesList.map(
        (comp: typeof competencies.$inferSelect) => {
          const compEntries = matrixEntries.filter(
            (e: typeof skillsMatrix.$inferSelect) => e.competencyId === comp.id
          );
          const scores = compEntries
            .filter(
              (e: typeof skillsMatrix.$inferSelect) => e.level in levelValues
            )
            .map(
              (e: typeof skillsMatrix.$inferSelect) =>
                levelValues[e.level as keyof typeof levelValues]
  ...
  exportToExcel: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const employeesList = input.departmentId
        ? await employeesListQuery.where(
      ...
      const exportData = employeesList.map((emp: any) => {
        ...
        competenciesList.forEach((comp: typeof competencies.$inferSelect) => {
          const entry = matrixEntries.find(
            (e: typeof skillsMatrix.$inferSelect) =>
              e.employeeId === emp.id && e.competencyId === comp.id
          );
      ...
      const developmentAnalysis = employeesList.map((emp: any) => {
        const empEntries = matrixEntries.filter(
          (e: any) => e.employeeId === emp.id
        );
        const totalCompetencies = competenciesList.length;
        const evaluatedCount = empEntries.filter(
          (e: any) => e.level !== "Sin evaluar"
        ).length;
        const avgLevel =
          empEntries.reduce(
            (sum: any, e: any) => sum + levelToValue(e.level),
            0
          ) / totalCompetencies;
        ...
        const gaps = competenciesList
          .map((comp: any) => {
            ...
            return {
              ...
              brecha:
                currentValue < 3 ? `Mejorar a Avanzado/Experto` : "Ninguna",
            };
          })
      ...
      employeesList.forEach((emp: any) => {
        ...
        successionAnalysis[deptName].push({
          ...
          nivelPromedio: avgLevel.toFixed(2),
          potencial:
            avgLevel >= 3 ? "Alto" : avgLevel >= 2 ? "Medio" : "En desarrollo",
        });
      });

      // Ordenar candidatos por nivel promedio (descendente)
      Object.keys(successionAnalysis).forEach((dept: any) => {
        successionAnalysis[dept].sort(
          (a: any, b: any) =>
            parseFloat(b.nivelPromedio) - parseFloat(a.nivelPromedio)
        );
      });
      ...
      Object.keys(successionAnalysis).forEach((dept: any) => {
        const deptEmployees = employeesList.filter(
          (e: any) => (e.departmentName || "Sin departamento") === dept
        );
        const deptEntries = matrixEntries.filter((e: any) =>
          deptEmployees.some((emp: any) => emp.id === e.employeeId)
        );

        // Identificar competencias con mayor brecha en el departamento
        const competencyGaps = competenciesList.map((comp: any) => {
          const compEntries = deptEntries.filter(
            (e: any) => e.competencyId === comp.id
          );
          const avgLevel =
            compEntries.reduce(
        ...
        const topGaps = competencyGaps
          .sort(
            (a: any, b: any) =>
              parseFloat(a.nivelPromedio) - parseFloat(b.nivelPromedio)
          )
          .slice(0, 5);
  ...
  getActiveDepartments: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db)
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        ...
        .where(
          and(
            input.departmentId
              ...
              : undefined,
            input.employeeIds
              ? sql`${employeeCompetencies.employeeId} IN (${sql.join(
      ...
      matrixData.forEach((row: any) => {
        ...
        if (gap > 0) {
          ...
          employeeGaps[row.employeeId].gaps.push({
            ...
            competencyType: row.competencyType as
              | "tecnica"
              | "transversal"
              | "conocimiento",
        ...
        for (const gap of data.gaps) {
          ...
          if (existing.length === 0) {
            ...
            await (db.insert(trainingNeeds) as any).values({
              ...
              competencyType: gap.competencyType as
                | "tecnica"
                | "transversal"
                | "conocimiento",
              currentLevel: gap.currentLevel as
                | "ninguno"
                | "basico"
                | "intermedio"
                | "avanzado"
              ...
              notes:
                "Generado automáticamente desde Análisis de Desarrollo de Matriz de Habilidades",
              createdAt: new Date(),
              updatedAt: new Date(),
            });
            competenciesAdded++;
  ...
  <div class="container">
    ...
    <div class="footer">
      ...
      <p>Si tienes alguna pregunta, contacta a tu coordinador de capacitación.</p>
            ...
            console.error(
              `Error al enviar notificación a empleado ${employeeId}:`,
              emailError
            );
