  ...
  getStats: protectedProcedure
    ...
    .query(async () => {
      ...
      const byPage = await db
        .select({ page: companyVisits.page, visits: count() })
        .from(companyVisits)
        .groupBy(companyVisits.page)
        .orderBy(desc(count()))
        .limit(10);
      const byCompany = await db
        .select({ companyName: companyVisits.companyName, visits: count() })
        .from(companyVisits)
        .groupBy(companyVisits.companyName)
        .orderBy(desc(count()))
        .limit(10);
      const recent = await db
        .select()
        .from(companyVisits)
        .orderBy(desc(companyVisits.visitedAt))
        .limit(50);
