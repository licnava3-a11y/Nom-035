 ...
 */
function calculateQuadrant(performanceScore: number, potentialScore: number): { quadrant: number; label: string; developmentPlan: string } {
  // Validar rangos (1-3)
  ...
  const quadrantData: Record<number, { label: string; developmentPlan: string }> = {
    1: {
      label: "Under Performer",
      developmentPlan: "Plan de Mejora del Desempeño (PIP): Establecer objetivos claros y medibles. Coaching intensivo. Evaluar fit organizacional en 3-6 meses."
    },
    2: {
      label: "Low Performer",
      developmentPlan: "Desarrollo de habilidades técnicas. Mentoría para mejorar desempeño. Clarificar expectativas del rol."
    },
    3: {
      label: "Inconsistent Performer",
      developmentPlan: "Identificar barreras de desempeño. Coaching para consistencia. Evaluar alineación rol-fortalezas."
    },
    4: {
      label: "Solid Contributor",
      developmentPlan: "Reconocimiento de contribuciones. Desarrollo de habilidades especializadas. Proyectos de ampliación de responsabilidades."
    },
    5: {
      label: "Core Performer",
      developmentPlan: "Backbone de la organización. Desarrollo continuo. Proyectos desafiantes. Reconocimiento y retención."
    },
    6: {
      label: "Emerging Talent",
      developmentPlan: "Desarrollo acelerado. Mentoría con líderes senior. Proyectos estratégicos. Plan de carrera a mediano plazo."
    },
    7: {
      label: "Solid Professional",
      developmentPlan: "Experto técnico. Desarrollo de habilidades de liderazgo. Oportunidades de mentoría a otros."
    },
    8: {
      label: "High Performer",
      developmentPlan: "Desarrollo de liderazgo. Proyectos de alto impacto. Preparación para roles de mayor responsabilidad."
    },
    9: {
      label: "High Potential",
      developmentPlan: "Fast-track de desarrollo. Exposición ejecutiva. Proyectos estratégicos críticos. Sucesión para roles clave."
    }
  };
...
export const nineBoxRouter = router({
  ...
  create: protectedProcedure
    .input(
      z.object({
        employeeId: z.number().int().positive({ message: "ID de empleado inválido" }),
        performanceScore: z.number().int().min(1).max(3, { message: "Score de desempeño debe ser 1-3 (Bajo/Medio/Alto)" }),
        potentialScore: z.number().int().min(1).max(3, { message: "Score de potencial debe ser 1-3 (Bajo/Medio/Alto)" }),
        evaluationDate: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Formato de fecha inválido (YYYY-MM-DD)" }),
      ...
      if (!db) throw new Error('Database not initialized');
        ...
        const latestEvaluations = db
          .select({
            ...
            maxDate: sql<string>`MAX(${nineBoxEvaluations.evaluationDate})`.as("max_date"),
          })
        ...
        const evaluations = await query.orderBy(desc(nineBoxEvaluations.evaluationDate));
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (!db) throw new Error('Database not initialized');
        ...
        const newPerformance = input.performanceScore ?? current.performanceScore;
        const newPotential = input.potentialScore ?? current.potentialScore;
        const { quadrant, label, developmentPlan } = calculateQuadrant(newPerformance, newPotential);
