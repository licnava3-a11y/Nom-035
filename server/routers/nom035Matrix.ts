...
function buildSurveyContext(
  ...
  for (const r of results) {
    ...
    if (r.domainScores) {
      try {
        const ds =
          typeof r.domainScores === "string"
            ? JSON.parse(r.domainScores)
            : r.domainScores;
  ...
  const riskPct = (level: string) =>
    total > 0 ? Math.round(((riskDist[level] ?? 0) / total) * 100) : 0;
  ...
  const guiaLabel =
    {
      guia_i: "Guía I (≤50 trabajadores)",
      guia_ii: "Guía II (>50 trabajadores)",
      guia_iii: "Guía III (Seguimiento)",
...
const updateActionSchema = z.object({
  ...
  estado: z
    .enum(["no_iniciada", "en_proceso", "cumplida", "vencida", "cancelada"])
    .optional(),
...
const filterActionsSchema = z.object({
  ...
  tipoPlan: z
    .enum(["intervencion", "violencia_laboral", "no_discriminacion"])
    .optional(),
  nivelAplicacion: z
...
export const nom035MatrixRouter = router({
  ...
  listPlans: protectedProcedure
    .input(
      z.object({
        status: z
          .enum(["borrador", "activo", "cerrado", "archivado"])
          .optional(),
        tipoPlan: z
          .enum([
            "intervencion",
    ...
    .query(async ({ input }) => {
      ...
      if (input.tipoPlan)
        conditions.push(eq(nom035Plans.tipoPlan, input.tipoPlan));
  ...
  getPlan: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [plan] = await db
        .select()
        .from(nom035Plans)
        .where(eq(nom035Plans.id, input.id));
      if (!plan)
        throw new TRPCError({
          code: "NOT_FOUND",
  ...
  generatePlan: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const tipoLabel = {
        ...
        consolidado:
          "Plan Consolidado (Intervención + Violencia + No Discriminación)",
      }[input.tipoPlan];
        ...
        if (!activePeriod) {
          ...
          if (lastClosed) {
            ...
            const results = await db
              ...
              .innerJoin(
                surveyResponses,
                eq(surveyResults.responseId, surveyResponses.id)
              )
      ...
      try {
        ...
        if (prevActions.length > 0) {
          ...
          const cumplidas = prevActions.filter(
            a => a.estado === "cumplida"
          ).length;
          const vencidas = prevActions.filter(
            a => a.estado === "vencida"
          ).length;
...
Responde ÚNICAMENTE con JSON válido con esta estructura:
{
          ...
          messages: [
            {
              role: "system",
              content:
                "Eres un experto en NOM-035-STPS-2018. Responde solo con JSON válido.",
            },
  ...
  updatePlanSignatures: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      await db
        .update(nom035Plans)
        .set({
          ...(updates.firmaNombreResponsable !== undefined && {
            firmaNombreResponsable: updates.firmaNombreResponsable,
          }),
          ...(updates.firmaCargoResponsable !== undefined && {
            firmaCargoResponsable: updates.firmaCargoResponsable,
          }),
          ...(updates.firmaNombreRepLegal !== undefined && {
            firmaNombreRepLegal: updates.firmaNombreRepLegal,
          }),
          ...(updates.firmaCargoRepLegal !== undefined && {
            firmaCargoRepLegal: updates.firmaCargoRepLegal,
          }),
          ...(updates.firmaFecha !== undefined && {
            firmaFecha: new Date(updates.firmaFecha),
          }),
  ...
  listActions: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.tipoPlan)
        conditions.push(eq(nom035Actions.tipoPlan, input.tipoPlan));
      if (input.nivelAplicacion)
        conditions.push(
          eq(nom035Actions.nivelAplicacion, input.nivelAplicacion)
        );
      if (input.estado) conditions.push(eq(nom035Actions.estado, input.estado));
      if (input.responsable)
        conditions.push(
          like(nom035Actions.responsable, `%${input.responsable}%`)
        );
      if (input.plazoDesde)
        conditions.push(gte(nom035Actions.plazo, new Date(input.plazoDesde)));
      if (input.plazoHasta)
        conditions.push(lte(nom035Actions.plazo, new Date(input.plazoHasta)));
      if (input.search) {
        conditions.push(
          or(
      ...
      if (actionIds.length > 0) {
        ...
        for (const ev of evidences) {
          if (!evidencesByAction[ev.actionId])
            evidencesByAction[ev.actionId] = [];
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db
        ...
        .set({
          ...
          ...(updates.responsableEmail !== undefined && {
            responsableEmail: updates.responsableEmail,
          }),
          ...(updates.plazo !== undefined && {
            plazo: new Date(updates.plazo),
          }),
  ...
  getEvidences: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const evidences = await db
        ...
        .where(
          and(
            eq(nom035Evidences.actionId, input.actionId),
            eq(nom035Evidences.isActive, true)
          )
        )
        .orderBy(desc(nom035Evidences.fechaSubida));
  ...
  deleteEvidence: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [evidence] = await db
        .select()
        .from(nom035Evidences)
        .where(eq(nom035Evidences.id, input.id));
      if (!evidence)
        throw new TRPCError({
          code: "NOT_FOUND",
      ...
      await db
        .update(nom035Evidences)
        .set({ isActive: false })
        .where(eq(nom035Evidences.id, input.id));

      // Registrar auditoría
      const [action] = await db
        .select({ planId: nom035Actions.planId })
        .from(nom035Actions)
        .where(eq(nom035Actions.id, evidence.actionId));
      await logAudit({
        evidenceId: input.id,
  ...
  getAuditLog: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const conditions = [];
      if (input.actionId)
        conditions.push(eq(nom035EvidenceAudit.actionId, input.actionId));
      if (input.planId)
        conditions.push(eq(nom035EvidenceAudit.planId, input.planId));
  ...
  getPlanStats: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const [stats] = await db
        ...
        .from(nom035Actions)
        .where(eq(nom035Actions.planId, input.planId));
  ...
  generatePdf: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [plan] = await db
        .select()
        .from(nom035Plans)
        .where(eq(nom035Plans.id, input.planId));
      if (!plan)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Plan no encontrado",
      ...
      const actions = await db
        ...
        .where(
          and(
            eq(nom035Actions.planId, input.planId),
      ...
      const { generateNom035MatrixPdf } = await import(
        "../lib/nom035MatrixPdfGenerator"
      );
  ...
  exportXlsx: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.planId) conditions.push(eq(nom035Actions.planId, input.planId));
      if (input.tipoPlan)
        conditions.push(eq(nom035Actions.tipoPlan, input.tipoPlan as any));
      if (input.estado)
        conditions.push(eq(nom035Actions.estado, input.estado as any));
      ...
      for (const action of actions) {
        ...
        const row = ws.addRow({
          ...
          plazo: action.plazo
            ? new Date(action.plazo).toLocaleDateString("es-MX")
            : "",
          ...
          ultimaEvidencia:
            evs.length > 0
              ? new Date(evs[evs.length - 1].fechaSubida).toLocaleDateString(
                  "es-MX"
                )
              : "",
        });
        ...
        if (evs.length > 0) {
          ...
          row.getCell("numEvidencias").font = {
            color: { argb: "FF2563EB" },
            bold: true,
          };
        }
  ...
  generateCompliancePdf: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const planesConStats = await Promise.all(
        planes.map(async plan => {
          const [stats] = await db
            ...
            .where(
              and(
                eq(nom035Actions.planId, plan.id),
                eq(nom035Actions.isActive, true)
              )
            );
      ...
      const porcentajeCumplimientoGlobal =
        totalGlobal > 0 ? Math.round((cumplidasGlobal / totalGlobal) * 100) : 0;
      const semaforoGlobal: "verde" | "amarillo" | "rojo" =
        porcentajeCumplimientoGlobal >= 80
          ? "verde"
          : porcentajeCumplimientoGlobal >= 50
            ? "amarillo"
            : "rojo";
      ...
      const pdfData = {
        ...
        byTipoPlan: byTipoPlan.map(r => ({
          ...
          porcentaje:
            Number(r.total) > 0
              ? Math.round((Number(r.cumplidas) / Number(r.total)) * 100)
              : 0,
        })),
      ...
      const { generateNom035CompliancePdf } = await import(
        "../lib/nom035CompliancePdfGenerator"
      );
  ...
  getActionHistory: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.actionId)
        conditions.push(eq(nom035ActionHistory.actionId, input.actionId));
      if (input.planId)
        conditions.push(eq(nom035ActionHistory.planId, input.planId));
  ...
  addHistoryNote: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Verificar que la acción existe
      const [action] = await db
        .select({ id: nom035Actions.id, planId: nom035Actions.planId })
        .from(nom035Actions)
        .where(eq(nom035Actions.id, input.actionId))
        .limit(1);
      if (!action)
  ...
  exportHistoryXlsx: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.actionId)
        conditions.push(eq(nom035ActionHistory.actionId, input.actionId));
      if (input.campo)
        conditions.push(eq(nom035ActionHistory.campo as any, input.campo));
      if (input.changedByName)
        conditions.push(
          like(nom035ActionHistory.changedByName, `%${input.changedByName}%`)
        );
  ...
  exportHistoryPdf: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      doc
        ...
        .font("Helvetica")
        .text(
          `Generado: ${new Date().toLocaleString("es-MX", { timeZone: "America/Mexico_City" })} | Total registros: ${rows.length}`,
          40,
          38
        );
      ...
      if (input.campo)
        filtros.push(`Campo: ${CAMPO_LABELS[input.campo] ?? input.campo}`);
      if (input.changedByName) filtros.push(`Usuario: ${input.changedByName}`);
      if (filtros.length > 0) {
        doc
          .fillColor("white")
          .fontSize(9)
          .text(`Filtros: ${filtros.join(" | ")}`, 40, 50);
      }
    ...
    return [
      {
        ...
        objetivo: "Establecer política de cero tolerancia",
        accion:
          "Redactar y publicar política de cero tolerancia a la violencia laboral",
        descripcionCompleta:
          "Elaborar documento formal de política contra violencia laboral, obtener firma de dirección y difundir a todos los trabajadores.",
        indicador: "Política publicada y firmada por dirección general",
        responsable: "Responsable NOM-035",
        plazoDias: 30,
        prioridad: "alta",
      },
      {
        accionId: "VL-02",
        tipoPlan: "violencia_laboral",
        objetivo: "Implementar canal de denuncia",
        accion: "Crear buzón de quejas y canal de denuncia confidencial",
        descripcionCompleta:
          "Establecer mecanismo seguro y confidencial para que los trabajadores reporten actos de violencia.",
        indicador: "Canal operativo con al menos 1 prueba de funcionamiento",
        responsable: "RH / Comité NOM-035",
        plazoDias: 45,
        prioridad: "alta",
      },
      {
        accionId: "VL-03",
        tipoPlan: "violencia_laboral",
        objetivo: "Capacitar a supervisores",
        accion:
          "Impartir taller de prevención de violencia laboral a mandos medios",
        descripcionCompleta:
          "Capacitación de 4 horas sobre identificación, prevención y atención de violencia laboral.",
        indicador: "Lista de asistencia con mínimo 80% de participación",
        responsable: "Instructor / RH",
        plazoDias: 60,
        prioridad: "media",
      },
    ];
    ...
    return [
      {
        ...
        objetivo: "Política de inclusión y no discriminación",
        accion: "Elaborar y publicar política de no discriminación",
        descripcionCompleta:
          "Documento formal que prohíbe discriminación por género, edad, discapacidad, origen étnico, etc.",
        indicador: "Política publicada en cartelera y portal interno",
        responsable: "Dirección / RH",
        plazoDias: 30,
        prioridad: "alta",
      },
      {
        accionId: "ND-02",
        tipoPlan: "no_discriminacion",
        objetivo: "Capacitación en diversidad e inclusión",
        accion: "Impartir taller de diversidad e inclusión a todo el personal",
        descripcionCompleta:
          "Taller de 3 horas sobre sesgos inconscientes, inclusión y respeto a la diversidad.",
        indicador: "80% del personal capacitado con lista de asistencia",
        responsable: "RH / Instructor",
        plazoDias: 60,
        prioridad: "media",
      },
      {
        accionId: "ND-03",
        tipoPlan: "no_discriminacion",
        objetivo: "Indicadores de equidad",
        accion: "Establecer métricas de equidad de género y diversidad",
        descripcionCompleta:
          "Definir y medir indicadores de paridad salarial, distribución por género en puestos directivos.",
        indicador: "Reporte trimestral de indicadores de equidad",
        responsable: "RH / Dirección",
        plazoDias: 90,
        prioridad: "baja",
      },
    ];
  ...
  return [
    {
      ...
      accion: "Revisar y redistribuir cargas de trabajo por área",
      descripcionCompleta:
        "Análisis de distribución de tareas y ajuste de asignaciones para evitar sobrecarga.",
      indicador: "Reducción de 20% en horas extra reportadas",
      responsable: "Jefes de área / RH",
      plazoDias: 30,
      prioridad: "alta",
    },
    {
      ...
      accion: "Implementar reuniones semanales de seguimiento",
      descripcionCompleta:
        "Reuniones breves (30 min) semanales entre jefes y equipos para comunicar avances y resolver dudas.",
      indicador: "Minutas de reunión semanales por 3 meses",
    ...
    },
    {
      ...
      accion: "Capacitar a mandos medios en liderazgo y gestión del estrés",
      descripcionCompleta:
        "Programa de 8 horas de capacitación en liderazgo empático y manejo del estrés laboral.",
      indicador: "Lista de asistencia y evaluación de satisfacción ≥ 80%",
      responsable: "RH / Instructor",
      plazoDias: 45,
      prioridad: "alta",
    },
    {
      accionId: "INT-04",
      tipoPlan: "intervencion",
      objetivo: "Establecer programa de bienestar",
      accion: "Implementar actividades de bienestar y pausas activas",
      descripcionCompleta:
        "Programa mensual de actividades de bienestar: pausas activas, meditación, actividad física.",
      indicador: "Registro de participación mensual",
      responsable: "Comité NOM-035",
      plazoDias: 60,
      prioridad: "media",
    },
  ];
