...
export const documentFormatsRouter = router({
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (input.codigo !== undefined) updateData.codigo = input.codigo.toUpperCase();
      if (input.nombre !== undefined) updateData.nombre = input.nombre;
      if (input.descripcion !== undefined) updateData.descripcion = input.descripcion;
      if (input.version !== undefined) updateData.version = input.version;
      if (input.fechaVersion !== undefined) updateData.fechaVersion = new Date(input.fechaVersion);
