...
async function consolidateReportData(startDate: Date, endDate: Date) {
  ...
  const openCases = casesData
    ...
    .reduce((sum: any, c: any) => sum + c.total, 0);
  const closedCases = casesData
    .filter(c => c.status === "closed")
    .reduce((sum: any, c: any) => sum + c.total, 0);
  const criticalCases = casesData
    .filter(c => (c as any).riskLevel === "muy_alto")
    .reduce((sum: any, c: any) => sum + c.total, 0);
  ...
  const highRiskCount = riskData
    .filter(
      r =>
        (r as any).riskLevel === "high" || (r as any).riskLevel === "very_high"
    )
    .reduce((sum: any, r: any) => sum + r.count, 0);
  const mediumRiskCount =
    riskData.find(r => (r as any).riskLevel === "medium")?.count || 0;
  const lowRiskCount =
    riskData.find(r => (r as any).riskLevel === "low")?.count || 0;
...
async function generatePDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc
      .fontSize(24)
      .font("Helvetica-Bold")
      .text("Reporte Ejecutivo NOM-035", { align: "center" });
    doc.moveDown();
    ...
    doc
      .fontSize(12)
      .text(
        `Periodo: ${reportData.period.startDate} a ${reportData.period.endDate}`,
        { align: "center" }
      );
    doc.moveDown(3);
    ...
    doc.text(
      `Encuestas Completadas: ${reportData.surveys.completed} de ${reportData.surveys.total} (${reportData.surveys.completionRate.toFixed(1)}%)`
    );
    doc.text(`Casos Registrados: ${reportData.cases.total}`);
    doc.text(`Casos Críticos: ${reportData.cases.critical}`);
    doc.text(
      `Tasa de Resolución: ${reportData.cases.resolutionRate.toFixed(1)}%`
    );
    doc.moveDown(2);
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Casos NOM-035", { underline: true });
    doc.moveDown();
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Evaluación de Riesgo Psicosocial", { underline: true });
    doc.moveDown();

    doc.fontSize(12).font("Helvetica");
    doc.text(`Total Evaluado: ${reportData.risk.total} empleados`);
    doc.text(
      `Riesgo Alto/Muy Alto: ${reportData.risk.high} (${reportData.risk.total > 0 ? ((reportData.risk.high / reportData.risk.total) * 100).toFixed(1) : 0}%)`
    );
    doc.text(
      `Riesgo Medio: ${reportData.risk.medium} (${reportData.risk.total > 0 ? ((reportData.risk.medium / reportData.risk.total) * 100).toFixed(1) : 0}%)`
    );
    doc.text(
      `Riesgo Bajo: ${reportData.risk.low} (${reportData.risk.total > 0 ? ((reportData.risk.low / reportData.risk.total) * 100).toFixed(1) : 0}%)`
    );
    doc.moveDown(2);
    ...
    doc
      .fontSize(16)
      .font("Helvetica-Bold")
      .text("Distribución por Departamento", { underline: true });
    doc.moveDown();

    doc.fontSize(12).font("Helvetica");
    reportData.employees.byDepartment.forEach((dept: any) => {
      doc.text(
        `${dept.department || "Sin departamento"}: ${dept.count} empleados`
      );
    });
    doc.moveDown(2);
    ...
    if (reportData.cases.critical > 0) {
      doc.text(
        `• Atención inmediata a ${reportData.cases.critical} casos críticos pendientes`
      );
    }

    if (reportData.risk.high > 0) {
      doc.text(
        `• Implementar intervenciones para ${reportData.risk.high} empleados en riesgo alto`
      );
    }

    if (reportData.surveys.completionRate < 80) {
      doc.text(
        `• Incrementar tasa de respuesta de encuestas (actual: ${reportData.surveys.completionRate.toFixed(1)}%)`
      );
    }

    if (reportData.cases.resolutionRate < 70) {
      doc.text(
        `• Mejorar tasa de resolución de casos (actual: ${reportData.cases.resolutionRate.toFixed(1)}%)`
      );
    }
  ...
  generateReport: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const [report] = await (
          db.insert(executiveReportsHistory) as any
        ).values({
