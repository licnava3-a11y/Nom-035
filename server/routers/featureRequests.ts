...
export const featureRequestsRouter = router({
  ...
  getStats: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const en_desarrollo = all.filter(
        r => r.status === "en_desarrollo"
      ).length;
      ...
      const pctImplemented =
        total > 0 ? Math.round((implementada / total) * 100) : 0;
      const pctInProgress =
        total > 0
          ? Math.round(((implementada + en_desarrollo) / total) * 100)
          : 0;
      return {
        total,
        implementada,
