...
export const reportsRouter = router({
  ...
  generateNom035Report: protectedProcedure
    .input(
      z.object({
        ...
        signerIds: z
          .array(z.number())
          .min(2, "Se requieren al menos 2 firmantes"),
        conclusions: z
          .string()
          .min(50, "Las conclusiones deben tener al menos 50 caracteres"),
        recommendations: z
          .string()
          .min(50, "Las recomendaciones deben tener al menos 50 caracteres"),
      })
    )
    .mutation(async ({ input }: { input: any }) => {
      ...
      const getSysSetting = (key: string) =>
        sysSettingsRows.find((s: any) => s.settingKey === key)?.settingValue ??
        "";
      ...
      const [employeeCount] = await db.select({ count: count() }).from(users);
      ...
      const controlMeasures = [
        {
          ...
          measure:
            "Implementar programa de mantenimiento preventivo de instalaciones y equipo",
        ...
        },
        {
          ...
          measure:
            "Redistribución de cargas de trabajo y contratación de personal adicional",
      ...
      const signers = await db
        ...
        .from(signatures)
        .where(
          sql`${signatures.id} IN (${sql.join(
            input.signerIds.map((id: number) => sql`${id}`),
            sql`, `
          )})`
        );
      ...
      const pdfResult = await generateNom035Report({
        company: {
          ...
          stpsRegistration:
            getSysSetting("company_stps_registration") || undefined,
        },
        reportPeriod: {
   ...
   */
  getAvailablePeriods: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new Error("Database connection failed");
   ...
   */
  getAvailableSigners: protectedProcedure.query(async () => {
    const db = await getDb();
    if (!db) throw new Error("Database connection failed");
  ...
  generateCasesPDF: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      doc
        .fontSize(12)
        .text(
          `Período: ${input.period === "monthly" ? "Mensual" : "Trimestral"}`,
          { align: "center" }
        );
      doc.text(
        `Del ${input.startDate.split("T")[0]} al ${input.endDate.split("T")[0]}`,
        { align: "center" }
      );
      doc.moveDown(2);
      ...
      doc.text(
        `Tiempo Promedio de Resolución: ${Math.round(metrics.avgResolutionDays || 0)} días`
      );
      doc.moveDown(2);
      ...
      typeDistribution.forEach((item: any) => {
        doc.text(
          `${typeLabels[item.case_type] || item.case_type}: ${item.count}`
        );
      });
      doc.moveDown(2);
      ...
      if (Number(metrics.criticalCases || 0) > 0) {
        recommendations.push(
          `Se detectaron ${metrics.criticalCases} casos críticos. Se recomienda priorizar su atención inmediata.`
        );
      }
      if (Number(metrics.avgResolutionDays || 0) > 14) {
        recommendations.push(
          `El tiempo promedio de resolución (${Math.round(metrics.avgResolutionDays)} días) excede el estándar recomendado de 14 días. Se sugiere revisar procesos de atención.`
        );
      }
      if (
        Number(metrics.openCases || 0) >
        Number(metrics.totalCases || 1) * 0.5
      ) {
        recommendations.push(
          `Más del 50% de los casos están abiertos. Se recomienda asignar más recursos al comité.`
        );
      }
      if (recommendations.length === 0) {
        recommendations.push(
          "El sistema de atención de casos muestra un desempeño adecuado. Continuar con el monitoreo periódico."
        );
      }
      ...
      try {
        ...
        for (const dept of allDepartments) {
          ...
          if (dept.managerId) {
            ...
            if (manager[0]) {
              managersByDept.set(dept.id, `${manager[0].name || "Sin nombre"}`);
            }
          }
        }
