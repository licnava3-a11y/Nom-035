...
describe("workflow de integración", () => {
  it("aísla y siembra MySQL por defecto, y ejecuta servicios externos solo con secretos", () => {
    const workflow = readFileSync(resolve(process.cwd(), ".github/workflows/integration.yml"), "utf8");
    const packageJson = JSON.parse(readFileSync(resolve(process.cwd(), "package.json"), "utf8"));

    expect(workflow).toContain("workflow_dispatch:");
    expect(workflow).toContain("run_external_integrations");
    ...
    expect(workflow).toContain("pnpm test:integration:ai --pool=forks --maxWorkers=1 --minWorkers=1");
