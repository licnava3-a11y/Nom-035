...
describe('Sistema de Permisos Backend', () => {
  ...
  describe('hasAnyPermission', () => {
    it('instructor debe tener al menos uno de [can_create, can_edit]', () => {
      expect(hasAnyPermission('instructor', ['can_create', 'can_edit'])).toBe(true);
    });

    it('student no debe tener ninguno de [can_create, can_edit]', () => {
      expect(hasAnyPermission('student', ['can_create', 'can_edit'])).toBe(false);
    });

    it('committee debe tener al menos uno de [can_approve, can_delete]', () => {
      expect(hasAnyPermission('committee', ['can_approve', 'can_delete'])).toBe(true);
    });
  });

  describe('hasAllPermissions', () => {
    it('gerente debe tener todos los permisos [can_view, can_create, can_edit]', () => {
      expect(hasAllPermissions('gerente', ['can_view', 'can_create', 'can_edit'])).toBe(true);
    });

    it('instructor no debe tener todos los permisos [can_create, can_delete]', () => {
      expect(hasAllPermissions('instructor', ['can_create', 'can_delete'])).toBe(false);
    });

    it('administrativo debe tener todos los permisos [can_view, can_create, can_edit]', () => {
      expect(hasAllPermissions('administrativo', ['can_view', 'can_create', 'can_edit'])).toBe(true);
    });
  });
  ...
  describe('Matriz de Permisos Completa', () => {
    const roles = ['gerente', 'instructor', 'administrativo', 'committee', 'student'];
    const permissions: Permission[] = ['can_view', 'can_create', 'can_edit', 'can_delete', 'can_approve', 'can_export'];
    ...
    it('matriz de permisos debe coincidir con la documentación', () => {
      ...
      expect(getUserPermissions('instructor')).toEqual(['can_view', 'can_create', 'can_edit', 'can_export']);

      // Administrativo: view, create, edit, export
      expect(getUserPermissions('administrativo')).toEqual(['can_view', 'can_create', 'can_edit', 'can_export']);

      // Committee: view, create, approve
      expect(getUserPermissions('committee')).toEqual(['can_view', 'can_create', 'can_approve']);
