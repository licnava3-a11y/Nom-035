...
export function calculateDaysOpen(createdAt: Date, now: Date = new Date()): number {
  const diffMs = now.getTime() - createdAt.getTime();
  return Math.floor(diffMs / (24 * 60 * 60 * 1000));
}

export function isCaseStale(createdAt: Date, status: string, daysThreshold: number = 7, now: Date = new Date()): boolean {
  if (status !== 'open') return false;
  const daysOpen = calculateDaysOpen(createdAt, now);
  return daysOpen >= daysThreshold;
}

export function isCriticalCaseStale(createdAt: Date, status: string, priority: string, now: Date = new Date()): boolean {
  if (status !== 'open' || priority !== 'critical') return false;
  const daysOpen = calculateDaysOpen(createdAt, now);
  return daysOpen >= 3;
}

export function generateStaleNotificationMessage(caseNumber: string, daysOpen: number, isCritical: boolean = false): string {
  if (isCritical) {
...
export function classifyCaseUrgency(daysOpen: number, priority: string): 'low' | 'medium' | 'high' | 'critical' {
...
describe('Sistema de Alertas de Casos Estancados', () => {
  ...
  describe('isCriticalCaseStale', () => {
    ...
    it('debe detectar caso crítico abierto por más de 3 días', () => {
      const createdAt = new Date('2024-01-01T00:00:00Z');
      expect(isCriticalCaseStale(createdAt, 'open', 'critical', now)).toBe(true);
    });

    it('debe retornar false para caso crítico abierto por menos de 3 días', () => {
      const createdAt = new Date('2024-01-08T00:00:00Z');
      expect(isCriticalCaseStale(createdAt, 'open', 'critical', now)).toBe(false);
    });
    ...
    it('debe retornar false para casos críticos cerrados', () => {
      const createdAt = new Date('2024-01-01T00:00:00Z');
      expect(isCriticalCaseStale(createdAt, 'closed', 'critical', now)).toBe(false);
    });
  ...
  describe('Integración: Flujo completo de detección de casos estancados', () => {
    ...
    it('debe identificar correctamente múltiples casos estancados', () => {
      const cases = [
        { id: 1, caseNumber: 'CASE-001', createdAt: new Date('2024-01-01'), status: 'open', priority: 'low' },
        { id: 2, caseNumber: 'CASE-002', createdAt: new Date('2024-01-10'), status: 'open', priority: 'medium' },
        { id: 3, caseNumber: 'CASE-003', createdAt: new Date('2024-01-15'), status: 'open', priority: 'high' },
    ...
    it('debe identificar correctamente casos críticos estancados', () => {
      const cases = [
        { id: 1, caseNumber: 'CASE-001', createdAt: new Date('2024-01-15'), status: 'open', priority: 'critical' },
        { id: 2, caseNumber: 'CASE-002', createdAt: new Date('2024-01-10'), status: 'open', priority: 'critical' },
    ...
    it('debe generar notificaciones apropiadas para cada tipo de caso', () => {
      const regularCase = { caseNumber: 'CASE-001', daysOpen: 10, priority: 'medium' };
      const criticalCase = { caseNumber: 'CASE-002', daysOpen: 5, priority: 'critical' };

      const regularMessage = generateStaleNotificationMessage(regularCase.caseNumber, regularCase.daysOpen, false);
      const criticalMessage = generateStaleNotificationMessage(criticalCase.caseNumber, criticalCase.daysOpen, true);
