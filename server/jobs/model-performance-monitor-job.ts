...
import { employeeTurnoverHistory, modelPerformanceAlerts } from "../../drizzle/schema";
...
export function startModelPerformanceMonitorJob() {
  ...
  cron.schedule("0 9 * * *", async () => {
    console.log("[Model Performance Monitor Job] Starting daily performance check...");
    ...
    try {
      ...
      if (turnoverRecords.length === 0) {
        console.log("[Model Performance Monitor Job] No turnover records found, skipping check");
        return;
      }
      ...
      if (precision < CRITICAL_THRESHOLDS.precision) {
        alerts.push({
          ...
          severity: precision < 50 ? "critical" : precision < 60 ? "high" : "medium",
          ...
          recommendation: "Considera ajustar los pesos de la fórmula predictiva en la página de Configuración de Umbrales. Aumenta el peso de los factores más correlacionados con rotación real.",
        });
      }
      ...
      if (recall < CRITICAL_THRESHOLDS.recall) {
        alerts.push({
          ...
          recommendation: "El modelo está perdiendo empleados de alto riesgo. Revisa si los umbrales de clasificación son demasiado altos o si faltan factores de riesgo en la fórmula.",
        });
      }
      ...
      if (f1Score < CRITICAL_THRESHOLDS.f1Score) {
        alerts.push({
          ...
          severity: f1Score < 45 ? "critical" : f1Score < 55 ? "high" : "medium",
          message: `El F1-Score del modelo ha caído a ${f1Score.toFixed(1)}%, por debajo del umbral crítico de ${CRITICAL_THRESHOLDS.f1Score}%`,
          recommendation: "El balance entre precisión y recall está comprometido. Usa el dashboard de A/B Testing para comparar configuraciones históricas y encontrar la óptima.",
        });
      }
      ...
      if (accuracy < CRITICAL_THRESHOLDS.accuracy) {
        alerts.push({
          ...
          severity: accuracy < 50 ? "critical" : accuracy < 60 ? "high" : "medium",
          ...
          recommendation: "La capacidad general del modelo para clasificar correctamente está degradada. Revisa la calidad de los datos de entrada y considera reentrenar el modelo.",
        });
      }
      ...
      if (alerts.length > 0) {
        ...
        await notifyOwner({
          ...
          content: `Se detectaron ${alerts.length} alertas de rendimiento del modelo:\n\n` +
            (criticalCount > 0 ? `🔴 ${criticalCount} alertas críticas\n` : "") +
            (highCount > 0 ? `🟠 ${highCount} alertas de alta prioridad\n` : "") +
            `\nRevisa el dashboard de Alertas de Rendimiento para más detalles y recomendaciones de ajuste.`,
        });
      } else {
        console.log("[Model Performance Monitor Job] All metrics within acceptable thresholds");
      }

      console.log("[Model Performance Monitor Job] Performance check completed successfully");
    } catch (error) {
      console.error("[Model Performance Monitor Job] Error during performance check:", error);
    }
  });

  console.log("[Model Performance Monitor Job] Scheduled to run daily at 09:00 AM");
}
