...
export async function runSurveyAlertsCheck() {
  ...
  try {
    ...
    console.log(
      `[Survey Alerts Job] Found ${allSurveys.length} surveys to check`
    );
    ...
    console.log(
      `[Survey Alerts Job] Using coverage threshold: ${coverageThreshold}%`
    );
    ...
    console.log(
      `[Survey Alerts Job] Coverage check completed:`,
      coverageResults
    );

    // Verificar alertas de trabajadores pendientes por 2+ días
    console.log("[Survey Alerts Job] Checking pending workers alerts...");
    const pendingResults = await caller.checkPendingWorkersAlerts({
      daysThreshold: 2,
    });
    console.log(
      `[Survey Alerts Job] Pending workers check completed:`,
      pendingResults
    );

    console.log(
      "[Survey Alerts Job] Automated alerts check completed successfully"
    );
    ...
    console.error(
      "[Survey Alerts Job] Error running automated alerts check:",
      error
    );
    return {
      success: false,
...
export function startSurveyAlertsJob() {
  console.log(
    "[Survey Alerts Job] Initializing automated alerts job (every 6 hours)..."
  );
