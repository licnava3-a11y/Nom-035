...
import { committeeMinuteAgreements, committeeMinutes } from "../../drizzle/schema";
...
export async function runAgreementsAlertsJob() {
  ...
  try {
    ...
    // Obtener acuerdos que vencen en 7 días
    ...
    // Obtener acuerdos que vencen en 3 días
    ...
    for (const agreement of agreementsIn7Days) {
      try {
        await sendEmail({
          to: 'admin@empresa.com', // TODO: Obtener email del responsable
          subject: `⚠️ Recordatorio: Acuerdo próximo a vencer en 7 días`,
          html: generate7DaysAlertEmail(agreement),
        });
        alertsSent++;
        console.log(`[Agreements Alerts Job] Alerta de 7 días enviada para acuerdo #${agreement.id}`);
      } catch (error) {
    ...
    for (const agreement of agreementsIn3Days) {
      try {
        ...
        alertsSent++;
        console.log(`[Agreements Alerts Job] Alerta de 3 días enviada para acuerdo #${agreement.id}`);
      } catch (error) {
  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          <p>Estimado/a <strong>${agreement.responsibleName}</strong>,</p>
          ...
          <div class="agreement-card">
            ...
            <p><strong>Minuta:</strong> ${agreement.numeroSesion}</p>
            ...
            <p>
              ...
              <span class="priority-badge" style="background: ${priorityColors[agreement.priority]};">
