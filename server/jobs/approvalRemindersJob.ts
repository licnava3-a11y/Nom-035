...
async function sendApprovalReminders() {
  ...
  try {
    ...
    console.log(
      `[Approval Reminders Job] Found ${pendingApprovals.length} pending approvals`
    );
    ...
    for (const approval of pendingApprovals) {
      if (!approval.approverEmail) {
        console.log(
          `[Approval Reminders Job] Skipping approval ${approval.approvalId}: no email for approver`
        );
        continue;
      }
      ...
      try {
        const hoursPending = Math.floor(
          (now.getTime() - new Date(approval.createdAt).getTime()) /
            (1000 * 60 * 60)
        );
        ...
        const roleLabel =
          roleLabels[approval.approverRole] || approval.approverRole;

        await sendEmail({
          to: approval.approverEmail,
          subject:
            "Recordatorio: Firma Pendiente - Base de Funcionamiento del Comité",
          html: `
            ...
            <ul>
              ...
              <li><strong>Fecha de solicitud:</strong> ${new Date(
                approval.createdAt
        ...
        remindersSent++;
        console.log(
          `[Approval Reminders Job] Reminder sent to ${approval.approverEmail}`
        );
      } catch (error) {
    ...
    console.log(
      `[Approval Reminders Job] Completed: ${remindersSent} reminders sent, ${errors.length} errors`
    );
