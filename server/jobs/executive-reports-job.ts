...
async function generateReportData(
  ...
  const avgResolutionDays =
    resolvedCasesWithDates.length > 0
  ...
  if (latestPeriod.length > 0) {
    ...
    surveyCoverage =
      coverageResult.totalEmployees > 0
    ...
    const riskCounts = await db
      ...
      .where(eq(surveyResponses.periodId, periodId));
    highRiskEmployees =
      riskCounts.find(r => r.riskLevel === "high")?.count || 0;
    mediumRiskEmployees =
      riskCounts.find(r => r.riskLevel === "medium")?.count || 0;
  }
  ...
  const departmentalData = await db
    ...
    .orderBy(
      desc(
        sql`COUNT(DISTINCT CASE WHEN ${cases.status} IN ('open', 'in_progress') THEN ${cases.id} END)`
      )
    )
    .limit(10);
  ...
  const departmentalRisks = departmentalData.map((d: any) => {
    const riskScore =
      (d.activeCases * 10 + d.highRiskCount * 5) / Math.max(d.employeeCount, 1);
  ...
  if (criticalCasesResult.count > 5) {
    recommendations.push(
      `Atención urgente: ${criticalCasesResult.count} casos críticos abiertos. Priorizar resolución inmediata.`
    );
  }

  if (surveyCoverage < 80) {
    recommendations.push(
      `Cobertura de encuestas baja (${surveyCoverage.toFixed(1)}%). Implementar campaña de sensibilización.`
    );
  }

  if (highRiskEmployees > totalEmployeesResult.count * 0.1) {
    recommendations.push(
      `${highRiskEmployees} empleados en riesgo alto. Programar intervenciones preventivas.`
    );
  }

  if (avgResolutionDays > 30) {
    recommendations.push(
      `Tiempo promedio de resolución elevado (${avgResolutionDays.toFixed(1)} días). Revisar procesos de atención.`
    );
  }

  if (trainingCoverage < 70) {
    recommendations.push(
      `Cobertura de capacitación baja (${trainingCoverage.toFixed(1)}%). Aumentar participación en cursos NOM-035.`
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "Sistema operando dentro de parámetros normales. Mantener monitoreo continuo."
    );
  }
 ...
 */
export async function runExecutiveReportsJob(
  period: "weekly" | "monthly" = "weekly"
) {
  console.log(
    `[Executive Reports Job] Starting ${period} report generation...`
  );
...
Ver reporte completo: ${reportUrl}`,
    ...
    console.log(
      `[Executive Reports Job] ${periodLabel} report generated successfully:`,
      reportUrl
    );
    ...
    console.error(
      `[Executive Reports Job] Error generating ${period} report:`,
      error
    );
    return {
      success: false,
