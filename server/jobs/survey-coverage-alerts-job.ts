...
export async function runCoverageAlertsCheck() {
  ...
  try {
    ...
    if (!result || !result.alerts || result.alerts.length === 0) {
      console.log(
        "[Coverage Alerts Job] No coverage alerts found. All surveys meet the 80% threshold."
      );
      return {
        success: true,
        alertsSent: 0,
    ...
    console.log(
      `[Coverage Alerts Job] Found ${result.totalAlerts} survey(s) with insufficient coverage`
    );
    ...
    console.log(
      `[Coverage Alerts Job] Sending alert email to ${coordinatorEmail}...`
    );
    ...
    console.error(
      "[Coverage Alerts Job] Error running coverage alerts check:",
      error
    );
    return {
      success: false,
...
export function startCoverageAlertsJob() {
  console.log(
    "[Coverage Alerts Job] Initializing weekly coverage alerts job (Mondays at 8:00 AM)..."
  );
  ...
  console.log(
    `[Coverage Alerts Job] Next execution scheduled for: ${nextMonday.toLocaleString("es-MX")}`
  );
  ...
  console.log(
    "[Coverage Alerts Job] Weekly coverage alerts job started successfully"
  );
}
