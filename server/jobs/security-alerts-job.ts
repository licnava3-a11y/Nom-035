...
export async function runSecurityAlertsCheck() {
  ...
  try {
    ...
    console.log(
      `[Security Alerts Job] Found ${recentAccesses.length} recent accesses to analyze`
    );

    // Agrupar por usuario
    const accessesByUser = recentAccesses.reduce<
      Record<number, typeof recentAccesses>
    >((acc, access) => {
    ...
    for (const [userIdStr, userAccesses] of Object.entries(accessesByUser)) {
      ...
      try {
        ...
        console.error(
          `[Security Alerts Job] Could not fetch user ${userId}:`,
          err
        );
        continue;
      }
      ...
      if (downloads.length > 5) {
        console.log(
          `[Security Alerts Job] ${downloads.length} downloads from user ${user.name}`
        );
        try {
          await withRetry(
            () =>
          ...
          console.error(
            `[Security Alerts Job] Error on suspicious downloads for user ${userId}:`,
            err
          );
        }
      }
      ...
      const uniqueIPs = Array.from(
        new Set(userAccesses.map(a => a.ipAddress).filter(Boolean))
      );
      for (const ip of uniqueIPs) {
        try {
          ...
          const knownIPs = new Set(
            historical.map(a => a.ipAddress).filter(Boolean)
          );
          if (knownIPs.size > 5 && !knownIPs.has(ip)) {
            console.log(
              `[Security Alerts Job] Unknown IP ${ip} for user ${user.name}`
            );
            try {
              await withRetry(
                () =>
                ...
                {
                  label: "detectSuspiciousActivity (unknown IP)",
                  maxAttempts: 2,
                }
              );
              alertsCreated++;
              ...
              console.error(
                `[Security Alerts Job] Error on unknown IP for user ${userId}:`,
                err
              );
            }
          }
          ...
          console.error(
            `[Security Alerts Job] Could not fetch historical accesses for user ${userId}:`,
            err
          );
        }
      }
      ...
      if (offHours.length > 0) {
        console.log(
          `[Security Alerts Job] ${offHours.length} off-hours accesses from user ${user.name}`
        );
        try {
          await withRetry(
            () =>
          ...
          console.error(
            `[Security Alerts Job] Error on off-hours access for user ${userId}:`,
            err
          );
        }
      }
    ...
    console.log(
      `[Security Alerts Job] Completed. Alerts created: ${alertsCreated}`
    );
    return {
      success: true,
...
export function startSecurityAlertsJob() {
  console.log(
    "[Security Alerts Job] Initializing (every 15 min, with exponential backoff)..."
  );
