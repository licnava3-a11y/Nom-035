 ...
 */
async function sendReminderEmail(action: any, responsible: any, daysUntilDue: number) {
  ...
  try {
    ...
    console.error(`[Corrective Actions Reminders] Error sending email to ${responsible.email}:`, error);
    return false;
  }
}
...
async function sendCoordinatorWeeklySummary() {
  ...
  try {
    ...
    if (overdueActions.length === 0) {
      console.log('[Corrective Actions Reminders] No overdue actions, skipping coordinator summary');
      return true;
    }
          ...
          <div class="content">
            ...
            ${overdueActions.map(action => {
              ...
              const daysOverdue = Math.ceil(
                (Date.now() - new Date(action.dueDate || Date.now()).getTime()) / ONE_DAY_MS
              );
    ...
    console.log(`[Corrective Actions Reminders] Weekly summary sent to coordinator: ${overdueActions.length} overdue actions`);
    return true;

  } catch (error) {
    console.error('[Corrective Actions Reminders] Error sending coordinator summary:', error);
    return false;
  }
}
...
export async function runCorrectiveActionsRemindersCheck(): Promise<ReminderResult> {
  console.log('[Corrective Actions Reminders] Starting automated reminders check...');
  ...
  try {
    ...
    console.log(`[Corrective Actions Reminders] Found ${upcomingActions.length} actions due in 7 days`);

    for (const action of upcomingActions) {
      if (!action.responsibleUserId) {
        console.warn(`[Corrective Actions Reminders] No responsible user assigned for action ${action.id}`);
        result.errors.push(`No responsible user assigned for action ${action.id}`);
        continue;
      }
      ...
      if (!responsible || !responsible.email) {
        console.warn(`[Corrective Actions Reminders] No responsible user found for action ${action.id}`);
        result.errors.push(`No responsible user found for action ${action.id}`);
        continue;
      }

      if (!action.dueDate) {
        console.warn(`[Corrective Actions Reminders] No due date for action ${action.id}`);
      ...
      if (sent) {
        ...
        console.log(`[Corrective Actions Reminders] Reminder sent for action ${action.id} to ${responsible.email}`);
      } else {
    ...
    console.log(`[Corrective Actions Reminders] Found ${actionsDueToday.length} actions due today`);
    ...
    for (const action of actionsDueToday) {
      ...
      if (sent) {
        result.dueTodaySent++;
        console.log(`[Corrective Actions Reminders] Due today alert sent for action ${action.id} to ${responsible.email}`);
      } else {
        result.errors.push(`Failed to send due today alert for action ${action.id}`);
      }
    }

    console.log(`[Corrective Actions Reminders] Check completed: ${result.remindersSent} reminders sent, ${result.dueTodaySent} due today alerts sent`);
    return result;

  } catch (error) {
    console.error('[Corrective Actions Reminders] Error running automated reminders check:', error);
    result.success = false;
...
export function startCorrectiveActionsRemindersJob() {
  console.log('[Corrective Actions Reminders] Initializing automated reminders job (daily at 8:00 AM)...');
  ...
  console.log(`[Corrective Actions Reminders] First execution scheduled for ${next8AM.toLocaleString('es-MX')}`);
  console.log('[Corrective Actions Reminders] Automated reminders job started successfully');
  ...
  console.log(`[Corrective Actions Reminders] Weekly coordinator summary scheduled for ${nextMonday.toLocaleString('es-MX')}`);
}
