...
function classifyRiskLevel(
  score: number,
  riskTable: Array<{ level: string; min: number; max: number }>
): string {
  const match = riskTable.find(
...
export function calculateGuideIIResults(answers: Record<number, string>) {
  ...
  if (answeredQuestions < totalQuestions) {
    throw new Error(
      `Faltan respuestas. Se requieren ${totalQuestions} respuestas, se recibieron ${answeredQuestions}.`
    );
  }
  ...
  const finalScore = Object.values(answers).reduce(
    (sum: any, answer: any, index: number) => {
...
function generateRecommendations(
  ...
  switch (finalRiskLevel) {
    case "Muy alto":
      recommendations.push(
        "🚨 ACCIÓN INMEDIATA REQUERIDA: Se detectó un nivel de riesgo psicosocial MUY ALTO. Es necesario implementar medidas correctivas urgentes."
      );
      recommendations.push(
        "Realizar evaluación clínica especializada a los trabajadores afectados."
      );
      recommendations.push(
        "Implementar programa de intervención inmediata con seguimiento semanal."
      );
      break;
    case "Alto":
      recommendations.push(
        "⚠️ ATENCIÓN PRIORITARIA: Se detectó un nivel de riesgo psicosocial ALTO. Se requieren acciones correctivas en el corto plazo."
      );
      recommendations.push(
        "Diseñar e implementar plan de acción correctivo con responsables y fechas límite."
      );
      recommendations.push(
        "Realizar seguimiento mensual de la evolución del riesgo."
      );
      break;
    case "Medio":
      recommendations.push(
        "⚡ ACCIÓN PREVENTIVA: Se detectó un nivel de riesgo psicosocial MEDIO. Se recomienda implementar acciones preventivas."
      );
    ...
    case "Bajo":
      recommendations.push(
        "✅ MANTENER CONDICIONES: Se detectó un nivel de riesgo psicosocial BAJO. Continuar con las prácticas actuales."
      );
      recommendations.push("Implementar acciones de mejora continua.");
      break;
    case "Nulo o despreciable":
      recommendations.push(
        "✅ EXCELENTE: No se detectaron factores de riesgo psicosocial significativos."
      );
      recommendations.push(
        "Mantener las buenas prácticas actuales y compartir experiencias con otras áreas."
      );
      break;
  }
  ...
  if (
    domainRiskLevels.condicionesAmbiente === "Alto" ||
    domainRiskLevels.condicionesAmbiente === "Muy alto"
  ) {
    recommendations.push(
      "🏭 AMBIENTE DE TRABAJO: Realizar inspección de seguridad e higiene. Mejorar condiciones físicas del lugar de trabajo (iluminación, ventilación, ruido, temperatura)."
    );
  }

  if (
    domainRiskLevels.cargaTrabajo === "Alto" ||
    domainRiskLevels.cargaTrabajo === "Muy alto"
  ) {
    recommendations.push(
      "📊 CARGA DE TRABAJO: Redistribuir cargas de trabajo. Implementar pausas activas. Revisar procesos para eliminar actividades innecesarias."
    );
  }

  if (
    domainRiskLevels.faltaControl === "Alto" ||
    domainRiskLevels.faltaControl === "Muy alto"
  ) {
    recommendations.push(
      "🎯 CONTROL SOBRE EL TRABAJO: Aumentar autonomía de los trabajadores. Implementar esquemas de trabajo flexible. Promover la participación en la toma de decisiones."
    );
  }

  if (
    domainRiskLevels.jornadasTrabajo === "Alto" ||
    domainRiskLevels.jornadasTrabajo === "Muy alto"
  ) {
    recommendations.push(
      "⏰ JORNADAS DE TRABAJO: Reducir horas extras. Respetar días de descanso. Implementar esquemas de trabajo que permitan balance vida-trabajo."
    );
  }

  if (
    domainRiskLevels.interferenciaFamilia === "Alto" ||
    domainRiskLevels.interferenciaFamilia === "Muy alto"
  ) {
    recommendations.push(
      "👨‍👩‍👧‍👦 BALANCE TRABAJO-FAMILIA: Implementar políticas de flexibilidad laboral. Ofrecer apoyo para cuidado de dependientes. Respetar horarios de salida."
    );
  }

  if (
    domainRiskLevels.liderazgo === "Alto" ||
    domainRiskLevels.liderazgo === "Muy alto"
  ) {
    recommendations.push(
      "👔 LIDERAZGO: Capacitar a líderes en habilidades de comunicación y gestión de equipos. Implementar evaluación 360° de líderes. Fomentar liderazgo participativo."
    );
  }

  if (
    domainRiskLevels.relacionesTrabajo === "Alto" ||
    domainRiskLevels.relacionesTrabajo === "Muy alto"
  ) {
    recommendations.push(
      "🤝 RELACIONES EN EL TRABAJO: Implementar actividades de integración y trabajo en equipo. Establecer canales de comunicación efectivos. Promover cultura de respeto y colaboración."
    );
  }

  if (
    domainRiskLevels.violencia === "Alto" ||
    domainRiskLevels.violencia === "Muy alto"
  ) {
    recommendations.push(
      "🚨 VIOLENCIA LABORAL: ACCIÓN URGENTE - Activar protocolo de atención a violencia laboral. Realizar investigación inmediata. Implementar medidas de protección a víctimas. Capacitar en prevención de violencia."
    );
  }
 ...
 */
export function validateGuideIIAnswers(answers: Record<number, string>): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  for (let i = 1; i <= 46; i++) {
    if (!answers[i]) {
      ...
      errors.push(
        `Respuesta inválida para la pregunta ${i}: "${answers[i]}". Debe ser una de: ${validAnswers.join(", ")}`
      );
    }
  }
