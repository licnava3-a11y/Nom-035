...
function calcularPuntajeDimension(
  valores: number[],
  maxPosible: number
): number {
  if (maxPosible === 0) return 0;
  return parseFloat(
    ((valores.reduce((s, v) => s + v, 0) / maxPosible) * 100).toFixed(2)
  );
}
...
function requierePlan(
  cat: { nivel: { nivel: string } },
  doms: Array<{ nivel: { nivel: string } }>,
  dims: Array<{ nivel: { nivel: string } }>
): boolean {
  return (
    cat.nivel.nivel === "Alto" ||
    cat.nivel.nivel === "Muy Alto" ||
...
describe("getNivelRiesgo — Tabla de niveles NOM-035", () => {
  ...
  casos.forEach(([p, e]) =>
    it(`puntaje ${p} → ${e}`, () => expect(getNivelRiesgo(p).nivel).toBe(e))
  );
});

describe("calcularPuntajeDimension", () => {
  it("todos en 0 → 0", () =>
    expect(calcularPuntajeDimension([0, 0, 0, 0], 16)).toBe(0));
  it("todos en máximo → 100", () =>
    expect(calcularPuntajeDimension([4, 4, 4, 4], 16)).toBe(100));
  it("mitad → 50", () =>
    expect(calcularPuntajeDimension([2, 2, 2, 2], 16)).toBe(50));
  it("maxPosible 0 → 0", () => expect(calcularPuntajeDimension([], 0)).toBe(0));
  it("mixtos → 62.5", () =>
    expect(calcularPuntajeDimension([1, 2, 3, 4], 16)).toBe(62.5));
});

describe("calcularPuntajeDominio", () => {
  it("un solo valor → mismo", () =>
    expect(calcularPuntajeDominio([75])).toBe(75));
  it("promedio de dos", () =>
    expect(calcularPuntajeDominio([60, 80])).toBe(70));
...
describe("requierePlanObligatorio", () => {
  it("todo Bajo → false", () =>
    expect(
      requierePlan(
        { nivel: getNivelRiesgo(20) },
        [{ nivel: getNivelRiesgo(20) }],
        [{ nivel: getNivelRiesgo(30) }]
      )
    ).toBe(false));
  it("categoría Alto → true", () =>
    expect(requierePlan({ nivel: getNivelRiesgo(70) }, [], [])).toBe(true));
  it("dominio Muy Alto → true", () =>
    expect(
      requierePlan(
        { nivel: getNivelRiesgo(20) },
        [{ nivel: getNivelRiesgo(90) }],
        []
      )
    ).toBe(true));
  it("dimensión Alto → true", () =>
    expect(
      requierePlan(
        { nivel: getNivelRiesgo(20) },
        [{ nivel: getNivelRiesgo(50) }],
        [{ nivel: getNivelRiesgo(65) }]
      )
    ).toBe(true));
  it("todos Medio → false", () =>
    expect(
      requierePlan(
        { nivel: getNivelRiesgo(55) },
        [{ nivel: getNivelRiesgo(55) }],
        [{ nivel: getNivelRiesgo(55) }]
      )
    ).toBe(false));
...
describe("Comparativa Q1/Q2 — filtro de fechas", () => {
  function inRange(d: Date, f: Date, t: Date) {
    return d >= f && d <= t;
  }
...
describe("Notificación por correo — construcción del cuerpo", () => {
  ...
  it("contiene el folio", () =>
    expect(buildBody("NOM035-DICT-001", "abc123", "Dr. Pérez")).toContain(
      "NOM035-DICT-001"
    ));
  it("contiene el hash", () =>
    expect(buildBody("NOM035-DICT-001", "abc123", "Dr. Pérez")).toContain(
