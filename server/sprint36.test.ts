...
describe("Sprint 36 — Indicador variación % rotación", () => {
  ...
  it("prevYearTurnoverRate es 0 cuando no hay empleados", () => {
    ...
    const prevYearTurnoverRate = totalEmployees > 0
  ...
  it("calcula prevYearTurnoverRate como porcentaje entero", () => {
    ...
    const prevYearTurnoverRate = Math.round((prevYearInactiveCount / totalEmployees) * 100);
...
describe("Sprint 36 — Badge variación % en KPI card", () => {
  function getBadgeConfig(turnoverChange: number) {
    if (turnoverChange > 0) return { class: "bg-red-100 text-red-700", symbol: "▲" };
    if (turnoverChange < 0) return { class: "bg-emerald-100 text-emerald-700", symbol: "▼" };
...
describe("Sprint 36 — Estados válidos de Plan de Acción", () => {
  const VALID_STATUSES = ["pendiente", "en_progreso", "completado", "cancelado"];
