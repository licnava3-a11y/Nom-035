...
describe('correctiveActions.generatePDF', () => {
  ...
  beforeAll(async () => {
    ...
    caller = appRouter.createCaller({
      user: { id: 1, email: 'test@nom035.com', name: 'Test User', role: 'admin' },
    } as any);
  ...
  afterAll(async () => {
    ...
    await db.delete(correctiveActions).where(eq(correctiveActions.id, testActionId));
  });

  it('should generate PDF for corrective action', async () => {
    const result = await caller.correctiveActions.generatePDF({ id: testActionId });
  ...
  it('should update action with pdfUrl after generation', async () => {
    ...
    const [action] = await db.select().from(correctiveActions).where(eq(correctiveActions.id, testActionId));
