...
describe("reporte de presupuesto de bundle", () => {
  ...
  it("mide vendor-xlsx de forma independiente en el reporte generado", () => {
    ...
    try {
      ...
      writeFileSync(join(assetsDir, "vendor-xlsx-fixture.js"), "export const workbook = 'xlsx';\n");
      writeFileSync(join(assetsDir, "application-fixture.js"), "export const app = 'nom035';\n");

      const result = spawnSync(process.execPath, [resolve(process.cwd(), "scripts/report-bundle.mjs")], {
        cwd: workspace,
        encoding: "utf8",
      });
      expect(result.status, result.stderr).toBe(0);

      const report = JSON.parse(readFileSync(join(workspace, "reports", "bundle-budget.json"), "utf8"));
