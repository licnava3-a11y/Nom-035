...
export async function generateConstitutiveActPDF(
  data: ConstitutiveActData
): Promise<Buffer> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      doc
        .fontSize(16)
        .font("Helvetica-Bold")
        .text("ACTA CONSTITUTIVA DEL COMITÉ", 50, 140, { align: "center" });
      doc
        .fontSize(14)
        .text("DE SEGURIDAD Y SALUD EN EL TRABAJO", { align: "center" });
      doc.moveDown(2);
      ...
      data.members.forEach((member: any, index: number) => {
        doc.text(`${index + 1}. ${member.name} - ${member.position}`, {
          continued: false,
        });
        doc
          .fontSize(9)
          .fillColor("#666")
          .text(`   Departamento: ${member.department}`, { continued: false });
      ...
      doc.fontSize(10).font("Helvetica");
      doc.text(
        "Siendo las __________ horas del mismo día, se da por concluida la presente reunión, firmando de conformidad los que en ella intervinieron."
      );
      doc.moveDown(2);
      ...
      data.members.forEach((member: any, index: number) => {
        ...
        doc
          .moveTo(currentX, currentY + 40)
          .lineTo(currentX + signatureWidth, currentY + 40)
          .stroke();
        doc.text(member.name, currentX, currentY + 45, {
          width: signatureWidth,
          align: "center",
        });
        doc.text(member.position, currentX, currentY + 58, {
          width: signatureWidth,
          align: "center",
        });
      ...
      doc.image(qrBuffer, doc.page.width - 130, doc.page.height - 130, {
        width: 80,
      });

      // Footer with folio
      doc.fontSize(8).fillColor("#666");
      doc.text(`Folio: ${data.folio}`, 50, doc.page.height - 50, {
        align: "left",
      });
