...
function sectionTitle(doc: InstanceType<typeof PDFDocument>, title: string) {
  ...
  doc
    ...
    .fill("#1e3a5f");
  doc.fillColor("#ffffff").fontSize(11).font("Helvetica-Bold").text(title, doc.page.margins.left + 6, doc.y - 14);
  doc.fillColor("#1a1a2e").font("Helvetica").fontSize(10);
  doc.moveDown(0.6);
}

function fieldRow(doc: InstanceType<typeof PDFDocument>, label: string, value: string | null | undefined) {
  if (!value) return;
...
// ─── Función principal ────────────────────────────────────────────────────────
export async function generateClinicalRecordPDF(data: ClinicalRecordPDFData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc
        ...
        .fontSize(9)
        .font("Helvetica")
        .text("Sistema de Gestión NOM-035 STPS 2018", 0, 30, { align: "center" });
      ...
      doc
        ...
        .font("Helvetica")
        .fillColor("#555")
        .text(`Folio: ${data.folio}   |   Fecha de emisión: ${formatDate(new Date())}`, { align: "center" });

      doc.moveDown(1);

      // Línea divisoria
      doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor("#1e3a5f").lineWidth(1.5).stroke();
      doc.moveDown(0.5);

      // ─── SECCIÓN 1: DATOS DE IDENTIFICACIÓN ──────────────────────────────
      sectionTitle(doc, "1. DATOS DE IDENTIFICACIÓN");
      fieldRow(doc, "Paciente / Empleado", data.record.patientName);
      if (data.record.patientAge) fieldRow(doc, "Edad", `${data.record.patientAge} años`);
      if (data.record.patientContact) fieldRow(doc, "Contacto", data.record.patientContact);
      doc.moveDown(0.3);
      fieldRow(doc, "Profesional tratante", data.record.professionalName);
      if (data.record.professionalLicense) fieldRow(doc, "Cédula profesional", data.record.professionalLicense);
      if (data.record.professionalSpecialty) fieldRow(doc, "Especialidad", data.record.professionalSpecialty);
      fieldRow(doc, "Fecha de apertura", formatDate(data.record.createdAt));
      fieldRow(doc, "Consentimiento informado", data.record.consentSigned ? `Firmado el ${formatDate(data.record.consentSignedAt)}` : "Pendiente");
      ...
      if (data.record.consultationReason) {
        ...
        doc.font("Helvetica").text(data.record.consultationReason, { width: pageW });
        doc.moveDown(0.4);
      }
      if (data.record.medicalHistory) {
        doc.font("Helvetica-Bold").text("Antecedentes médicos:");
        doc.font("Helvetica").text(data.record.medicalHistory, { width: pageW });
        doc.moveDown(0.4);
      }
      if (data.record.personalHistory) {
        doc.font("Helvetica-Bold").text("Antecedentes personales:");
        doc.font("Helvetica").text(data.record.personalHistory, { width: pageW });
        doc.moveDown(0.4);
      }
      ...
      if (data.record.treatmentObjectives || data.record.treatmentActivities) {
        ...
        if (data.record.treatmentObjectives) {
          ...
          doc.font("Helvetica").text(data.record.treatmentObjectives, { width: pageW });
          doc.moveDown(0.4);
        }
        if (data.record.treatmentActivities) {
          doc.font("Helvetica-Bold").text("Actividades / Intervenciones:");
          doc.font("Helvetica").text(data.record.treatmentActivities, { width: pageW });
        }
      }
      ...
      if (data.evaluations.length > 0) {
        ...
        data.evaluations.forEach((ev, idx) => {
          doc.fontSize(10).font("Helvetica-Bold").text(`${idx + 1}. ${ev.testName}  (${formatDate(ev.evaluationDate)})`);
          if (ev.result) {
            doc.font("Helvetica").text(`Resultado: ${ev.result}`, { width: pageW });
          }
          if (ev.interpretation) {
            doc.font("Helvetica").text(`Interpretación: ${ev.interpretation}`, { width: pageW });
          }
          doc.moveDown(0.4);
        });
      }
      ...
      if (data.sessionNotes.length > 0) {
        ...
        data.sessionNotes.forEach((note, idx) => {
          ...
          if (note.nextAppointment) {
            doc.fillColor("#1e3a5f").text(`Próxima cita: ${formatDate(note.nextAppointment)}`).fillColor("#1a1a2e");
          }
          doc.moveDown(0.5);
        });
      }
      ...
      doc.moveDown(2);
      doc.moveTo(50, doc.y).lineTo(doc.page.width - 50, doc.y).strokeColor("#1e3a5f").lineWidth(1).stroke();
      doc.moveDown(1);
      ...
      if (data.record.professionalLicense) {
        doc.fontSize(9).font("Helvetica").text(`Cédula Profesional: ${data.record.professionalLicense}`, { align: "center" });
      }
      if (data.record.professionalSpecialty) {
        doc.fontSize(9).font("Helvetica").text(data.record.professionalSpecialty, { align: "center" });
      }
