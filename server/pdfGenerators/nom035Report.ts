...
export async function generateNom035Report(
  data: Nom035ReportData
): Promise<{ url: string; key: string; base64: string }> {
  ...
  doc.fontSize(12).text("NOM-035-STPS-2018, Numeral 7.5", { align: "center" });
  ...
  if (data.company.workCenter)
    addField(doc, "Centro de Trabajo:", data.company.workCenter);
  if (data.company.stpsRegistration)
    addField(doc, "Registro STPS:", data.company.stpsRegistration);
  addField(
    doc,
    "Total de Trabajadores:",
    data.company.totalEmployees.toString()
  );
  addField(
    doc,
    "Período del Informe:",
  ...
  if (data.surveyResults.guideI.applied) {
    doc.text(
      `• Guía de Referencia I: Identificación de trabajadores expuestos a acontecimientos traumáticos severos`,
      { indent: 40 }
    );
    doc.text(
      `  Casos identificados: ${data.surveyResults.guideI.casesIdentified}`,
      { indent: 60 }
    );
  }
  if (data.surveyResults.guideII.applied) {
    doc.text(
      `• Guía de Referencia II: Identificación y análisis de factores de riesgo psicosocial`,
      { indent: 40 }
    );
    doc.text(
      `  Trabajadores evaluados: ${data.surveyResults.guideII.totalResponses}`,
      { indent: 60 }
    );
  }
  if (data.surveyResults.guideIII.applied) {
    doc.text(
      `• Guía de Referencia III: Identificación y análisis de factores de riesgo psicosocial y evaluación del entorno organizacional`,
      { indent: 40 }
    );
    doc.text(
      `  Trabajadores evaluados: ${data.surveyResults.guideIII.totalResponses}`,
      { indent: 60 }
    );
  }
  ...
  doc.fontSize(10).font("Helvetica");
  doc.text("Se identificaron los siguientes factores de riesgo psicosocial:", {
    indent: 20,
  });
  doc.moveDown(1);
  ...
  // Línea separadora
  doc
    .moveTo(72, tableTop + 15)
    .lineTo(492, tableTop + 15)
    .stroke();
  ...
  doc
    .fontSize(12)
    .font("Helvetica-Bold")
    .text("FIRMAS DE RESPONSABLES", { align: "center" });
  doc.moveDown(2);
  ...
  for (let i = 0; i < data.signers.length; i++) {
    ...
    // Línea de firma
    doc
      .moveTo(sigX, sigY + signatureHeight)
      .lineTo(sigX + signatureWidth, sigY + signatureHeight)
      .stroke();

    // Nombre y cargo
    doc.fontSize(10).font("Helvetica-Bold");
    doc.text(signer.name, sigX, sigY + signatureHeight + 5, {
      width: signatureWidth,
      align: "center",
    });
    doc.fontSize(9).font("Helvetica");
    doc.text(signer.position, sigX, sigY + signatureHeight + 20, {
      width: signatureWidth,
      align: "center",
    });
    doc.text(
  ...
  doc
    .fontSize(8)
    .font("Helvetica")
    .text("Código de Validación NOM-151", { align: "center" });
