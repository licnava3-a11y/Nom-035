...
function buildMockDb() {
  const db: any = {};
  const chainMethods = ["select", "from", "where", "orderBy", "leftJoin", "set", "delete", "offset"];
  chainMethods.forEach((m) => { db[m] = vi.fn().mockReturnValue(db); });
...
vi.mock("../drizzle/schema", () => ({
  minuteRecipients: { id: "id", email: "email", name: "name", position: "position", department: "department", isActive: "isActive" },
  minuteDispatches: { id: "id", minuteId: "minuteId", recipientId: "recipientId", sentAt: "sentAt", readAt: "readAt", status: "status" },
  meetingMinutes: { id: "id", folio: "folio", title: "title", meetingDate: "meetingDate", meetingType: "meetingType" },
}));
vi.mock("drizzle-orm", () => ({
...
describe("minuteRecipients.bulkImport", () => {
  beforeEach(() => { mockDb = buildMockDb(); });

  it("crea un nuevo destinatario cuando el correo no existe", async () => {
    mockDb.limit.mockResolvedValueOnce([]);
    mockDb.values.mockResolvedValueOnce([{ insertId: 10 }]);
    let created = 0; let updated = 0;
    const rows = [{ name: "Ana Torres", email: "a.torres@empresa.com", position: "Directora", department: "Dirección" }];
    for (const row of rows) {
      const existing = await mockDb.select().from().where().limit(1);
      if (existing.length > 0) { updated++; } else { await mockDb.insert().values({}); created++; }
    }
  ...
  it("actualiza un destinatario existente cuando el correo ya existe", async () => {
    ...
    let created = 0; let updated = 0;
    const rows = [{ name: "Ana Torres Actualizada", email: "a.torres@empresa.com", position: "CEO", department: null }];
    for (const row of rows) {
      const existing = await mockDb.select().from().where().limit(1);
      if (existing.length > 0) { await mockDb.update().set({}).where(); updated++; } else { created++; }
    }
  ...
  it("devuelve resumen con created, updated y errors", async () => {
    ...
    expect({ created, updated, errors, total: rows.length }).toMatchObject({ created: 2, updated: 0, total: 2 });
  });
  ...
  it("registra errores sin detener el proceso completo", async () => {
    ...
    const rows = [
      { name: "Válido", email: "valido@empresa.com", position: "Cargo", department: null },
      { name: "Error", email: "error@empresa.com", position: "Cargo", department: null },
    ];
    for (const row of rows) {
      try {
        const existing = await mockDb.select().from().where().limit(1);
        if (existing.length === 0) { await mockDb.insert().values({}); created++; }
      } catch (e: any) { errors.push(`${row.email}: ${e.message}`); }
    }
...
describe("minuteRecipients.getDispatches", () => {
  ...
  it("devuelve arreglo vacío si no hay despachos para el destinatario", () => {
    ...
    expect({ total, read: readCount, unread: total - readCount }).toEqual({ total: 0, read: 0, unread: 0 });
  });
...
describe("meetingMinutes.addRecipients", () => {
  ...
  it("lanza error si no hay destinatarios activos en los IDs proporcionados", () => {
    expect(() => {
      const r: any[] = [];
      if (r.length === 0) throw new Error("No se encontraron destinatarios activos");
