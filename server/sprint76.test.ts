...
function detectarCambios(
  ...
  if (updates.estado !== undefined && updates.estado !== current.estado) {
    entries.push({ campo: "estado", valorAnterior: current.estado, valorNuevo: updates.estado, changedByName, nota: nota ?? null });
  }
  if (updates.responsable !== undefined && updates.responsable !== current.responsable) {
    entries.push({ campo: "responsable", valorAnterior: current.responsable, valorNuevo: updates.responsable, changedByName, nota: nota ?? null });
  }
  if (updates.plazo !== undefined) {
    const prevPlazo = current.plazo
      ? (current.plazo instanceof Date ? current.plazo.toISOString().split("T")[0] : String(current.plazo))
      : null;
    if (prevPlazo !== updates.plazo) {
      entries.push({ campo: "plazo", valorAnterior: prevPlazo, valorNuevo: updates.plazo, changedByName, nota: nota ?? null });
    }
  }
  if (updates.prioridad !== undefined && updates.prioridad !== current.prioridad) {
    entries.push({ campo: "prioridad", valorAnterior: current.prioridad, valorNuevo: updates.prioridad, changedByName, nota: nota ?? null });
  }
  if (updates.observaciones !== undefined && updates.observaciones !== current.observaciones) {
    entries.push({ campo: "observaciones", valorAnterior: current.observaciones, valorNuevo: updates.observaciones, changedByName, nota: nota ?? null });
  }

  // Si hay nota pero no hubo cambios de campo, registrar la nota como entrada independiente
  if (nota && entries.length === 0) {
    entries.push({ campo: "observaciones", valorAnterior: null, valorNuevo: nota, changedByName, nota });
  }
 ...
 */
function formatearValorCampo(campo: CampoHistorial, valor: string | null): string {
  if (!valor) return "—";
  ...
  if (campo === "plazo") {
    ...
    return isNaN(d.getTime()) ? valor : d.toLocaleDateString("es-MX", { year: "numeric", month: "short", day: "numeric" });
  }
...
describe("Sprint 76 — Bitácora de Historial de Acciones NOM-035", () => {
  ...
  describe("detectarCambios()", () => {
    ...
    it("detecta cambio de estado", () => {
      const entries = detectarCambios(baseAction, { estado: "en_proceso" }, "Admin");
    ...
    it("no registra si el estado es el mismo", () => {
      const entries = detectarCambios(baseAction, { estado: "no_iniciada" }, "Admin");
      expect(entries).toHaveLength(0);
    });

    it("detecta cambio de responsable", () => {
      const entries = detectarCambios(baseAction, { responsable: "María García" }, "Admin");
      expect(entries).toHaveLength(1);
    ...
    it("detecta cambio de plazo", () => {
      const entries = detectarCambios(baseAction, { plazo: "2026-09-15" }, "Admin");
      expect(entries).toHaveLength(1);
    ...
    it("no registra plazo si no cambia", () => {
      const entries = detectarCambios(baseAction, { plazo: "2026-06-30" }, "Admin");
      expect(entries).toHaveLength(0);
    });

    it("detecta cambio de prioridad", () => {
      const entries = detectarCambios(baseAction, { prioridad: "alta" }, "Admin");
      expect(entries).toHaveLength(1);
    ...
    it("detecta cambio de observaciones", () => {
      const entries = detectarCambios(baseAction, { observaciones: "Acción en revisión" }, "Admin");
      expect(entries).toHaveLength(1);
    ...
    it("detecta múltiples cambios simultáneos", () => {
      const entries = detectarCambios(
        baseAction,
        { estado: "en_proceso", responsable: "María García", prioridad: "alta" },
        "Admin"
      );
    ...
    it("registra nota sola si no hay cambios de campo", () => {
      const entries = detectarCambios(baseAction, {}, "Admin", "Revisión semanal completada");
      expect(entries).toHaveLength(1);
      expect(entries[0].campo).toBe("observaciones");
    ...
    it("maneja responsable null correctamente", () => {
      ...
      const entries = detectarCambios(actionSinResponsable, { responsable: "Nuevo Responsable" }, "Admin");
      expect(entries[0].valorAnterior).toBeNull();
    ...
    it("maneja plazo null correctamente", () => {
      ...
      const entries = detectarCambios(actionSinPlazo, { plazo: "2026-12-31" }, "Admin");
      expect(entries[0].valorAnterior).toBeNull();
  ...
  describe("formatearValorCampo()", () => {
    ...
    it("no trunca textos cortos", () => {
      ...
      expect(formatearValorCampo("observaciones", textoCorto)).toBe("Texto normal");
    });

    it("maneja estados desconocidos retornando el valor original", () => {
      expect(formatearValorCampo("estado", "estado_desconocido")).toBe("estado_desconocido");
    });
  });
  ...
  describe("generarResumenCambios()", () => {
    ...
    it("genera resumen de múltiples cambios separados por ';'", () => {
      const entries: HistoryEntry[] = [
        { campo: "estado", valorAnterior: "no_iniciada", valorNuevo: "en_proceso", changedByName: "Admin" },
        { campo: "prioridad", valorAnterior: "media", valorNuevo: "alta", changedByName: "Admin" },
      ];
      const resumen = generarResumenCambios(entries);
  ...
  describe("Validaciones de integridad de la bitácora", () => {
    it("todos los campos del enum son válidos", () => {
      const camposValidos: CampoHistorial[] = [
        "estado", "responsable", "plazo", "prioridad", "objetivo",
        "observaciones", "evidencia_agregada", "evidencia_eliminada", "creacion",
      ];
    ...
    it("el autor del cambio se registra correctamente", () => {
      ...
      const entries = detectarCambios(baseAction, { estado: "cumplida" }, "María García");
