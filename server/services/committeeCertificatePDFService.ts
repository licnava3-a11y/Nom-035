 ...
 */
export async function generateCommitteeCertificatePDF(data: CertificateData): Promise<string> {
    ...
    try {
      ...
      doc.on('end', async () => {
        try {
          ...
          const { url } = await storagePut(`committee-certificates/${fileName}`, pdfBuffer, 'application/pdf');
          resolve(url);
        } catch (error) {
          reject(error);
        }
      ...
      doc
        .fontSize(14)
        .font('Helvetica')
        .text('Por haber asistido y completado satisfactoriamente el programa de capacitación:', { align: 'center' })
        .moveDown(0.5);
