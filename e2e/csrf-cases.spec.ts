...
import { removeCSRFHeader, invalidateCSRFToken, waitForCSRFToken } from './helpers/csrf';
...
test.describe('CSRF Protection - Casos NOM-035', () => {
  ...
  test('debe rechazar creación de caso sin token CSRF', async ({ page }) => {
    ...
    await expect(page.locator('text=/CSRF|Token|403|Forbidden/i')).toBeVisible({ timeout: 5000 });
  ...
  test('debe permitir creación de caso con token CSRF válido', async ({ page }) => {
    ...
    await page.fill('[name="description"]', 'Caso de prueba con token CSRF válido');
    ...
    await expect(page.locator('text=/creado exitosamente|success/i')).toBeVisible({ timeout: 5000 });
  ...
  test('debe rechazar creación de caso con token CSRF inválido', async ({ page }) => {
    ...
    await page.fill('[name="description"]', 'Caso de prueba con token inválido');
    await page.selectOption('[name="departmentId"]', '1');
    ...
    // Verificar que se muestre error 403
    await expect(page.locator('text=/CSRF|Token|403|inválido/i')).toBeVisible({ timeout: 5000 });
  });
  
  test('debe actualizar caso existente con token CSRF válido', async ({ page }) => {
    // Esperar a que el token CSRF se cargue
    ...
    await page.fill('[name="description"]', 'Descripción actualizada con token CSRF');
    
    // Guardar cambios
    await page.click('[data-testid="save-case-button"]');
    
    // Verificar mensaje de éxito
    await expect(page.locator('text=/actualizado exitosamente|success/i')).toBeVisible({ timeout: 5000 });
  });
  
  test('debe rechazar actualización de caso sin token CSRF', async ({ page }) => {
    // Seleccionar un caso existente
    const firstCase = page.locator('[data-testid="case-item"]').first();
    await firstCase.click();
    ...
    // Modificar descripción
    await page.fill('[name="description"]', 'Intento de actualización sin token');
  ...
  test('debe asignar caso con token CSRF válido', async ({ page }) => {
    ...
    const unassignedCase = page.locator('[data-testid="case-item"][data-status="open"]').first();
    ...
    // Verificar mensaje de éxito
    await expect(page.locator('text=/asignado exitosamente|success/i')).toBeVisible({ timeout: 5000 });
  });
  
  test('debe manejar múltiples mutations consecutivas con mismo token', async ({ page }) => {
    // Esperar a que el token CSRF se cargue
    ...
    await page.selectOption('[name="category"]', 'harassment');
    await page.click('[data-testid="submit-case-button"]');
    await expect(page.locator('text=/creado exitosamente/i')).toBeVisible({ timeout: 5000 });
