...
test.describe('CSRF Token Renewal', () => {
  ...
  test('debe mantener el mismo token durante la sesión activa', async ({ page }) => {
    // Navegar a la página principal
    await page.goto('/');
  ...
  test('debe renovar token automáticamente antes de expiración', async ({ page }) => {
  ...
  test('debe manejar token expirado con renovación automática', async ({ page }) => {
    ...
    await page.fill('[name="description"]', 'Caso de prueba con token expirado');
    ...
    await expect(page.locator('text=/sesión ha expirado|intenta nuevamente/i')).toBeVisible({ timeout: 5000 });
  ...
  test('debe incluir token CSRF en todas las mutations', async ({ page }) => {
    ...
    const hasCSRFHeader = postRequests.some(r => 
